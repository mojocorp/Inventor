...
public:
    ...
    // this converts an Qt event into an SoEvent.
    // this returns NULL if the event is not from this device.
    virtual const SoEvent * translateEvent(QEvent *qevent) = 0;
...
// Inline functions
void
SoQtDevice::setEventPosition(SoEvent *event, int x, int y)
{
    event->setPosition(SbVec2s(x, (winSize[1] - 1) - y));

    lastMousePos.setValue(x, y);
}

void
SoQtDevice::fillInEventState (SoEvent* event, QInputEvent* ie)
{
