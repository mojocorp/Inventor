...
class SOQT_EXPORT SoQtRenderArea : public SoQtGLWidget {
    Q_OBJECT
...
public:
  ...
  protected:
    ...
    SbPList* m_deviceList;      // TGS compat - same as deviceList
...
private:
    ...
    SbBool  	    	clearFirst, clearOverlayFirst, clearZFirst;	// call clear() before rendering
    SbBool		autoRedraw;	// if TRUE, then we automatically redraw

    SoQtMouse		*mouseDevice;
    SoQtKeyboard	*keybdDevice;
