...
class SoQtWalkViewer : public SoQtConstrainedViewer {
 public:
    ...
    // C-api: interpret #define SoQtWalkVwrCreateStd(parent,name) SoQtWalkVwrCreate(parent,name,TRUE,SO_Qt_FULL_VWR_BUILD_ALL,SO_Qt_VWR_BROWSER)
    ~SoQtWalkViewer();
 ...
 protected:
    ...
    // redefine this to add the eQtra thumbwheel on the left side
 ...
 private:
    ...
    QCursor	    walkCursor, panCursor, tiltCursor, seekCursor, upCursor;
    SbVec2s	    locator; // mouse position
    SbVec2s	    prevPos; // mouse previous location
    SbVec2s	    startPos; // mouse starting position
    
    // eQtra decoration vars
    int		    heightWheelVal;
    static void	    heightWheelCB(QWidget*, SoQtWalkViewer *p, void *d);
    ...
    static void		setFocalPointFinishCallback(void *, SoQtViewer *);
