...
///
///
/// Also, some methods of <tt>SoPath</tt> may not be called on an <tt>SoNodeKitPath</tt>.
/// Any methods which take a regular <tt>SoNode</tt> as an argument (except for
/// #setHead()) are not accessible, and replaced by methods that take an
/// <tt>SoBaseKit</tt> as an argument instead.  Methods which allow the programmer to
/// refer to the child index of a node beneath its parent are also inaccessible;
/// since a <tt>SoNodeKitPath</tt> only shows nodekits and hides any private parts,
/// successive nodekits in the path may not actually be parent and child.
/// \sa SoBaseKit, SoPath, SoRayPickAction, SoSearchAction
class INVENTOR_API SoNodeKitPath : public SoPath {
  ...
  public:
    ...
    /// #getHead() is not redefined from \c SoPath,
    /// since an \c SoNodeKitPath need not begin with a nodekit; the restriction is
    ...
    /// equivalent to #setHead(childKit).
