...
void
SoLineSet::GLRenderGeneric(SoGLRenderAction* action )
{
    SoState *state = action->getState();
    ...
    const char *colorPtr = vpCache.getColors(materialBinding == PER_VERTEX ? startIndex.getValue() : 0);
    const unsigned int colorStride = vpCache.getColorStride();
    SoVPCacheFunc *const colorFunc = vpCache.colorFunc;

    const char *normalPtr = vpCache.getNormals(normalBinding == PER_VERTEX ? startIndex.getValue() : 0);
    ...
    for (int polyline = 0; polyline < numPolylines; polyline++) {
        ...
        for (int vert = 0; vert < vertsInLine; vert++) {
            ...
            if (materialBinding == PER_VERTEX || materialBinding == PER_SEGMENT) {
                (*colorFunc)(colorPtr);
                colorPtr += colorStride;
            }
            if (normalBinding == PER_VERTEX || normalBinding == PER_SEGMENT) {
                (*normalFunc)(normalPtr);
                normalPtr += normalStride;
            }
