...
void
SbRotationd::getValue(SbMatrix &matrix) const
//
////////////////////////////////////////////////////////////////////////
{
    matrix[0][0] = float(1.0 - 2.0 * (quat[1] * quat[1] + quat[2] * quat[2]));
    matrix[0][1] = float(      2.0 * (quat[0] * quat[1] + quat[2] * quat[3]));
    matrix[0][2] = float(      2.0 * (quat[2] * quat[0] - quat[1] * quat[3]));
    matrix[0][3] = 0.0f;

    matrix[1][0] = float(      2.0 * (quat[0] * quat[1] - quat[2] * quat[3]));
    matrix[1][1] = float(1.0 - 2.0 * (quat[2] * quat[2] + quat[0] * quat[0]));
    matrix[1][2] = float(      2.0 * (quat[1] * quat[2] + quat[0] * quat[3]));
    matrix[1][3] = 0.0f;

    matrix[2][0] = float(      2.0 * (quat[2] * quat[0] + quat[1] * quat[3]));
    matrix[2][1] = float(      2.0 * (quat[1] * quat[2] - quat[0] * quat[3]));
    matrix[2][2] = float(1.0 - 2.0 * (quat[1] * quat[1] + quat[0] * quat[0]));
    matrix[2][3] = 0.0f;
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (!ok) {
        SoDebugError::post("SbRotationd::setValue(const SbMatrix &)",
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (m[0][0]+m[1][1]+m[2][2] > m[i][i]) {
	// Compute w first:
        quat[3] = (double)(sqrt(m[0][0]+m[1][1]+m[2][2]+m[3][3])/2.0);

	// And compute other values:
        quat[0] = (double)(m[1][2]-m[2][1])/(4*quat[3]);
        quat[1] = (double)(m[2][0]-m[0][2])/(4*quat[3]);
        quat[2] = (double)(m[0][1]-m[1][0])/(4*quat[3]);
    }
    else {
	// Compute x, y, or z first:
	j = (i+1)%3; k = (i+2)%3;
	...
	// Compute first value:
        quat[i] = (double)(sqrt(m[i][i]-m[j][j]-m[k][k]+m[3][3])/2.0);
       
	// And the others:
        quat[j] = (double)(m[i][j]+m[j][i])/(4*quat[i]);
        quat[k] = (double)(m[i][k]+m[k][i])/(4*quat[i]);

        quat[3] = (double)(m[j][k]-m[k][j])/(4*quat[i]);
    }
    ...
    for (i = 0; i < 4 && ok; i++) {
	for (j = 0; j < 4 && ok; j++) {
            if (fabs(m[i][j]-check[i][j]) > 1.0e-5)
		ok = FALSE;
	}
    }
    if (!ok) {
...
////////////////////////////////////////////////////////////////////////
{
        ...
        for (i = 0; i <4; i++)
                rot.quat[i] = (double)(scalerot0 * rot0.quat[i] + scalerot1 * rot1q[i]);
