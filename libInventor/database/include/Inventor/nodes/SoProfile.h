...
/// \sa SoLinearProfile,SoNurbsProfile,SoNurbsSurface,SoProfileCoordinate2,SoProfileCoordinate3,SoText3
class INVENTOR_API SoProfile : public SoNode{
  ...
  public:
    enum Profile {
        START_FIRST    = SoProfileElement::START_FIRST,     ///< Start a new profile and remove any existing profiles from the current list
        START_NEW      = SoProfileElement::START_NEW,       ///< Start a new profile and add it to the current list
        ADD_TO_CURRENT = SoProfileElement::ADD_TO_CURRENT   ///< Add to end of the last profile in the current list
    };

    // Fields
    SoMFInt32		index;	      ///< Indices into profile coordinates.
    SoSFEnum		linkage;      ///< Specifies connectivity of profile curve with respect to profiles in current list in state.
