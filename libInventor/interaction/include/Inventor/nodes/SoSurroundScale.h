...
/// by its fields. Transform manipulators, such as <tt>SoHandleBoxManip</tt>,
...
/// bounding box.
///
/// For example, when an <tt>SoHandleBoxManip</tt> wants to surround the objects it
...
/// \endcode
/// The <tt>SoHandleBoxDragger</tt> wants to transform the <em>cubeGeom</em> so that it
/// surrounds the <em>movingStuff</em>. So it sets the <em>surroundScale</em> fields to:
/// \code
...
/// So the <tt>SoSurroundScale</tt> will surround the objects below <em>separator</em>, and
/// to the right of <em>handleBoxManip</em>, producing the desired effect.
///
/// \par Action behavior:
...
/// }
/// \endcode
/// \sa SoTransformation, SoTransformManip,SoCenterballDragger, SoCenterballManip,SoHandleBoxDragger,
/// \sa SoHandleBoxManip,SoJackDragger, SoJackManip,SoTabBoxDragger, SoTabBoxManip,SoTrackballDragger,
/// \sa SoTrackballManip,SoTransformBoxDragger, SoTransformBoxManip
class INVENTOR_API SoSurroundScale : public SoTransformation {
  ...
  public:
    ...
    /// looks at the current path in
    /// the action. It travels up this path a distance of \v numNodesUpToContainer
    /// and  applies an \c SoGetBoundingBoxAction to the node that it finds there.
    SoSFInt32		numNodesUpToContainer;

    /// Before applying the \c SoGetBoundingBoxAction (see the
    /// \v numNodesUpToContainer field aove) the surroundScale node
    /// travels up the path a distance of \v numNodesUpToReset
