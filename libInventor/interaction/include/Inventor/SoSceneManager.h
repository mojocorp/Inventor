...
/// The <tt>SoXtRenderArea</tt> class employs a <tt>SoSceneManager</tt>,
...
/// The <tt>SoSceneManager</tt> class is available for programmers not working
/// with the <em>Inventor Xt Component and Utility Library</em>.
...
class INVENTOR_API SoSceneManager {
  public:
    ...
    /// If \a clearWindow is TRUE, this clears the graphics window before rendering.
    /// If \a clearZbuffer is TRUE, the z buffer will be cleared before rendering.
    virtual void	render(SbBool clearWindow = TRUE, SbBool clearZbuffer = TRUE);

    /// Process the passed event by applying an \c SoHandleEventAction
    ...
    /// lighting should be turned off (the \verbatim model field of \c SoLightModel
    /// should be set to \e BASE_COLOR),
    /// and the \c SoColorIndex node should be used to specify colors.
