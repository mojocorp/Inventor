...
/// \ingroup Manips
/// <tt>SoPointLightManip</tt>  is the base class for all <tt>SoPointLight</tt> nodes
...
/// <tt>SoPointLight</tt>, any changes to its fields result in a change of
/// lighting for nodes that follow it in the scene graph.  In this case, the
/// interface edits the #location field.  Also, the color of the
/// manipulator's geometry will reflect the color of the light (but you can
/// not edit the color using this manipulator).
///
/// Typically, you will want to replace a regular <tt>SoPointLight</tt> with an
/// <tt>SoPointLightManip</tt> (as when the user selects a light to be edited),
/// or vice versa (as when the user is done moving the light and the interface
/// should go away).
/// Use the #replaceNode() method to insert a manipulator into a scene graph, and the
/// #replaceManip() method to remove it when done.
///
/// The <tt>SoPointLightManip</tt>  utilizes an <tt>SoPointLightDragger</tt> to
...
/// <tt>SoPointLight</tt>, it alters the state.  The fields values and movement
...
/// see the reference page for <tt>SoPointLightDragger</tt>.  The interfaces of
/// the dragger and the manipulator are identical.
///
/// The <tt>SoPointLightManip</tt> utilizes its dragger by adding it
...
/// #location field of the <em>manipulator</em> will change by the same
/// amount, thus changing the lighting of nodes which follow
/// in the scene graph.
/// Similarly, if you set the #location field of the
/// <tt>SoPointLightManip</tt>, the manipulator will
...
/// when finished.
/// Then the <tt>SoPointLightManip</tt> adds a point light into the state,
/// just like its base class, <tt>SoPointLight</tt>.
/// <b>SoSearchAction</b>
/// Searches just like an <tt>SoPointLight</tt>.  Does not search the dragger,
/// which is a hidden child.
/// <b>SoWriteAction</b>
/// Writes out just like an <tt>SoPointLight</tt>. Does not write the dragger,
...
/// }
/// \endcode
/// \sa SoDragger,SoPointLight,SoPointLightDragger,SoDirectionalLightManip,SoSpotLightManip
class INVENTOR_API SoPointLightManip : public SoPointLight
{
  ...
  public:
    ...
    /// The tail of the path must be an \c SoPointLight node (or subclass thereof).
    ...
    /// with the given \c SoPointLight node.
    /// If the path has a nodekit, this will try to use
