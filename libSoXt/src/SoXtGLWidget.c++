...
SoContextListKeeper::find(Display *d, int screen) {
    ...
    for (int i = 0; i < getLength(); i++) {
        SoContextListDisplayPair *p = (SoContextListDisplayPair *)this->get(i);
...
#ifdef DEBUG
    SoDebugError::post("SoContextListKeeper::whichList", "Context not found!");
#endif
    return -1;
}

// When you need to find a context list, do this:
//    SbPList *contextList = contextListKeeper.find(XtDisplay(myWidget),
//    SCREEN(myWidget));
...
// Constructor
//
SoXtGLWidget::SoXtGLWidget(Widget parent, const char *name,
                           SbBool buildInsideParent, int modes, SbBool buildNow)
...
////////////////////////////////////////////////////////////////////////
{
    ...
    buildNormalGLXWidget(
        vis); // this MUST be called after we destroy the old contexes
}
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (flag) {
        vis = glXChooseVisual(XtDisplay(mgrWidget),
                              XScreenNumberOfScreen(XtScreen(mgrWidget)),
                              attribList);
        if (!vis) {
#ifdef DEBUG
...
////////////////////////////////////////////////////////////////////////
{
    if (ctx) {
        ...
        XtAddCallback(w, XmNdestroyCallback, glxWidgetDestroyCB,
                      (XtPointer)NULL);
    ...
    if (normalWindow) {
        XtRemoveCallback(w, SoGLwNginitCallback,
                         (XtCallbackProc)SoXtGLWidget::ginitCB,
                         (XtPointer)this);
        XtRemoveCallback(w, SoGLwNresizeCallback,
                         (XtCallbackProc)SoXtGLWidget::resizeCB,
                         (XtPointer)this);
        XtRemoveCallback(w, SoGLwNexposeCallback,
                         (XtCallbackProc)SoXtGLWidget::exposeCB,
                         (XtPointer)this);
    } else {
        XtRemoveCallback(w, SoGLwNginitCallback,
                         (XtCallbackProc)SoXtGLWidget::overlayGinitCB,
                         (XtPointer)this);
        XtRemoveCallback(w, SoGLwNexposeCallback,
                         (XtCallbackProc)SoXtGLWidget::overlayExposeCB,
                         (XtPointer)this);
    }
...
////////////////////////////////////////////////////////////////////////
{
    ...
    n++;
    XtSetArg(args[n], XmNshadowThickness, borderSize);
    n++;

    // pick an initial size if set
    SbVec2s size = (glxSize[0] && glxSize[1]) ? glxSize : getSize();
    if (size[0] && size[1]) {
        XtSetArg(args[n], XtNwidth, size[0] + 2 * borderSize);
        n++;
        XtSetArg(args[n], XtNheight, size[1] + 2 * borderSize);
        n++;
    }
...
////////////////////////////////////////////////////////////////////////
{
    ...
    n++;
    XtSetArg(args[n], XmNleftOffset, borderSize);
    n++;
    XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
    n++;
    XtSetArg(args[n], XmNrightOffset, borderSize);
    n++;
    XtSetArg(args[n], XmNtopAttachment, XmATTACH_FORM);
    n++;
    XtSetArg(args[n], XmNtopOffset, borderSize);
    n++;
    XtSetArg(args[n], XmNbottomAttachment, XmATTACH_FORM);
    n++;
    XtSetArg(args[n], XmNbottomOffset, borderSize);
    n++;
...
void
SoXtGLWidget::eventHandler(Widget w, SoXtGLWidget *p, XAnyEvent *xe,
                           Boolean *) {
...
SoXtGLWidget::ginitCB(Widget glx, SoXtGLWidget *p, XtPointer pt) {
    ...
    if (ctx == 0) {
        SbPList *contextList =
            contextListKeeper.find(XtDisplay(glx), SCREEN(glx));
...
void
SoXtGLWidget::redrawOverlay() {}
void
SoXtGLWidget::processEvent(XAnyEvent *) {}
void
SoXtGLWidget::initGraphic() {}
void
SoXtGLWidget::initOverlayGraphic() {}
void
SoXtGLWidget::sizeChanged(const SbVec2s &) {}
void
SoXtGLWidget::widgetChanged(Widget) {}
