...
////////////////////////////////////////////////////////////////////////
{
    ...
    pasteInterest->append(_ATOM_(widget, "INVENTOR_2_1_FILE"), pasteImportCB,
                          this);
    ...
    pasteInterest->append(_ATOM_(widget, "INVENTOR_2_0_FILE"), pasteImportCB,
                          this);
...
// that we now own the X selection identified by clipboardAtom (clipboardAtom
...
////////////////////////////////////////////////////////////////////////
{
    ...
    // SoWWWInline won't write it's data out unless the alternateRep field is
    // set. Run through each node in each path, and do this so that the
    // resulting file has all the data it can have.
    //???
...
// that we now own the X selection identified by clipboardAtom (clipboardAtom
// was passed during construction). The data is not necessarily Inventor.
//
// Use: protected
//
void
...
//
void
SoXtClipboard::paste(Time t, SoXtClipboardPasteCB *pasteDoneFunc,
                     void *pasteUserData)
//
////////////////////////////////////////////////////////////////////////
{
    ...
    if (clipboard == this) {
        ...
        XtGetSelectionValue(
            widget, clipboardAtom, _ATOM_(widget, "TARGETS"),
...
//
void
SoXtClipboard::getExportTargets(char **value, uint32_t *length)
//
////////////////////////////////////////////////////////////////////////
{
...
//
Boolean
SoXtClipboard::exportSelection(Widget widget, Atom *selectionAtom, Atom *target,
                               Atom *returnTarget, char **value,
                               uint32_t *length, int *format)
//
////////////////////////////////////////////////////////////////////////
{
    Boolean ok = False;
    ...
    if (value == NULL) {
        SoDebugError::post("SoXtClipboard::exportSelection", "- value is NULL");
        return FALSE;
    }
    if (length == NULL) {
        SoDebugError::post("SoXtClipboard::exportSelection",
    ...
    // The most likely case - the pasting app wants to know what data types we
    // have.
    ...
    // Just serve it up.
        ...
        if (clipboard->copyDataType == *target) {
            // Copy the data buffer since the requestor will own the data
            char *exportData =
                (char *)malloc((size_t)clipboard->copyBuffer->getNumBytes());
            if (exportData != NULL) {
        ...
        if (convertData(widget, clipboard->copyBuffer->getData(),
                        clipboard->copyBuffer->getNumBytes(), *target, value,
                        length)) {
            *format = 8;
            *returnTarget = *target;
            ok = True;
...
//
SbBool
SoXtClipboard::convertData(Widget widget, void *srcData, uint32_t srcNumBytes,
                           Atom desiredType, char **returnData,
...
////////////////////////////////////////////////////////////////////////
{
    ...
    //??? It would be nice if we put conversion callbacks in the copyInterest
    //list,
...
#ifdef DEBUG
            SoDebugError::post("SoXtClipboard::convertData",
                               "could not allocate %d bytes for return buffer",
                               srcNumBytes);
#endif
            return False;
        }
...
#define BUFSIZE 512
    ...
    sprintf(conversionCmd, IVPREFIX "/bin/ivdowngrade -v 2.0 %s %s", infile,
            outfile);

    if (system(conversionCmd) == -1) {
#ifdef DEBUG
        SoDebugError::post("SoXtClipboard::convertData", "command <%s> failed",
                           conversionCmd);
#endif
...
#ifdef DEBUG
        SoDebugError::post("SoXtClipboard::convertData",
                           "could not open output file %s for reading",
                           outfile);
#endif
...
#ifdef DEBUG
        SoDebugError::post("SoXtClipboard::convertData",
                           "could not read %d bytes from output file %s", len,
                           outfile);
#endif
...
//
SbBool
SoXtClipboard::writeToFile(SbString &tmpfile, void *srcData,
...
//
void
SoXtClipboard::importSelectionTargets(Widget widget, SoXtClipboard *clipboard,
                                      Atom *selectionAtom, Atom *target,
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (supportedAtom != 0) {
        XtGetSelectionValue(
            clipboard->widget, *selectionAtom, supportedAtom,
...
//
Atom
SoXtClipboard::chooseFromImportTargets(Atom *supportedTargets, int length)
//
////////////////////////////////////////////////////////////////////////
{
    Atom chosenType = 0;
...
// into the scene graph. This callback is registered by
// importSelectionTargets().
//
// Use: static, private
//
void
SoXtClipboard::importSelection(Widget /*w*/, SoXtClipboard *   clipboard,
                               Atom * /*selectionAtom*/, Atom *target,
...
//
void
SoXtClipboard::addPasteInterest(Atom                   dataType,
...
#ifdef DEBUG
        SoDebugError::post("SoXtClipboard::paste", "could not open file %s",
                           filename);
#endif
...
#ifdef DEBUG
        SoDebugError::post("SoXtClipboard::paste",
                           "could not read Inventor data from file %s",
                           filename);
#endif
    }
}
...
//
void
SoXtClipboard::loseSelection(Widget /*w*/, Atom *selectionAtom)
//
////////////////////////////////////////////////////////////////////////
{
    // get the clipboard pointer for this object
