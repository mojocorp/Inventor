...
#define TOGGLE_ON(BUTTON) XmToggleButtonSetState((Widget)BUTTON, True, False)
#define TOGGLE_OFF(BUTTON) XmToggleButtonSetState((Widget)BUTTON, False, False)
...
static char *     defaults[] = {"Seek animation time:",
                           "Seek to:",
                           "Seek distance:",
                           "Camera Zoom:",
...
SoXtFullViewer::SoXtFullViewer(Widget parent, const char *name,
                               SbBool                    buildInsideParent,
                               SoXtFullViewer::BuildFlag buildFlag,
                               SoXtViewer::Type t, SbBool buildNow)
    : SoXtViewer(parent, name, buildInsideParent, t,
                 FALSE) // buildNow
//
////////////////////////////////////////////////////////////////////////
{
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (popupToggleWidgets[VIEWING_WIDGET])
        XmToggleButtonSetState(popupToggleWidgets[VIEWING_WIDGET], viewingFlag,
                               False);
}
...
////////////////////////////////////////////////////////////////////////
{
    ...
    XtAddEventHandler(mgrWidget, ButtonPressMask, FALSE,
                      (XtEventHandler)&SoXtFullViewer::popMenuCallback,
                      (XtPointer)this);
    ...
    buttons[butnum++] =
        XtCreateWidget(popupTitle, xmLabelGadgetClass, popupWidget, NULL, 0);
    buttons[butnum++] =
        XtCreateWidget("sep", xmSeparatorGadgetClass, popupWidget, NULL, 0);
...
#define ADD_TOGGLE(NAME, W, ID, STATE)                                         \
    ...
    buttons[butnum++] = popupToggleWidgets[W] = XtCreateWidget(                \
        NAME, xmToggleButtonGadgetClass, popupWidget, args, n + 1);            \
...
#define ADD_ENTRY(NAME, ID)                                                    \
    buttons[butnum] =                                                          \
        XtCreateWidget(NAME, xmPushButtonGadgetClass, popupWidget, args, n);   \
...
////////////////////////////////////////////////////////////////////////
{
    ...
    Widget cascade =
        XtCreateWidget("Functions", xmCascadeButtonGadgetClass, popup, NULL, 0);
...
#define ADD_ENTRY(NAME, ID)                                                    \
    buttons[butnum] =                                                          \
        XtCreateWidget(NAME, xmPushButtonGadgetClass, submenu, args, n);       \
    XtAddCallback(buttons[butnum], XmNactivateCallback,                        \
                  (XtCallbackProc)SoXtFullViewer::menuPick, (XtPointer)ID);    \
    butnum++;
    ...
    buttons[butnum++] =
        XtCreateWidget("sep", xmSeparatorGadgetClass, submenu, NULL, 0);
...
#define ADD_ENTRY(NAME, ID, STATE)                                             \
    XtSetArg(args[n], XmNset, STATE);                                          \
    buttons[butnum++] = drawStyleWidgets[ID] =                                 \
        XtCreateWidget(NAME, xmToggleButtonGadgetClass, submenu, args, n + 1); \
    XtAddCallback(drawStyleWidgets[ID], XmNvalueChangedCallback,               \
                  (XtCallbackProc)SoXtFullViewer::drawStyleMenuPick,           \
                  (XtPointer)ID);
    ...
    ADD_ENTRY("hidden line", HIDDEN_LINE,
              drawType == SoXtViewer::VIEW_HIDDEN_LINE)
    ADD_ENTRY("no texture", NO_TXT, drawType == SoXtViewer::VIEW_NO_TEXTURE)
    ADD_ENTRY("low resolution", LOW_RES,
              drawType == SoXtViewer::VIEW_LOW_COMPLEXITY)
    ADD_ENTRY("wireframe", LINE, drawType == SoXtViewer::VIEW_LINE)
    ADD_ENTRY("points", POINT, drawType == SoXtViewer::VIEW_POINT)
    ADD_ENTRY("bounding box (no depth)", BBOX,
              drawType == SoXtViewer::VIEW_BBOX)
    ...
    ADD_ENTRY("move same as still", MOVE_SAME_AS,
              drawType == SoXtViewer::VIEW_SAME_AS_STILL)
    ADD_ENTRY("move no texture", MOVE_NO_TXT,
              drawType == SoXtViewer::VIEW_NO_TEXTURE)
    ADD_ENTRY("move low res", MOVE_LOW_RES,
              drawType == SoXtViewer::VIEW_LOW_COMPLEXITY)
    ADD_ENTRY("move wireframe", MOVE_LINE, drawType == SoXtViewer::VIEW_LINE)
    ADD_ENTRY("move low res wireframe (no depth)", MOVE_LOW_LINE,
              drawType == SoXtViewer::VIEW_LOW_RES_LINE)
    ADD_ENTRY("move points", MOVE_POINT, drawType == SoXtViewer::VIEW_POINT)
    ADD_ENTRY("move low res points (no depth)", MOVE_LOW_POINT,
              drawType == SoXtViewer::VIEW_LOW_RES_POINT)
    ADD_ENTRY("move bounding box (no depth)", MOVE_BBOX,
              drawType == SoXtViewer::VIEW_BBOX)
#undef ADD_ENTRY
...
#define ADD_ENTRY(NAME, ID)                                                    \
    ...
    buttons[butnum++] = bufferStyleWidgets[ID] =                               \
        XtCreateWidget(NAME, xmToggleButtonGadgetClass, submenu, args, n + 1); \
    XtAddCallback(bufferStyleWidgets[ID], XmNvalueChangedCallback,             \
                  (XtCallbackProc)SoXtFullViewer::bufferStyleMenuPick,         \
                  (XtPointer)ID);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    //
    if (!xr->getResource("stereoErrorTitle", "StereoErrorTitle",
                         rl.stereoErrorTitle))
...
////////////////////////////////////////////////////////////////////////
{
    ...
    mgrWidget =
        XtCreateWidget(getWidgetName(), xmFormWidgetClass, parent, args, n);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    Widget form =
        XtCreateWidget("LeftTrimForm", xmFormWidgetClass, parent, NULL, 0);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    // create a form to hold all the parts
    Widget form =
        XtCreateWidget("BottomTrimForm", xmFormWidgetClass, parent, NULL, 0);

    // create all the parts
    if (rightWheelStr == NULL)
        rightWheelStr = strdup("Motion Z");
    rightWheelLabel =
        XtCreateWidget(rightWheelStr, xmLabelGadgetClass, form, NULL, 0);
    if (bottomWheelStr == NULL)
        bottomWheelStr = strdup("Motion X");
    bottomWheelLabel =
        XtCreateWidget(bottomWheelStr, xmLabelGadgetClass, form, NULL, 0);
    if (leftWheelStr == NULL)
        leftWheelStr = strdup("Motion Y");
    leftWheelLabel =
        XtCreateWidget(leftWheelStr, xmLabelGadgetClass, form, NULL, 0);
    ...
    XtAddCallback(bottomWheel, XmNvalueChangedCallback,
                  (XtCallbackProc)SoXtFullViewer::bottomWheelCB,
                  (XtPointer)this);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    // create a form to hold all the parts
    Widget form =
        XtCreateWidget("RightTrimForm", xmFormWidgetClass, parent, NULL, 0);
    ...
    XtAddCallback(rightWheel, XmNvalueChangedCallback,
                  (XtCallbackProc)SoXtFullViewer::rightWheelCB,
                  (XtPointer)this);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    for (int i = 0; i < PUSH_NUM; i++) {
        ...
        XtAddCallback(w, XmNactivateCallback,
                      (XtCallbackProc)SoXtFullViewer::pushButtonCB,
                      (XtPointer)(unsigned long)i);
    ...
    buttonList[PICK_PUSH]->setIcon(so_xt_pick_bits, so_xt_icon_width,
                                   so_xt_icon_height);
    buttonList[VIEW_PUSH]->setIcon(so_xt_view_bits, so_xt_icon_width,
                                   so_xt_icon_height);
    buttonList[HELP_PUSH]->setIcon(so_xt_help_bits, so_xt_icon_width,
                                   so_xt_icon_height);
    buttonList[HOME_PUSH]->setIcon(so_xt_home_bits, so_xt_icon_width,
                                   so_xt_icon_height);
    buttonList[SET_HOME_PUSH]->setIcon(so_xt_set_home_bits, so_xt_icon_width,
                                       so_xt_icon_height);
    buttonList[VIEW_ALL_PUSH]->setIcon(so_xt_see_all_bits, so_xt_icon_width,
                                       so_xt_icon_height);
    buttonList[SEEK_PUSH]->setIcon(so_xt_seek_bits, so_xt_icon_width,
                                   so_xt_icon_height);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    appButtonForm =
        XtCreateWidget("AppButtForm", xmFormWidgetClass, parent, NULL, 0);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    n++;
    prefSheetShellWidget = shell =
        XtCreatePopupShell("preferenceSheet", topLevelShellWidgetClass,
                           SoXt::getShellWidget(mgrWidget), args, n);
...
// Use: protected
void
SoXtFullViewer::createDefaultPrefSheetParts(Widget widgetList[], int &num,
                                            Widget form)
//
////////////////////////////////////////////////////////////////////////
{
...
// Use: protected
void
SoXtFullViewer::layoutPartsAndMapPrefSheet(Widget widgetList[], int num,
...
////////////////////////////////////////////////////////////////////////
{
    ...
    widgetList[0] =
        XtCreateWidget(rl.seekAniTime, xmLabelGadgetClass, form, NULL, 0);
    ...
    XtAddCallback(widgetList[1], XmNactivateCallback,
                  (XtCallbackProc)SoXtFullViewer::seekPrefSheetFieldCB,
                  (XtPointer)this);

    widgetList[2] =
        XtCreateWidget("seconds", xmLabelGadgetClass, form, NULL, 0);

    // create the second line
    widgetList[3] =
        XtCreateWidget(rl.seekToLabel, xmLabelGadgetClass, form, NULL, 0);
    ...
    n++;
    widgetList[4] =
        XtCreateWidget("point", xmToggleButtonGadgetClass, form, args, n);
    widgetList[5] =
        XtCreateWidget("object", xmToggleButtonGadgetClass, form, args, n);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    // create the first line
    label = XtCreateWidget(rl.seekDistance, xmLabelGadgetClass, form, NULL, 0);
    ...
    XtAddCallback(thumb, XmNdragCallback,
                  (XtCallbackProc)SoXtFullViewer::seekDistWheelCB,
                  (XtPointer)this);
    seekDistWheelVal = 0;
    ...
    n++;
    seekDistField = text = XtCreateWidget("", xmTextWidgetClass, form, args, n);
    ...
    n++;
    toggles[0] =
        XtCreateWidget("percentage", xmToggleButtonGadgetClass, form, args, n);
    toggles[1] =
        XtCreateWidget("absolute", xmToggleButtonGadgetClass, form, args, n);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    // create a form to hold verything together
    Widget form =
        XtCreateWidget("ZoomForm", xmFormWidgetClass, parent, NULL, 0);

    // create all the parts
    zoomWidgets[ZOOM_LABEL] =
        XtCreateWidget(rl.cameraZoom, xmLabelGadgetClass, form, NULL, 0);
    zoomWidgets[ZOOM_RANGE_LAB1] =
        XtCreateWidget(rl.zoomSlider, xmLabelGadgetClass, form, NULL, 0);
    zoomWidgets[ZOOM_RANGE_LAB2] =
        XtCreateWidget(rl.to, xmLabelGadgetClass, form, NULL, 0);
    ...
    XtSetArg(args[n], XmNorientation, XmHORIZONTAL);
    n++;
    zoomWidgets[ZOOM_SLIDER] =
        XtCreateWidget("ZoomSlider", xmScaleWidgetClass, form, args, n);

    XtAddCallback(zoomWidgets[ZOOM_SLIDER], XmNvalueChangedCallback,
                  (XtCallbackProc)&SoXtFullViewer::zoomSliderCB,
                  (XtPointer)this);
    ...
    zoomWidgets[ZOOM_FIELD] =
        XtCreateWidget("ZoomField", xmTextWidgetClass, form, args, n);

    sprintf(str, "%.1f", zoomSldRange[0]);
    XtSetArg(args[n], XmNvalue, str);
    zoomWidgets[ZOOM_RANGE_FIELD1] =
        XtCreateWidget("zoomFrom", xmTextWidgetClass, form, args, n + 1);

    sprintf(str, "%.1f", zoomSldRange[1]);
    XtSetArg(args[n], XmNvalue, str);
    zoomWidgets[ZOOM_RANGE_FIELD2] =
        XtCreateWidget("zoomTo", xmTextWidgetClass, form, args, n + 1);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    n++;
    Widget toggle =
        XtCreateWidget("", xmToggleButtonGadgetClass, form, args, n);
    n = 0;
    XtSetArg(args[n], XmNsensitive, camera != NULL);
    n++;
    Widget label = XtCreateWidget("Auto clipping planes", xmLabelGadgetClass,
                                  form, args, n);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    // toggle text
    stereoLabel =
        XtCreateWidget("Stereo Viewing", xmLabelGadgetClass, form, NULL, 0);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    // create all the parts
    widgetList[0] =
        XtCreateWidget(rl.viewerSpeed, xmLabelGadgetClass, form, NULL, 0);

    n = 0;
    XtSetArg(args[n], XmNhighlightThickness, 0);
    n++;
    widgetList[1] =
        XtCreateWidget(rl.increase, xmPushButtonGadgetClass, form, args, n);
    widgetList[2] =
        XtCreateWidget(rl.decrease, xmPushButtonGadgetClass, form, args, n);
    XtAddCallback(widgetList[1], XmNactivateCallback,
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (camera->isOfType(SoPerspectiveCamera::getClassTypeId()))
        return ((SoPerspectiveCamera *)camera)->heightAngle.getValue() * 180.0 /
               M_PI;
    else if (camera->isOfType(SoOrthographicCamera::getClassTypeId()))
...
//
void
SoXtFullViewer::rightWheelStart() {
    interactiveCountInc();
}
void
...
void
SoXtFullViewer::rightWheelFinish() {
    interactiveCountDec();
}
void
SoXtFullViewer::bottomWheelFinish() {
    interactiveCountDec();
}
void
SoXtFullViewer::leftWheelFinish() {
    interactiveCountDec();
}
...
void
SoXtFullViewer::leftWheelMotion(float) {}
void
SoXtFullViewer::openViewerHelpCard() {}
...
void
SoXtFullViewer::rightWheelCB(Widget, SoXtFullViewer *v, XtPointer *d) {
...
void
SoXtFullViewer::bottomWheelCB(Widget, SoXtFullViewer *v, XtPointer *d) {
    SgThumbWheelCallbackStruct *data = (SgThumbWheelCallbackStruct *)d;
...
void
SoXtFullViewer::leftWheelCB(Widget, SoXtFullViewer *v, XtPointer *d) {
    SgThumbWheelCallbackStruct *data = (SgThumbWheelCallbackStruct *)d;
...
void
SoXtFullViewer::seekDistPercPrefSheetToggleCB(Widget tog1, Widget tog2,
                                              void *) {
    XmToggleButtonSetState(tog2, !XmToggleButtonGetState(tog1), FALSE);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (data->reason == XmCR_DRAG ||
        ...
        f *= f;
        float zoom =
            v->zoomSldRange[0] + f * (v->zoomSldRange[1] - v->zoomSldRange[0]);
...
//
void
SoXtFullViewer::popMenuCallback(Widget, SoXtFullViewer *v, XEvent *event,
                                Boolean *)
//
////////////////////////////////////////////////////////////////////////
{
...
////////////////////////////////////////////////////////////////////////
{
    ...
    switch (id) {
    case AS_IS:
        v->setDrawStyle(SoXtViewer::STILL, SoXtViewer::VIEW_AS_IS);
        break;
    case HIDDEN_LINE:
        v->setDrawStyle(SoXtViewer::STILL, SoXtViewer::VIEW_HIDDEN_LINE);
        break;
    case NO_TXT:
        v->setDrawStyle(SoXtViewer::STILL, SoXtViewer::VIEW_NO_TEXTURE);
        break;
    case LOW_RES:
        v->setDrawStyle(SoXtViewer::STILL, SoXtViewer::VIEW_LOW_COMPLEXITY);
        break;
    case LINE:
        v->setDrawStyle(SoXtViewer::STILL, SoXtViewer::VIEW_LINE);
        break;
    case POINT:
        v->setDrawStyle(SoXtViewer::STILL, SoXtViewer::VIEW_POINT);
        break;
    case BBOX:
        v->setDrawStyle(SoXtViewer::STILL, SoXtViewer::VIEW_BBOX);
        break;

    case MOVE_SAME_AS:
        v->setDrawStyle(SoXtViewer::INTERACTIVE,
                        SoXtViewer::VIEW_SAME_AS_STILL);
        break;
    case MOVE_NO_TXT:
        v->setDrawStyle(SoXtViewer::INTERACTIVE, SoXtViewer::VIEW_NO_TEXTURE);
        break;
    case MOVE_LOW_RES:
        v->setDrawStyle(SoXtViewer::INTERACTIVE,
        ...
        break;
    case MOVE_LINE:
        v->setDrawStyle(SoXtViewer::INTERACTIVE, SoXtViewer::VIEW_LINE);
        break;
    case MOVE_LOW_LINE:
        v->setDrawStyle(SoXtViewer::INTERACTIVE, SoXtViewer::VIEW_LOW_RES_LINE);
        break;
    case MOVE_POINT:
        v->setDrawStyle(SoXtViewer::INTERACTIVE, SoXtViewer::VIEW_POINT);
        break;
    case MOVE_LOW_POINT:
        v->setDrawStyle(SoXtViewer::INTERACTIVE,
        ...
        break;
    case MOVE_BBOX:
        v->setDrawStyle(SoXtViewer::INTERACTIVE, SoXtViewer::VIEW_BBOX);
        break;
    }
}
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (v->isAutoClipping()) {
        ...
        // create the labels
        label[0] =
            XtCreateWidget(rl.nearPlane, xmLabelGadgetClass, form, NULL, 0);
        label[1] =
            XtCreateWidget(rl.farPlane, xmLabelGadgetClass, form, NULL, 0);
        ...
        XtAddCallback(thumb[0], XmNvalueChangedCallback,
                      (XtCallbackProc)SoXtFullViewer::clipNearWheelCB,
                      (XtPointer)v);
        XtAddCallback(thumb[0], XmNdragCallback,
                      (XtCallbackProc)SoXtFullViewer::clipNearWheelCB,
                      (XtPointer)v);
        XtAddCallback(thumb[1], XmNvalueChangedCallback,
                      (XtCallbackProc)SoXtFullViewer::clipFarWheelCB,
                      (XtPointer)v);
        ...
        char  str[15];
        float val =
            (v->camera != NULL) ? v->camera->nearDistance.getValue() : 0;
        sprintf(str, "%g", val);
        XtSetArg(args[0], XmNvalue, str);
        n++;
        XtSetArg(args[n], XmNhighlightThickness, 1);
        ...
        n++;
        v->clipNearField = text[0] =
            XtCreateWidget("", xmTextWidgetClass, form, args, n);
        val = (v->camera != NULL) ? v->camera->farDistance.getValue() : 0;
        sprintf(str, "%g", val);
        XtSetArg(args[0], XmNvalue, str);
        v->clipFarField = text[1] =
            XtCreateWidget("", xmTextWidgetClass, form, args, n);
...
SoXtFullViewer::clipFieldCB(Widget field, SoXtFullViewer *v, void *) {
    ...
    if (sscanf(str, "%f", &val) &&
        (val > 0 ||
         v->camera->isOfType(SoOrthographicCamera::getClassTypeId()))) {
...
#ifdef SB_OS_IRIX
static void
destroyStereoInfoDialogCB(Widget dialog, void *, void *) {
...
static void
createStereoInfoDialog(Widget shell) {
    ...
    Arg      args[5];
    XmString xmstr = XmStringCreateLocalized(rm.str1);
    ...
    n++;
    XtSetArg(args[n], XtNtitle, rm.title);
    n++;
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (!toggleState) {
        ...
        // create the combobox
        char *stype[] = {"Raw Stereo (quad buffer)", "Anaglyph Red/Cyan",
                         "Anaglyph Blue/Yellow", "Anaglyph Green/Magenta"};
        ...
        XtAddCallback(combo, XmNselectionCallback,
                      (XtCallbackProc)SoXtFullViewer::stereoTypeComboCB,
                      (XtPointer)v);

        // create the labels
        Widget typeLabel =
            XtCreateWidget("type:", xmLabelGadgetClass, form, NULL, 0);
        Widget offsetLabel =
            XtCreateWidget(rl.stereoOffset, xmLabelGadgetClass, form, NULL, 0);
        Widget balanceLabel =
            XtCreateWidget(rl.stereoBalance, xmLabelGadgetClass, form, NULL, 0);
        ...
        XtAddCallback(offsetThumb, XmNvalueChangedCallback,
                      (XtCallbackProc)SoXtFullViewer::stereoOffsetWheelCB,
                      (XtPointer)v);
        ...
        XtAddCallback(balanceThumb, XmNvalueChangedCallback,
                      (XtCallbackProc)SoXtFullViewer::stereoBalanceWheelCB,
                      (XtPointer)v);
        ...
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET);
        n++;
        XtSetArg(args[n], XmNbottomWidget, combo);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNrightWidget, combo);
        n++;
        XtSetArg(args[n], XmNrightOffset, 5);
        n++;
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, combo);
        n++;
        XtSetArg(args[n], XmNtopOffset, 2);
        n++;
        ...
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET);
        n++;
        XtSetArg(args[n], XmNbottomWidget, v->stereoOffsetField);
        n++;
        XtSetArg(args[n], XmNbottomOffset, 3);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNrightWidget, v->stereoOffsetField);
        n++;
        XtSetArg(args[n], XmNrightOffset, 3);
        n++;
        ...
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET);
        n++;
        XtSetArg(args[n], XmNbottomWidget, offsetThumb);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNrightWidget, offsetThumb);
        n++;
        XtSetArg(args[n], XmNrightOffset, 5);
        n++;
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, v->stereoOffsetField);
        n++;
        XtSetArg(args[n], XmNtopOffset, 2);
        n++;
        ...
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET);
        n++;
        XtSetArg(args[n], XmNbottomWidget, v->stereoBalanceField);
        n++;
        XtSetArg(args[n], XmNbottomOffset, 3);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNrightWidget, v->stereoBalanceField);
        n++;
        XtSetArg(args[n], XmNrightOffset, 3);
        n++;
        ...
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET);
        n++;
        XtSetArg(args[n], XmNbottomWidget, balanceThumb);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNrightWidget, balanceThumb);
        n++;
        XtSetArg(args[n], XmNrightOffset, 5);
        n++;
...
void
SoXtFullViewer::zoomPrefSheetMinFieldCB(Widget field, SoXtFullViewer *v,
                                        void *) {
    // get text value from the label
    char *str = XmTextGetString(field);
    float val;
...
void
SoXtFullViewer::zoomPrefSheetMaxFieldCB(Widget field, SoXtFullViewer *v,
                                        void *) {
