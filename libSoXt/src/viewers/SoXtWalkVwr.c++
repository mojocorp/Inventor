...
//
SoXtWalkViewer::SoXtWalkViewer(Widget parent, const char *name,
                               SbBool                    buildInsideParent,
                               SoXtFullViewer::BuildFlag b, SoXtViewer::Type t)
    : SoXtConstrainedViewer(parent, name, buildInsideParent, b, t,
...
////////////////////////////////////////////////////////////////////////
{
    ...
    animatingFlag = FALSE;
    animationSensor =
        new SoFieldSensor(SoXtWalkViewer::animationSensorCB, this);
    ...
    if (buildNow) {
        ...
        if (!xr.getResource("walkViewerPreferenceSheet",
                            "WalkViewerPreferenceSheet", rl.wvPrefSheet))
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (getDisplay()) {
        ...
        if (walkCursor)
            XFreeCursor(display, walkCursor);
        if (panCursor)
            XFreeCursor(display, panCursor);
        if (tiltCursor)
            XFreeCursor(display, tiltCursor);
        if (seekCursor)
            XFreeCursor(display, seekCursor);
        if (upCursor)
            XFreeCursor(display, upCursor);
    }
}
...
////////////////////////////////////////////////////////////////////////
{
    ...
    XtAddCallback(wheel, XmNvalueChangedCallback,
                  (XtCallbackProc)SoXtWalkViewer::heightWheelCB,
                  (XtPointer)this);
    ...
    XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
    n++;
    XtSetArg(args[n], XmNleftOffset, 7);
    n++;
    ...
    XtSetArg(args[n], XmNbottomAttachment, XmATTACH_WIDGET);
    n++;
    XtSetArg(args[n], XmNbottomWidget, wheel);
    n++;
...
////////////////////////////////////////////////////////////////////////
{
    ...
    // walk cursor
    source = XCreateBitmapFromData(display, d, so_xt_walk_bits,
                                   so_xt_walk_width, so_xt_walk_height);
    mask = XCreateBitmapFromData(display, d, so_xt_walk_mask_bits,
                                 so_xt_walk_width, so_xt_walk_height);
    walkCursor =
        XCreatePixmapCursor(display, source, mask, &foreground, &background,
                            so_xt_walk_x_hot, so_xt_walk_y_hot);
    XFreePixmap(display, source);
    XFreePixmap(display, mask);

    // panning cursor
    source = XCreateBitmapFromData(display, d, so_xt_pan_bits, so_xt_pan_width,
                                   so_xt_pan_height);
    panCursor =
        XCreatePixmapCursor(display, source, source, &foreground, &foreground,
                            so_xt_pan_x_hot, so_xt_pan_y_hot);
    XFreePixmap(display, source);

    // tilt cursor
    source = XCreateBitmapFromData(display, d, so_xt_tilt_bits,
                                   so_xt_tilt_width, so_xt_tilt_height);
    mask = XCreateBitmapFromData(display, d, so_xt_tilt_mask_bits,
                                 so_xt_tilt_width, so_xt_tilt_height);
    tiltCursor =
        XCreatePixmapCursor(display, source, mask, &foreground, &background,
                            so_xt_tilt_x_hot, so_xt_tilt_y_hot);
    XFreePixmap(display, source);
    XFreePixmap(display, mask);
    ...
    // up direction cursor
    source =
        XCreateBitmapFromData(display, d, so_xt_normal_vec_bits,
                              so_xt_normal_vec_width, so_xt_normal_vec_height);
    upCursor =
        XCreatePixmapCursor(display, source, source, &foreground, &foreground,
                            so_xt_normal_vec_x_hot, so_xt_normal_vec_y_hot);
    XFreePixmap(display, source);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    //
        ...
        if (distX)
            camera->position =
                camera->position.getValue() + distX * rightVector;
        if (distY)
...
{
    float dist =
        (newVal - rightWheelVal) * sceneSize * viewerSpeed * WHEEL_DOLLY_FACTOR;
    dollyCamera(dist);
...
SoXtWalkViewer::heightWheelCB(Widget, SoXtWalkViewer *p, XtPointer *d) {
    ...
    if (data->reason == XmCR_DRAG) {
        ...
        p->camera->position =
            p->camera->position.getValue() + d * p->upDirection;
