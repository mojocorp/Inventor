...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (!ctlDown) {
        ...
        newHitPt = motionLineProj->project(getNormalizedLocaterPosition());
    }
    ...
    if (!shftDown || ctlDown)
        ...
        if (translateDir == -1) {
            ...
            if (isAdequateConstraintMotion()) {
                if (std::abs(workSpaceMotion[0]) >
                        std::abs(workSpaceMotion[1]) &&
                    std::abs(workSpaceMotion[0]) > std::abs(workSpaceMotion[2]))
                    translateDir = 0;
                else if (std::abs(workSpaceMotion[1]) >
                         std::abs(workSpaceMotion[2]))
                    translateDir = 1;
                else
                    translateDir = 2;
...
#undef TINY
    ...
    // of the drag, to find the current motion matrix.
    // We need to send our matrix that converts from workSpace to localSpace.
    setMotionMatrix(appendScale(getStartMotionMatrix(), delta, scaleCenter,
                                &workSpaceToLocal));
    return TRUE;
}
