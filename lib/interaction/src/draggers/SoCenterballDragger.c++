...
////////////////////////////////////////////////////////////////////////
{
    ...
    SO_KIT_ADD_CATALOG_ENTRY(antiSquish, SoAntiSquish, FALSE, topSeparator,
                             geomSeparator, TRUE);

    // This whole manip gets modelled in BASE_COLOR
    SO_KIT_ADD_CATALOG_ENTRY(lightModel, SoLightModel, TRUE, topSeparator,
                             geomSeparator, TRUE);

    // This little grouping goes under the geomSeparator, since it's
    // geometry only, not draggers.
    SO_KIT_ADD_CATALOG_ENTRY(XAxisSwitch, SoSwitch, TRUE, geomSeparator, \0,
                             FALSE);
    SO_KIT_ADD_CATALOG_ENTRY(XAxis, SoSeparator, TRUE, XAxisSwitch, \0, TRUE);
    SO_KIT_ADD_CATALOG_ENTRY(YAxisSwitch, SoSwitch, TRUE, geomSeparator, \0,
                             FALSE);
    SO_KIT_ADD_CATALOG_ENTRY(YAxis, SoSeparator, TRUE, YAxisSwitch, \0, TRUE);
    SO_KIT_ADD_CATALOG_ENTRY(ZAxisSwitch, SoSwitch, TRUE, geomSeparator, \0,
                             FALSE);
    SO_KIT_ADD_CATALOG_ENTRY(ZAxis, SoSeparator, TRUE, ZAxisSwitch, \0, TRUE);
    ...
    SO_KIT_ADD_CATALOG_ENTRY(rotX90, SoRotation, TRUE, topSeparator, \0, FALSE);
    ...
    SO_KIT_ADD_CATALOG_ENTRY(rotY90, SoRotation, TRUE, topSeparator, \0, FALSE);
    SO_KIT_ADD_CATALOG_ENTRY(XCenterChanger, SoTranslate2Dragger, TRUE,
                             topSeparator, \0, TRUE);

    SO_KIT_ADD_CATALOG_ENTRY(rot2X90, SoRotation, TRUE, topSeparator, \0,
                             FALSE);
    SO_KIT_ADD_CATALOG_ENTRY(XRotator, SoRotateCylindricalDragger, TRUE,
                             topSeparator, \0, TRUE);

    if (SO_KIT_IS_FIRST_INSTANCE())
        readDefaultParts("centerballDragger.iv", geomBuffer,
                         sizeof(geomBuffer));
    ...
    rotFieldSensor =
        new SoFieldSensor(&SoCenterballDragger::fieldSensorCB, this);
...
SoCenterballDragger::setUpConnections(SbBool onOff, SbBool doItAlways) {
    ...
    if (onOff) {
        ...
        for (i = 0; i < 3; i++) {
            ...
            stps[i]->setPartAsDefault("rotatorActive",
        ...
        for (i = 0; i < 3; i++) {
            if (ccs[i] == NULL)
                continue;
            ccs[i]->setPartAsDefault("translator", "centerballCenterChanger");
        ...
        for (i = 0; i < 3; i++) {
            ...
            stps[i]->removeStartCallback(&SoCenterballDragger::kidStartCB,
                                         this);
            stps[i]->removeFinishCallback(&SoCenterballDragger::kidFinishCB,
                                          this);
        }
        // CENTER CHANGERS
        SoDragger *ccs[3];
        ...
        for (i = 0; i < 3; i++) {
            ...
            ccs[i]->removeFinishCallback(&SoCenterballDragger::kidFinishCB,
                                         this);
        }
...
SoCenterballDragger::valueChangedCB(void *, SoDragger *inDragger) {
    ...
    if (cd != NULL && (cd == dBall->XCenterChanger.getValue() ||
...
SoCenterballDragger::fieldSensorCB(void *inDragger, SoSensor *inSensor) {
    ...
    if (doCenter) {
        ...
        SoMatrixTransform *mx =
            SO_GET_ANY_PART(dragger, "translateToCenter", SoMatrixTransform);
        mx->matrix = newMat;
...
SoCenterballDragger::setSwitches(SoDragger *activeChild) {
    ...
    SbBool changeCenter =
        (activeChild && (activeChild == XCenterChanger.getValue() ||
...
////////////////////////////////////////////////////////////////////////
{
    ...
    SbVec3f newCenter(curCenterMatrix[3][0], curCenterMatrix[3][1],
                      curCenterMatrix[3][2]);
