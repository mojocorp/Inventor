...
SoTransformBoxDragger::SoTransformBoxDragger() {
    ...
    if (SO_KIT_IS_FIRST_INSTANCE())
        readDefaultParts("transformBoxDragger.iv", geomBuffer,
                         sizeof(geomBuffer));
...
SoTransformBoxDragger::setUpConnections(SbBool onOff, SbBool doItAlways) {
    ...
    if (onOff) {
        ...
        if (sD) {
            ...
            sD->setPartAsDefault("scaler", "transformBoxScalerScaler");
            sD->setPartAsDefault("scalerActive",
                                 "transformBoxScalerScalerActive");
            sD->setPartAsDefault("feedback", "transformBoxScalerFeedback");
        ...
        for (i = 0; i < 3; i++) {
            if (rDs[i] == NULL)
                continue;
            rDs[i]->setPartAsDefault("rotator", "transformBoxRotatorRotator");
            rDs[i]->setPartAsDefault("rotatorActive",
                                     "transformBoxRotatorRotatorActive");
            rDs[i]->setPartAsDefault("feedback", "transformBoxRotatorFeedback");
...
void
SoTransformBoxDragger::invalidateSurroundScaleCB(void *parentAsVoid,
                                                 SoDragger *) {
