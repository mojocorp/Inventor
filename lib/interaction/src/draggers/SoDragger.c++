    ...
    if (tempPathToThis != NULL) {
        ...
        for (int i = 0; i < tempPathToThis->getLength(); i++) {
            ...
            if (!pathNode || !pathNode->getChildren())
                ...
                tempPathNumKidsHack.push_back(
                    pathNode->getChildren()->getLength());
        }
    }
...
SoDragger::isTempPathToThisOk() {
    ...
    for (int numFmTail = 0; numFmTail < tempPathToThis->getLength();
         numFmTail++) {
            ...
            if (aList.find(nodeInPath, SoNotRec::PARENT) == -1) {
                ...
                if ((numKidsNow != numKidsBefore) ||
                    ...
                    for (early = bestSpot, late = bestSpot;
                         early >= 0 || late < numKidsNow; early--, late++) {
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (pathToPart == NULL) {
        ...
        if (pNum != SO_CATALOG_NAME_NOT_FOUND) {
            ...
            while ((pathToPart == NULL) && (pNum != thisPnum) &&
                   (pNum != motMatPnum)) {
                ...
                if (pNum != thisPnum && pNum != motMatPnum) {
                    pathToPart = createPathToAnyPart(cat->getName(pNum), FALSE,
            ...
            pathToPart =
                createPathToAnyPart(partName, TRUE, FALSE, FALSE, pathToMe);
        }
    }
...
/////////
void
SoDragger::addStartCallback(SoDraggerCB *f, void *d) {
    startCallbacks->addCallback((SoCallbackListCB *)f, d);
}

void
SoDragger::removeStartCallback(SoDraggerCB *f, void *d) {
...
/////////
void
SoDragger::addMotionCallback(SoDraggerCB *f, void *d) {
    motionCallbacks->addCallback((SoCallbackListCB *)f, d);
}

void
SoDragger::removeMotionCallback(SoDraggerCB *f, void *d) {
...
/////////
void
SoDragger::addFinishCallback(SoDraggerCB *f, void *d) {
    finishCallbacks->addCallback((SoCallbackListCB *)f, d);
}

void
SoDragger::removeFinishCallback(SoDraggerCB *f, void *d) {
...
/////////
void
SoDragger::addValueChangedCallback(SoDraggerCB *f, void *d) {
    valueChangedCallbacks->addCallback((SoCallbackListCB *)f, d);
}

void
SoDragger::removeValueChangedCallback(SoDraggerCB *f, void *d) {
...
/////////
void
SoDragger::addOtherEventCallback(SoDraggerCB *f, void *d) {
    otherEventCallbacks->addCallback((SoCallbackListCB *)f, d);
}

void
SoDragger::removeOtherEventCallback(SoDraggerCB *f, void *d) {
...
void
SoDragger::childTransferMotionAndValueChangedCB(void *     parentAsVoid,
...
void
SoDragger::childValueChangedCB(void *parentAsVoid, SoDragger *childDragger) {
    SoDragger *parent = (SoDragger *)parentAsVoid;
...
void
SoDragger::childStartCB(void *parentAsVoid, SoDragger *childDragger) {
    SoDragger *parent = (SoDragger *)parentAsVoid;
...
void
SoDragger::childMotionCB(void *parentAsVoid, SoDragger *childDragger) {
    SoDragger *parent = (SoDragger *)parentAsVoid;
...
void
SoDragger::childFinishCB(void *parentAsVoid, SoDragger *childDragger) {
    SoDragger *parent = (SoDragger *)parentAsVoid;
...
void
SoDragger::childOtherEventCB(void *parentAsVoid, SoDragger *childDragger) {
    SoDragger *parent = (SoDragger *)parentAsVoid;
...
SoDragger::workFieldsIntoTransform(SbMatrix &mtx) {
    ...
    workValuesIntoTransform(mtx, translationPtr, rotationPtr, scaleFactorPtr,
                            scaleOrientationPtr, centerPtr);
}

void
SoDragger::workValuesIntoTransform(SbMatrix &mtx, const SbVec3f *translationPtr,
...
#undef TINY
    ...
    if (canDoFast == TRUE) {
        ...
        scaleFactor.setValue(matXVec.normalize(), matYVec.normalize(),
                             matZVec.normalize());

        rotation = SbMatrix(matXVec[0], matXVec[1], matXVec[2], 0, matYVec[0],
                            matYVec[1], matYVec[2], 0, matZVec[0], matZVec[1],
                            matZVec[2], 0, 0, 0, 0, 1);
    } else {
	...
	SoDebugError::post("SoDragger::getTransformFast", 
        ...
        mtx.factor(scaleOrientMatrix, scaleFactor, rotMatrix, translation,
                   proj);
        rotation = rotMatrix;
...
SbMatrix
SoDragger::appendTranslation(const SbMatrix &mtx, const SbVec3f &translation,
                             const SbMatrix *conversion) {
    SbBool isCnvIdent =
        (conversion == NULL || (*conversion) == SbMatrix::identity());
...
SbMatrix
SoDragger::appendScale(const SbMatrix &mtx, const SbVec3f &scale,
                       const SbVec3f &scaleCenter, const SbMatrix *conversion) {
    SbBool isCnvIdent =
        (conversion == NULL || (*conversion) == SbMatrix::identity());
    SbBool   isMtxIdent = (mtx == SbMatrix::identity());
    ...
    if (okayMrgScale == mrgScale)
        ...
        okayMtxWithScale.setTransform(mrgTrans, mrgRot, okayMrgScale,
                                      mrgScaleOrient);
...
SbMatrix
SoDragger::appendRotation(const SbMatrix &mtx, const SbRotation &rot,
...
#define TINY 0.00001f
    ...
    if ((f = getField("rotation")) != NULL) {
        if (!(f->isConnected() && f->isConnectionEnabled()) &&
            (((SoSFRotation *)f)->getValue())
                .equals(SbRotation::identity(), TINY))
            f->setDefault(TRUE);
    }
    if ((f = getField("translation")) != NULL) {
        if (!(f->isConnected() && f->isConnectionEnabled()) &&
