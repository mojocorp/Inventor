...
SbBool
SoInteractionKit::setAnyPartAsDefault(const SbName &partName, SoNode *newNode,
                                      SbBool anyPart, SbBool onlyIfAlready) {
    ...
    if (owner == NULL) {
        ...
        // newNode is NULL.
        SoNodeKitPath *nkp =
            createPathToAnyPart(partName, TRUE, !anyPart, !anyPart);
        if (nkp == NULL)
            reply = FALSE;
        else {
            nkp->ref();
...
//
SoGroup *
SoInteractionKit::readFromBuffer(const unsigned char defaultBuffer[],
                                 int                 defBufSize)
//
////////////////////////////////////////////////////////////////////////
{
...
SoInteractionKit::setAnyPartAsPath(const SbName &partName,
                                   SoPath *surrogatePath, SbBool leafCheck,
                                   SbBool publicCheck)
//
////////////////////////////////////////////////////////////////////////
{
    ...
    if (!setAnySurrogatePath(partName, surrogatePath, leafCheck, publicCheck)) {
        ...
        SoDebugError::post("SoInteractionKit::setPartAsPath",
                           "can not set surrogate path for part %s",
                           partName.getString());
#endif
        return FALSE;
    }
    ...
    SoNodeKitPath *pathToPart =
        SoBaseKit::createPathToAnyPart(partName, FALSE, leafCheck, publicCheck);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    SoNodeKitPath *answerPath = SoBaseKit::createPathToAnyPart(
        partName, makeIfNeeded, leafCheck, publicCheck, pathToExtend);
...
//
SbBool
SoInteractionKit::setAnySurrogatePath(const SbName &partName, SoPath *from,
                                      SbBool leafCheck, SbBool publicCheck)
//
////////////////////////////////////////////////////////////////////////
{
    ...
    if (from == NULL) {
        ...
        if (dotPtr != NULL || brackPtr != NULL) {
            ...
            if (firstPartNum != SO_CATALOG_NAME_NOT_FOUND) {
                ...
                firstPartNode =
                    SoBaseKit::getAnyPart(firstName, FALSE, TRUE, publicCheck);
            }
    ...
    partPath = SoBaseKit::createPathToAnyPart(partName, makeIfNeeded, leafCheck,
                                              publicCheck);
    ...
    if (partPath == NULL) {
        ...
        SoDebugError::post("SoInteractionKit::setAnySurrogatePath",
                           "can not get a part path for part %s",
                           partName.getString());
#endif
...
////////////////////////////////////////////////////////////////////////
{
    ...
    return (isPathSurrogateInMySubgraph(pathToCheck, pathToOwner, nameInOwner,
                                        pathInOwner, FALSE));
}
        ...
        if (surrogateNum != -1) {
            if (fillArgs == TRUE) {
                ...
                surrogateNameInOwner =
                    (char *)(*(owner->surrogatePartNameList))[surrogateNum];
...
SoInteractionKit::connectSeparatorFields(SoSeparator *dest, SbBool onOff) {
    ...
    if (onOff) {
        SoField *f;
        if (!dest->renderCaching.getConnectedField(f) || f != &renderCaching)
        ...
        if (!dest->renderCulling.getConnectedField(f) || f != &renderCulling)
            dest->renderCulling.connectFrom(&renderCulling);
        if (!dest->pickCulling.getConnectedField(f) || f != &pickCulling)
...
#define __SO_FCHECK(f, v)                                                      \
    (!(f.isConnected() && f.isConnectionEnabled()) && f.getValue() == v)
