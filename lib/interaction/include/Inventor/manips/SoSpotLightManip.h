...
/// \ingroup Manips
/// <tt>SoSpotLightManip</tt>
/// is the base class for all <tt>SoSpotLight</tt> nodes that have a built-in
/// 3D user interface (this is the only such class provided with the
/// Inventor toolkit).
/// Since it is derived from <tt>SoSpotLight</tt>, any changes to its
/// fields result in a change of lighting for nodes that
/// follow it in the scene graph.
/// In this case, the interface edits the #location, #direction, and
/// #cutOffAngle fields.
...
/// see the reference page for <tt>SoSpotLightDragger</tt>.  The interfaces of the
...
/// when finished.
/// Then the <tt>SoSpotLightManip</tt> adds a spotlight to the state,
/// just like its base class, <tt>SoSpotLight</tt>.
/// <b>SoSearchAction</b>
/// Searches just like an <tt>SoSpotLight</tt>.  Does not search the dragger,
/// which is a hidden child.
/// <b>SoWriteAction</b>
/// Writes out just like an <tt>SoSpotLight</tt>. Does not write the dragger,
...
/// }
/// \endcode
/// \sa SoDragger,SoSpotLight,SoSpotLightDragger,SoDirectionalLightManip,SoPointLightManip
class INVENTOR_API SoSpotLightManip : public SoSpotLight
{
  ...
  public:
    ...
    /// The tail of the path must be an \c SoSpotLight node (or subclass thereof).
    ...
    /// with the given \c SoSpotLight node.
    /// If the path has a nodekit, this will try to use
