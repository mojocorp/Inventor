...
/// \ingroup Manips
/// <tt>SoPointLightManip</tt>  is the base class for all <tt>SoPointLight</tt>
...
/// the <tt>SoPointLightManip</tt> adds a point light into the state, just like
/// its base class, <tt>SoPointLight</tt>. <b>SoSearchAction</b> Searches just
/// like an <tt>SoPointLight</tt>.  Does not search the dragger, which is a
/// hidden child. <b>SoWriteAction</b> Writes out just like an
/// <tt>SoPointLight</tt>. Does not write the dragger, which is a hidden child.
/// If you really need to write valuable information about the dragger, such as
/// customized geometry, you can retrieve the dragger with the #getDragger()
...
class SoPointLightManip : public SoPointLight {
  ...
  public:
    ...
    /// The tail of the path must be an SoPointLight node (or subclass
