...
/// is the base class for all <tt>SoDirectionalLight</tt> nodes that have a
...
///
///
/// Typically, you will want to replace a regular <tt>SoDirectionalLight</tt>
/// with an <tt>SoDirectionalLightManip</tt> (as when the user selects a light
/// to be edited), or vice versa (as when the user is done moving the light and
/// the interface should go away). Use the #replaceNode() method to insert a
/// manipulator into a scene graph, and the #replaceManip() method to remove it
/// when done.
///
///
...
/// see the reference page for <tt>SoDirectionalLightDragger</tt>.  The
...
/// the <tt>SoDirectionalLightManip</tt> adds a directional light to the state
/// just like its base class, <tt>SoDirectionalLight</tt>.
/// <b>SoSearchAction</b>
/// Searches just like an <tt>SoDirectionalLight</tt>.  Does not search the
/// dragger, which is a hidden child. <b>SoWriteAction</b> Writes out just like
/// an <tt>SoDirectionalLight</tt>. Does not write the dragger, which is a
...
class INVENTOR_API SoDirectionalLightManip : public SoDirectionalLight {
  ...
  public:
    ...
    /// SoBaseKit::setPart() to insert the manipulator.
