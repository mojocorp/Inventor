...
/// \ingroup Manips
/// <tt>SoDirectionalLightManip</tt>
/// is the base class for all <tt>SoDirectionalLight</tt> nodes that have a built-in
/// 3D user interface (this is the only such class provided with the
/// Inventor toolkit).
/// Since it is derived from <tt>SoDirectionalLight</tt>, any changes to its
/// fields result in a change of lighting for nodes that
/// follow it in the scene graph.
/// In this case, the interface edits the #direction field.
...
///
///
/// Typically, you will want to replace a regular <tt>SoDirectionalLight</tt> with an
/// <tt>SoDirectionalLightManip</tt> (as when the user selects a light to be edited),
/// or vice versa (as when the user is done moving the light and the interface
/// should go away).
/// Use the #replaceNode() method
/// to insert a manipulator into a scene graph, and the
/// #replaceManip() method to remove it when done.
///
///
/// The <tt>SoDirectionalLightManip</tt>
/// utilizes an <tt>SoDirectionalLightDragger</tt> to provide a 3D interface.
/// However, the manipulator differs from the dragger; it lights other objects
/// in the scene because, as an <tt>SoDirectionalLight</tt>,
/// it alters the state.
...
/// see the reference page for <tt>SoDirectionalLightDragger</tt>.  The interfaces of the
/// dragger and the manipulator are identical.
///
///
/// The <tt>SoDirectionalLightManip</tt> utilizes its dragger by adding it
...
/// #direction field of the <em>manipulator</em> will change by the same
/// amount, thus changing the lighting of nodes which follow
/// in the scene graph.
/// Similarly, if you set the #direction field of the
/// <tt>SoDirectionalLightManip</tt>, the manipulator will
...
/// does not show up when you write the manipulator to file. Also, any <tt>SoPath</tt>
...
///
/// \par Action behavior:
/// <b>SoGLRenderAction, SoCallbackAction, SoGetBoundingBoxAction, SoGetMatrixAction, SoHandleEventAction, SoRayPickAction</b>
/// First, traverses the dragger the way an <tt>SoGroup</tt> would. All draggers
/// place themselves in space, but leave the current transformation unchanged
/// when finished.
/// Then the <tt>SoDirectionalLightManip</tt> adds a
/// directional light to the state
/// just like its base class, <tt>SoDirectionalLight</tt>.
/// <b>SoSearchAction</b>
/// Searches just like an <tt>SoDirectionalLight</tt>.  Does not search the dragger,
/// which is a hidden child.
/// <b>SoWriteAction</b>
/// Writes out just like an <tt>SoDirectionalLight</tt>. Does not write the dragger,
...
/// }
/// \endcode
/// \sa SoDragger,SoDirectionalLight,SoDirectionalLightDragger,SoPointLightManip,SoSpotLightManip
class INVENTOR_API SoDirectionalLightManip : public SoDirectionalLight {
  ...
  public:
    ...
    /// the methods found in the \c SoDragger reference page.
    SoDragger *getDragger();

    /// Replaces the tail of the path with this manipulator.
    /// The tail of the path must be an \c SoDirectionalLight
    ...
    /// with the given \c SoDirectionalLight node.
    /// If the path has a nodekit, this will try to use
