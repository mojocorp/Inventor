...
///
/// Every subclass of <tt>SoTransformManip</tt> utilizes a dragger of some sort to
...
/// where you specified.
///
/// Since each <tt>SoTransformManip</tt> uses a dragger to provide its interface,
...
/// when finished.
/// Then the <tt>SoTransformManip</tt> accumulates a transformation into the
/// current transformation just like its base class, <tt>SoTransform</tt>.
/// <b>SoSearchAction</b>
...
/// }
/// \endcode
/// \sa SoDragger,SoTransform,SoCenterballManip,SoHandleBoxManip,SoJackManip,SoSurroundScale,SoTabBoxManip,SoTrackballManip,SoTransformBoxManip,SoTransformerManip
class INVENTOR_API SoTransformManip : public SoTransform
{
  ...
  public:
    ...
    /// The tail of the path must be an \c SoTransform node (or subclass thereof).
    ...
    /// with the given \c SoTransform node.
    /// If the path has a nodekit, this will try to use
