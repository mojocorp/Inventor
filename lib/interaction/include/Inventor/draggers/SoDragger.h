...
/// events.
/// \ingroup Draggers
/// <tt>SoDragger</tt> is the base class for all
/// nodekits you move by using the mouse to click-drag-and-release. More
/// specifically, they are operated by a start (mouse button 1 pressed over
/// dragger to pick it), followed by dragging (mouse motion events are
/// interpreted by the dragger and result in some form of motion and/or change
/// to a field), followed by finish (mouse up).
///
///
...
/// 3D plane, then translates to follow the cursor as it moves within that
/// plane.
///
///
/// Every dragger has <em>fields</em> that describe its current state.
/// Scaling draggers have a <b>scaleFactor</b> field,  rotational draggers have
/// a <b>rotation</b> field, etc.
...
/// <tt>SoTransformBoxDragger</tt> is a composite dragger made entirely of
/// simple draggers.  <tt>SoDirectionalLightDragger</tt> contains both a simple
/// dragger
/// (<tt>SoRotateSphericalDragger</tt>) and a composite dragger
/// (<tt>SoDragPointDragger</tt>) When using a composite dragger, the fields of
/// the composite dragger are the ones you should work with. Draggers lower down
/// in the assemblage usually have zeroed out values. For example, when you drag
/// the face of a transformBox, an <tt>SoTranslate2Dragger</tt>, the
/// transformBox "steals" the translation from the child dragger and transfers
/// it up to the top of the composite dragger, where it effects all pieces of
/// the assemblage.
///
///
...
/// dragging, (by calling setValue(), for example), the dragger's internal
/// <tt>SoFieldSensor</tt> will sense this and the dragger will move to satisfy
/// that new value.
///
///
/// This makes it easy to constrain draggers to keep their fields within certain
/// limits: if the limit is exceeded, just set it back to the exceeded maximum
/// or minimum.  You can do this even as the dragger is in use, by constraining
/// the field value within a value-changed callback that you add with
...
/// as <tt>SoTrackballDragger</tt>, have a corresponding
/// <tt>SoTransformManip</tt>, in this case <tt>SoTrackballManip</tt>.  The
/// manipulator is a subclass of <tt>SoTransform</tt>, and affects other objects
/// in the scene; it uses a trackball <em>dragger</em> to provide its user
/// interface. In this way, draggers are employed extensively by manipulators.
...
/// node with that <b>resourceName</b>. By putting a new entry in the
/// dictionary, you can override that default. The default part geometries are
/// defined as resources for each class, and each class has a file you can
/// change to alter the defaults. The files are listed in each dragger's man
/// page. You can make your program use different default resources for the
/// parts by copying the listed file from the directory
...
/// \endcode
/// \sa
/// SoInteractionKit,SoCenterballDragger,SoDirectionalLightDragger,SoDragPointDragger,
/// \sa
...
class INVENTOR_API SoDragger : public SoInteractionKit {
  ...
  public:
    ...
    /// fields. This does not include changes to the <b>isActive</b> field. See
    /// also #enableValueChangedCallbacks().
    ...
    /// Disable first, then change the field, then re-enable the callbacks (if
    /// they were enabled to start with). All this prevents you from entering an
    /// infinite loop of changing values, calling callbacks which change values,
    /// etc.
