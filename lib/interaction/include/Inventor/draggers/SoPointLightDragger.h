...
/// directions, and has a #translation field that
/// always reflects its position in local space.
/// The point light dragger also has a special \a material part which can be
...
///
///
/// [a] Use an <tt>SoPointLightManip</tt>, which is subclassed from <tt>SoLight</tt>.
/// It creates one of
/// these draggers and uses it as the interface to change the #location of its
/// light source (see the <tt>SoPointLightManip</tt> reference page).
...
/// because the default part contains no <tt>SoMaterial</tt> nodes.   This fact
/// enables the <tt>SoPointLightManip</tt> (not the dragger, the manipulator) to
/// color its dragger to match the color of the light it is emanating.
/// Recall that a point light <em>manip</em> is
/// derived from <tt>SoLight</tt> and creates a point light <em>dragger</em> to provide
/// an interface and geometrical presence on screen. The manipulator also has a
/// #color field; when the light color changes, it changes the #material
...
/// \sa SoDragPointDragger,SoHandleBoxDragger,SoJackDragger,SoRotateCylindricalDragger,
