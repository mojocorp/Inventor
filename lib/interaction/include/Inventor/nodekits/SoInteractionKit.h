...
class INVENTOR_API SoInteractionKit : public SoBaseKit {
  ...
  public:
    ...
    virtual SbBool setPartAsDefault(const SbName &partName, SoNode *newNode,
  ...
  protected:
    ...
    static void
    readDefaultParts(const char *        fileName,        // file to read
    ...
    virtual SbBool setAnyPartAsDefault(const SbName &partName, SoNode *newNode,
                                       SbBool anyPart = TRUE,
                                       SbBool onlyIfAlready = TRUE);
  ...
  private:
    ...
    static SoGroup *readFromBuffer(const unsigned char defaultBuffer[],
                                   int                 defBufSize);
};
