...
/// \ingroup Interaction
/// <tt>SoSelection</tt> defines a node which can be inserted into a scene graph
...
///
///
/// When handling events, <tt>SoSelection</tt> makes sure that the mouse release
...
/// <tt>SoXtRenderArea</tt>, or the application can provide custom highlights.
/// Please see the chapter "Creating a Selection Highlight Style" in the
...
class SoSelection : public SoSeparator {
  ...
  public:
    ...
    /// instance of \a node beneath the selection node, the created
    ...
    /// The argument \a which specifies which path in the list to be removed.
    ...
    /// the selection list, it is removed; if not in the list, it is added</em>.
    ...
    /// Returns TRUE if the passed node is selected by creating a path to it,
    /// then calling isSelected(). If there is more than one
    /// instance of \a node beneath the selection node, the created
    /// path will be the first instance found.
    ...
    /// Return the \a i th path in the selection list.
    SoPath *getPath(int index) const;

    /// Return the \a i th path in the selection list.
    SoPath *operator[](int i) const { return getPath(i); }
    ...
    /// whether it be from user interaction or from method call. The callbacks
    /// are invoked after the object has been added to the selection list.
    ...
    /// deselected, whether it be from user interaction or from method call.
    ...
    /// Remove the Deselection callback;
    ...
    /// interactive change to the selection list (by picking objects). This will
    /// be followed by invocations of the select and/or deselect callbacks,
    /// finally followed by each finish callback. A start callback can be used,
    /// for instance, to save the current selection for later restoration (e.g.
    /// undo/redo). The start callbacks are not called when the selection list
    ...
    /// interactively changing the selection list (by picking objects). This was
    /// preceded by an invocation of each start callback, and invocations of the
    /// select and/or deselect callbacks. The finish callbacks are not called
    ...
    ///
    /// The \a callOnlyIfSelectable argument, when set to TRUE, means the pick
    ...
    // Return TRUE is pickMatching is enabled.
