...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (theNode != NULL && !theNode->isOfType(partType)) {
#ifdef DEBUG
        SoDebugError::post(
            "SoV1BaseKit::getAnyPart",
...
SoV1BaseKit::getPart(const SbName &partName, const SoType &partType,
                     SbBool makeIfNeeded, SoPath *inheritancePath)
//
////////////////////////////////////////////////////////////////////////
{
...
////////////////////////////////////////////////////////////////////////
{
    ...
    //   for 'this' of course).
    if (!inst->customData.isDefault() && (inst->customData[(int)index] != "")) {
        SbName entryName;
    ...
    while (newPartName && newChildNumString) {
        ...
        if (partNum == SO_V1_CATALOG_NAME_NOT_FOUND) {
            ...
            SoDebugError::postWarning(
                "SoV1BaseKit::interpretCustomData",
        ...
        if (isOk) {
            ...
            if (!parentNode || newChildNum >= parentNode->getNumChildren()) {
#ifdef DEBUG
                SoDebugError::post("SoV1BaseKit::interpretCustomData",
                                   "parent of node %s only has %d children, "
                                   "but you are asking for child number %d",
                                   newPartName, parentNode->getNumChildren(),
                                   newChildNum);
#endif
                isOk = FALSE;
            }
        ...
        if (isOk) {
            ...
            if (!childNode || !childNode->isOfType(theCat->getType(partNum))) {
#ifdef DEBUG
                SoDebugError::post("SoV1BaseKit::interpretCustomData",
...
#ifdef DEBUG
                SoDebugError::post("SoV1BaseKit::interpretCustomData",
                                   " given part %s already in parts list",
                                   newPartName);
#endif
                isOk = FALSE;
            }
...
#ifdef DEBUG
                    SoDebugError::post("SoV1BaseKit::interpretCustomData",
                                       " part %s is list, but children chosen "
                                       "node are not of the list item type",
                                       newPartName);
#endif
                    isOk = FALSE;
                }
        ...
        if (!tryToSetPartInNewNode(newKit, thePart, theName)) {
            ...
            if (!dealWithUpgradedPart(newKit, thePart, theName)) {
                ...
                SoDebugError::post("SoV1BaseKit::setUpNewNode",
                                   "Dont know what to do with part named %s. "
                                   "The node is going to be ignored and lost.",
                                   theName.getString());
#endif
            }
        }
...
//
SbBool
SoV1BaseKit::tryToSetPartInNewNode(SoBaseKit *newNode, SoNode *newPart,
    ...
    if (cat->isList(partNumber)) {
        ...
        if (!newPart->isOfType(SoGroup::getClassTypeId())) {
            SoDebugError::post("SoV1BaseKit::dealWithUpgradedPart",
                               "the node for part %s in the old node isn't "
                               "subclassed from SoGroup",
                               newPartName.getString());
            return FALSE;
        }
#endif
