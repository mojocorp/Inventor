...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (rootOfKit == NULL) {
        SoDebugError::post("SoNodekitParts::SoNodekitParts",
                           "rootOfKit is NULL. Can not continue construction. "
                           "Expect a core dump");
        return;
    }
#endif
...
////////////////////////////////////////////////////////////////////////
{
    ...
    // create the node
    SoNode *inst = (SoNode *)catalog->getDefaultType(partNum).createInstance();
#ifdef DEBUG
    if (inst == NULL) {
    ...
    if (sibPartNum == -1) {
        ...
        if (parentKit != NULL) {
            int sibIndex =
                parentKit->findChild(fieldList[sibPartNum]->getValue());
            parentKit->insertChild(fieldList[partNum]->getValue(), sibIndex);
        } else {
            int sibIndex =
                parentGroup->findChild(fieldList[sibPartNum]->getValue());
...
#ifdef DEBUG
            SoDebugError::post("SoNodekitParts::replacePart",
                               "Allowable child types in given list do not "
                               "match the specs in the catalog");
#endif
            return FALSE;
        }
    }
...
#ifdef DEBUG
            SoDebugError::post("SoNodekitParts::replacePart",
                               "The given part is a non-leaf in the catalog "
                               "and contains children. When setting a "
                               "non-leaf, the node must have 0 children.");
#endif
            return FALSE;
        }
    }
...
SoNode *
SoNodekitParts::getPartFromThisCatalog(const int partNum, SbBool makeIfNeeded,
...
////////////////////////////////////////////////////////////////////////
{
    SoNode *n = getPartFromThisCatalog(partNum, makeIfNeeded, leafCheck,
...
SbBool
SoNodekitParts::setPartFromThisCatalog(const int partNum, SoNode *newPartNode,
                                       SbBool anyPart)
//
////////////////////////////////////////////////////////////////////////
{
    ...
    if (partNum == SO_CATALOG_THIS_PART_NUM) {
        SoDebugError::post(
            "SoNodekitParts::setPartFromThisCatalog",
...
SoNode *
SoNodekitParts::getSingleNamePart(const SbName &nameOfPart, SbBool makeIfNeeded,
                                  SbBool leafCheck, SbBool publicCheck,
                                  SbBool &existedBefore)
//
////////////////////////////////////////////////////////////////////////
    ...
    if (nameOfPart.rfind('[') != -1) {
        ...
        if (!n->isOfType(SoNodeKitListPart::getClassTypeId())) {
            ...
            SoDebugError::post("SoNodekitParts::getSingleNamePart",
                               "part specified %s was not a list",
        ...
        if ((listGroup->getNumChildren() == arrayIndex) && makeIfNeeded) {
            ...
            if (newNode == NULL) {
                ...
                if (SoDebug::GetEnv("IV_DEBUG_KIT_PARTS")) {
                    ...
                    fprintf(stderr,
                            "    and could not be created.The list %s\n",
                            listNameCopy.getString());
        ...
        if (listGroup->getNumChildren() <= arrayIndex) {
            if (makeIfNeeded) {
                ...
                if (SoDebug::GetEnv("IV_DEBUG_KIT_PARTS")) {
                    ...
                    fprintf(stderr,
                            "    %d entries. You want entry number %d\n",
    ...
    if (partNum != SO_CATALOG_NAME_NOT_FOUND)
        ...
        for (int i = 0; i < numEntries; i++) {
            ...
            if (catalog->recursiveSearch(i, nameOfPart, typesChecked) == TRUE) {
                ...
                SoNode *        answer =
                    interParts->getAnyPart(nameOfPart, makeIfNeeded, leafCheck,
                                           publicCheck, answerExistedBefore);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (nameOfPart.rfind('[') != -1) {
        ...
        // get the list given by 'listNameCopy'
        SbBool listExistedBefore;
        answerPath = getSingleNamePathToPart(listNameCopy, makeIfNeeded, TRUE,
                                             publicCheck, listExistedBefore);
        if (answerPath == NULL) {
            return NULL;
        }
        ...
        if (listGroup->getNumChildren() <= arrayIndex) {
            if (makeIfNeeded) {
                ...
                if (SoDebug::GetEnv("IV_DEBUG_KIT_PARTS")) {
                    ...
                    fprintf(
                        stderr,
                        "NODE KIT PARTS DEBUG: Could not return the part\n");
                    fprintf(stderr, "    %s. The list %s only has\n",
                            nameOfPart.getString(), listNameCopy.getString());
                    fprintf(stderr,
                            "    %d entries, but you want entry number %d\n",
                            listGroup->getNumChildren(), arrayIndex);
                    fprintf(stderr, "    Returning NULL\n");
                    fprintf(stderr, "\n");
                }
    ...
    if (partNum != SO_CATALOG_NAME_NOT_FOUND)
        ...
        // IF SO, THEN GET IT FROM THIS CATALOG
        return (createPathToPartFromThisCatalog(
            partNum, makeIfNeeded, leafCheck, publicCheck, existedBefore));
        ...
        for (int i = 0; i < numEntries; i++) {
            ...
            if (catalog->recursiveSearch(i, nameOfPart, typesChecked) == TRUE) {
                ...
                SoFullPath *    pathB =
                    (SoFullPath *)interParts->createPathToAnyPart(
                        nameOfPart, makeIfNeeded, leafCheck, publicCheck,
                        secondExistedBefore);
                if (pathA)
                    pathA->ref();
                if (pathB)
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (nameOfPart.rfind('[') != -1) {
        ...
        SbBool  listExistedBefore;
        SoNode *n = getSingleNamePart(listNameCopy, needToMake, TRUE, !anyPart,
                                      listExistedBefore);
...
#ifdef DEBUG
        SoDebugError::post("SoNodekitParts::setSingleNamePart",
                           "Node given is wrong type. Its type is %s, which is "
                           "inappropriate for list named %s",
                           newPartNode->getTypeId().getName().getString(),
                           listNameCopy.getString());
#endif
        if (listExistedBefore == FALSE) {
            // if we just created the list, we'd better get rid of it...
    ...
    if (partNum != SO_CATALOG_NAME_NOT_FOUND)
        ...
        for (int i = 0; i < numEntries; i++) {
            ...
            if (catalog->recursiveSearch(i, nameOfPart, typesChecked) == TRUE) {
                ...
                SoNodekitParts *interParts = intermediary->nodekitPartsList;
                SbBool          didIt =
                    interParts->setAnyPart(nameOfPart, newPartNode, anyPart);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    firstNode = getSingleNamePart(firstName, makeIfNeeded, TRUE, publicCheck,
                                  firstExistedBefore);
    ...
    // parts
        ...
        SoDebugError::post("SoNodekitParts::getAnyPart",
                           "you asked for %s but %s is not a nodeKit, so you "
                           "can not look inside it for %s. returning NULL",
        ...
        SbBool          secondExistedBefore;
        secondNode =
            interParts->getAnyPart(remainderString, makeIfNeeded, leafCheck,
...
////////////////////////////////////////////////////////////////////////
{
    ...
    // JUST A SINGLE NAME...
    if (nameOfPart.rfind('.') == -1)
        return getSingleNamePathToPart(nameOfPart, makeIfNeeded, leafCheck,
                                       publicCheck, existedBefore);
    ...
    if (firstPath == NULL) {
        ...
        // parts
...
#ifdef DEBUG
            SoDebugError::post(
                "SoNodekitParts::createPathToAnyPart",
                "you asked for %s but %s is not a nodeKit, so you can not look "
                "inside it for %s. returning NULL",
                nameOfPart.getString(), firstName, remainderString);
#endif
            if (firstExistedBefore == FALSE) {
                // if we just created the node, we'd better get rid of it...
            ...
            SbBool secondExistedBefore;
            secondPath = (SoFullPath *)interParts->createPathToAnyPart(
                remainderString, makeIfNeeded, leafCheck, publicCheck,
                secondExistedBefore);
            if (secondPath)
                secondPath->ref();
...
////////////////////////////////////////////////////////////////////////
{
    ...
    SbBool needToMake = (newPartNode != NULL);
    SbBool firstExistedBefore;
    firstNode = getSingleNamePart(firstName, needToMake, TRUE, !anyPart,
                                  firstExistedBefore);
...
SoFullPath *
SoNodekitParts::addPaths(const SoFullPath *pathA, const SoFullPath *pathB) const
//
////////////////////////////////////////////////////////////////////////
{
    if (pathA == NULL)
...
#ifdef DEBUG
        SoDebugError::post(
            "SoNodekitParts::partIsLeafCheck",
            "can't return the part %s because it is not a leaf node in the "
...
#ifdef DEBUG
        SoDebugError::post(
            "SoNodekitParts::partIsPublicCheck",
    ...
    if (partNum == SO_CATALOG_THIS_PART_NUM) {
        if (rootPointer)
            ...
            SoDebugError::post("SoNodekitParts::verifyPartExistence",
                               "the part \"this\" is NULL, and should NEVER "
                               "be. Expect a core dump");
#endif
            return FALSE;
        }
    }
