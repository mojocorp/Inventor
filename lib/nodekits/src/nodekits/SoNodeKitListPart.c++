...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (readOK) {
        ...
        if (!ct.isDerivedFrom(SoGroup::getClassTypeId()) ||
            ...
            SoReadError::post(in, "Given container type is not derived from "
                                  "Group or is an abstract class");
            readOK = FALSE;
        } else {
            // [4] Copy child types from name field into type list.
            //     If none are specified, then allow anything by making
            ...
            if (contNode != NULL && (contNode->isOfType(ct) == FALSE)) {
                SoReadError::post(in,
                                  "Given container node is of incorrect type");
                readOK = FALSE;
                ...
                for (int i = grp->getNumChildren() - 1; i >= 0; i--) {
                    if (!isChildPermitted(grp->getChild(i))) {
                        SoReadError::post(
                            in, "Removing child number %d of illegal type", i);
                        grp->removeChild(i);
                    }
                }
            }
...
////////////////////////////////////////////////////////////////////////
{
    if (isTypeLocked()) {
        ...
        SoDebugError::post("SoNodeKitListPart::setContainerType",
                           "You can\'t change the type because the type lock "
                           "has been turned on");
#endif
        return;
    }
    ...
    if (oldContainer == NULL || !oldContainer->isOfType(newContainerType)) {
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (childTypes.find(typeToAdd) == -1) {
        ...
        childTypeNames.set1Value(childTypes.getLength() - 1,
                                 typeToAdd.getName());
    }
}
