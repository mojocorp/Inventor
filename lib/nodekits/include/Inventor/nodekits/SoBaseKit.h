...
/// you can use a nodekit of the appropriate type (in this case,
/// <tt>SoShapeKit</tt>).
///
///
/// Each class of nodekit has a <em>nodekit catalog</em>
/// (<tt>SoNodekitCatalog</tt>) that describes the nodes in the subgraph,
/// referred to as <em>parts</em>. The catalog has an entry for each part, with
/// information such as the <em>partName</em>, <em>partType</em>, and
/// <em>nullByDefault</em> (if FALSE the constructor creates it). The catalog
/// also describes the arrangement of parts in the subgraph. (Other information
/// is described below; a complete description is in the
...
/// other leaves are <em>private</em>. All non-leaf parts are considered
/// internal to the nodekit structure and are marked private. Public parts are
/// accessible; they may be requested, changed, or set by the programmer with
/// member functions such as #getPart().  Private parts are not accessible, so
/// methods such as #getPart() will have no effect on them. For example,
/// if you call #getPart() to retrieve a private part, <b>NULL</b> will be
...
///
///
/// Since nodekits hide their children, any <tt>SoPath</tt> containing nodekits
/// will end at the topmost nodekit.  However, since nodekits may be nested
/// within other nodekits, you may wish to cast an <tt>(SoPath *)</tt> into an
/// <tt>(SoNodeKitPath *)</tt>. The methods of <tt>SoNodeKitPath</tt> allow you
/// to view all nodekits that lie on the path (see the reference page for
/// <tt>SoNodeKitPath</tt>).
///
///
...
///
///
/// [2] <em>nodekits</em>, or <em>nested nodekits</em> (which may nest
/// recursively). Any node which is public in a nested nodekit is accessible to
...
///
///
/// [3] <em>lists</em>, or <em>list parts</em>. These parts group together
/// children
/// (<em>list elements</em>) of a particular type or types.  As with nested
/// nodekits, you can refer to individual elements using notation described in
/// #getPart()
...
/// add illegal children. (See the <tt>SoNodeKitListPart</tt> reference page for
/// more information).  As an example, the <em>callbackList</em> part of
/// <tt>SoBaseKit</tt> has an <tt>SoSeparator</tt> container and allows only
/// <tt>SoCallback</tt> and <tt>SoEventCallback</tt> nodes in the list. Children
/// may be added, retrieved, and removed from an <tt>SoNodeKitListPart</tt> node
/// using methods that parallel those of <tt>SoGroup</tt>. However,
...
/// <tt>SoWrapperKit</tt> has a part called <em>contents</em> with a part type
/// of <tt>SoSeparator</tt>. You can put whatever you want underneath the
...
/// For example, the <em>appearance</em> part of the <tt>SoSeparatorKit</tt> is
/// a leaf node in the <tt>SoSeparatorKit</tt> catalog. But <em>appearance</em>
/// is in turn an <tt>SoAppearanceKit</tt>, containing parts such as
/// <em>material</em> and <em>drawStyle</em>. The two nodekits combine to make
/// an even larger template, which the <tt>SoSeparatorKit</tt> can examine by
/// looking at the catalogs for both classes. So an <tt>SoSeparatorKit</tt> can
/// successfully return a part named \a "material"; first it finds (or creates)
/// the <em>appearance</em> part, then it gets the <em>material</em> by calling
/// #getPart() on the <em>appearance</em>.
///
///
...
/// SoHandleEventAction</b> Behaves like an <tt>SoGroup</tt>. Traverses each
/// child in order. <b>SoRayPickAction</b> Traverses each child in order. Then,
/// for any pick containing the kit on its path, makes an
/// <tt>SoNodeKitDetail</tt> as follows: Sets the <b>"detailNodeKit"</b>
/// (retrievable with #SoNodeKitDetail::getNodeKit()) to be a pointer to itself.
...
/// A nodekit does <em>not</em> write out its parts the way an <tt>SoGroup</tt>
/// writes out its children.  Instead, it writes each part as an
/// <tt>SoSFNode</tt> field. First the partName is written, then the node being
/// used for that part.
///
///
/// To keep the files terse, nodekits write out as few parts as possible.
/// However, nodekits <em>always</em> write a part if another instance or a path
/// is writing it. If this is not the case, parts are left out according to the
/// following rules:
///
///
...
class INVENTOR_API SoBaseKit : public SoNode {
  ...
  public:
    ...
    /// eturns the SoNodekitCatalog for this instance of SoBaseKit.
  ...
  public:
    ...
    /// Searches the nodekit catalog (and those of all nested nodekits) for the
    /// part named \a partName. Returns a pointer to the part if a \e "match is
    /// found", the part is \e public, and the part has \e "already been built".
    /// If no match is found, or if the part is \e private, <b>NULL</b> is
    /// returned. If \a partName is in the catalog (or that of one of its nested
    /// nodekit parts), but the part has not been built yet, the argument \a
    /// makeIfNeeded determines the course of action. When \a makeIfNeeded is
    /// <b>FALSE</b>, <b>NULL</b> is returned; when \a makeIfNeeded is
    /// <b>TRUE</b>, #getPart() will create the part (as well as any necessary
    /// intermediary parts), put it in the correct place, and return a pointer
    ...
    ///
    /// <b>singlePartName</b> = the name of any single part in the catalog
    ...
    /// It is okay for \a pathToExtend to go beyond the nodekit; extra nodes
    /// will be popped off the tail before continuing from <b>this</b> down to
    /// \a partName.
    ...
    /// if \a partName was a \e material for an SoSeparatorKit, but
    /// \a newPart was an SoTransform node, then the node would not be
    /// installed, and <b>FALSE</b> would be returned.
    ///
    /// If \a newPart is <b>NULL</b>, then the node specified by \a partName is
    /// removed. If this renders any private parts useless (as occurs when you
    /// remove the last child of an SoGroup node), they will also be removed.
    ...
    /// In this case, the nodekit SoSeparatorKit has a part named \e material
    /// which is of type SoMaterial.
    ///
    ...
    /// Sets and queries if nodekit children are searched during \c
  ...
  protected:
    ...
    virtual SoNodeKitPath *
                   createPathToAnyPart(const SbName &partName, SbBool makeIfNeeded,
...
#define SO_GET_PART(kitContainingPart, partName, partClassName)                \
    ((partClassName *)SoBaseKit::typeCheck(                                    \
        partName, partClassName::getClassTypeId(),                             \
...
#define SO_GET_ANY_PART(kitContainingPart, partName, partClassName)            \
    ((partClassName *)kitContainingPart->getAnyPart(partName, TRUE, FALSE,     \
                                                    FALSE))
#define SO_CHECK_ANY_PART(kitContainingPart, partName, partClassName)          \
    ((partClassName *)kitContainingPart->getAnyPart(partName, FALSE, FALSE,    \
                                                    FALSE))
#endif
