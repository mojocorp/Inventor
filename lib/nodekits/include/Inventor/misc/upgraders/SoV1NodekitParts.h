...
class INVENTOR_API SoV1NodekitParts {
    ...
    SoEXTENDER
  public:
    // Instances of SoV1NodekitParts are created only by subclasses of
    // SoV1BaseKit.

    // Constructor -- requires a catalog to build
    SoV1NodekitParts(SoV1BaseKit *     rootOfKit,
                     SoV1NodekitParts *partsSoFar = NULL);
  ...
  private:
    ...
    SbBool  setPartFromThisCatalog(const int partNum, SoNode *newPartNode,
                                   SbBool anyPart);
