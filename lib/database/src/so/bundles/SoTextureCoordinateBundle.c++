    ...
    if (!SoGLTextureEnabledElement::get(action->getState()))
        ...
        case SoGLTextureCoordinateElement::EXPLICIT:
            if (GLTexCoordElt->getNum() > 0) {
                ...
                setFunction = TRUE;
                SoNode *currentNode = action->getCurPathTail();
                SoGLTextureCoordinateElement::setTexGen(
                    state, currentNode, setUpTexGen, this, generateCoord, this);

                // Get the new instance in here
                texCoordElt = SoTextureCoordinateElement::getInstance(state);
                GLTexCoordElt =
                    (const SoGLTextureCoordinateElement *)texCoordElt;
...
#ifdef DEBUG
            SoDebugError::post("SoTextureCoordinateBundle::setUpForGLRender",
...
const SbVec4f &
SoTextureCoordinateBundle::generateCoord(void *userData, const SbVec3f &point,
...
////////////////////////////////////////////////////////////////////////
{
    ...
    result.setValue(point[sDim] * tcb->sVector[sDim] + tcb->sVector[3],
                    point[tDim] * tcb->tVector[tDim] + tcb->tVector[3], 0.0,
                    1.0);
