...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (normal.getNum() > 0) {
        SoNormalElement::set(state, this, normal.getNum(), normal.getValues(0));
    }
    SoNormalBindingElement::set(
        state, (SoNormalBindingElement::Binding)normalBinding.getValue());
    ...
    if (orderedRGBA.getNum() > 0 &&
        ...
        SoLazyElement::setPacked(state, this, orderedRGBA.getNum(),
                                 orderedRGBA.getValues(0), isTransparent());
    }
    // set material binding
    SoMaterialBindingElement::set(
        state, (SoMaterialBindingElement::Binding)materialBinding.getValue());
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (vertex.getNum() > 0)
        SoCoordinateElement::set3(action->getState(), this, vertex.getNum(),
                                  vertex.getValues(0));
}
...
void
vp_glColor4ubv(const GLubyte *v) {
    GLubyte _v[4];
...
void
SoVertexPropertyCache::fillInCache(const SoVertexProperty *vp, SoState *state) {
    renderCase = 0;
    ...
    if (needFromState & COLOR_FROM_STATE_BITS ||
        ...
        materialBinding =
            (SoMaterialBindingElement::Binding)vp->materialBinding.getValue();
    }
    ...
    if (vp && (numTexCoords = vp->texCoord.getNum()) != 0) {
        ...
        texCoordBinding = SoTextureCoordinateBindingElement::PER_VERTEX_INDEXED;
        ...
        if ((numTexCoords = tce->getNum()) > 0) {
            ...
            texCoordBinding = SoTextureCoordinateBindingElement::get(state);
            renderCase |= TEXCOORD_BIT;
            generateTexCoords = FALSE;
        } else {
            texCoordFunc = NULL;
