...
////////////////////////////////////////////////////////////////////////
{
    SO__NODE_INIT_CLASS(SoIndexedTriangleStripSet, "IndexedTriangleStripSet",
                        SoIndexedShape);
}
...
////////////////////////////////////////////////////////////////////////
{
    ...
    while (curIndex < numIndices) {
        ...
        for (int j = 0; j < vertsInStrip; j++) {
            ...
            curTexCoord =
                (texCoordsIndexed ? (int)texCoordIndices[curIndex] : curIndex);
            ...
            if (tcb.isFunction()) {
                if (!forPicking)
                    pvs[thisVert].setTextureCoords(tcb.get(
                        pvs[thisVert].getPoint(), pvs[thisVert].getNormal()));
            } else
            ...
            if (j >= 2) {
                ...
                if (j & 1) {
                    invokeTriangleCallbacks(action, &pvs[0], &pvs[2], &pvs[1]);
                } else {
                    invokeTriangleCallbacks(action, &pvs[0], &pvs[1], &pvs[2]);
                }
                curTri++;
            }
