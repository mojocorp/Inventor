...
SoLineSet::PMLS SoLineSet::renderFunc[32] = {
    &SoLineSet::OmOn, &SoLineSet::OmOnT, &SoLineSet::OmPn, &SoLineSet::OmPnT,
    &SoLineSet::OmFn, &SoLineSet::OmFnT, &SoLineSet::OmVn, &SoLineSet::OmVnT,
    &SoLineSet::PmOn, &SoLineSet::PmOnT, &SoLineSet::PmPn, &SoLineSet::PmPnT,
    &SoLineSet::PmFn, &SoLineSet::PmFnT, &SoLineSet::PmVn, &SoLineSet::PmVnT,
    &SoLineSet::FmOn, &SoLineSet::FmOnT, &SoLineSet::FmPn, &SoLineSet::FmPnT,
    &SoLineSet::FmFn, &SoLineSet::FmFnT, &SoLineSet::FmVn, &SoLineSet::FmVnT,
    &SoLineSet::VmOn, &SoLineSet::VmOnT, &SoLineSet::VmPn, &SoLineSet::VmPnT,
    &SoLineSet::VmFn, &SoLineSet::VmFnT, &SoLineSet::VmVn, &SoLineSet::VmVnT,
};
    ...
    for (line = 0; line < numLines; line++) {
        ...
        for (vert = 0; vert < vertsInLine; vert++) {
            ...
            if (tcb.isFunction()) {
                if (!forPicking)
                    pv->setTextureCoords(
                        tcb.get(pv->getPoint(), pv->getNormal()));
            } else
            ...
            if (vert > 0) {
                ...
                invokeLineSegmentCallbacks(action, &pvs[(vert - 1) % 2],
                                           &pvs[(vert - 0) % 2]);
...
SoLineSet::createLineSegmentDetail(SoRayPickAction *        action,
                                   ...
                                   const SoPrimitiveVertex *, SoPickedPoint *pp)
//
////////////////////////////////////////////////////////////////////////
{
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (vpCache.mightNeedSomethingFromState(shapeStyle)) {
        ...
        if (numPolylines && numVertices[numPolylines - 1] < 0) {
            ...
            numVertices.set1Value(numPolylines - 1, vpCache.numVerts -
                                                        totalNumVertices -
                                                        startIndex.getValue());
        ...
        if (vpCache.numVerts < startIndex.getValue() + totalNumVertices) {
            SoDebugError::post("SoLineSet::GLRender",
                               ...
                               " need %d, have %d",
                               startIndex.getValue() + totalNumVertices,
                               vpCache.numVerts);
        }
