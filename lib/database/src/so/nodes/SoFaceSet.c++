...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (vpCache.mightNeedSomethingFromState(shapeStyle)) {
        ...
        if (vpCache.colorIsInVtxProp()) {
            ...
            lazyElt->sendVPPacked(state,
                                  (const unsigned char *)vpCache.getColors(0));
        } else
            lazyElt->send(state, SoLazyElement::ALL_MASK);
...
SoFaceSet::setupNumTrisQuadsFaces() {
    ...
    if (nfaces && numVertices[nfaces - 1] < 0) {
        ...
        numVertices.set1Value(nfaces - 1, vpCache.numVerts - totalNumVertices -
                                              startIndex.getValue());
...
////////////////////////////////////////////////////////////////////////
{
    ...
    for (face = 0; face < numFaces; face++) {
        ...
        for (vert = 0; vert < vertsInFace; vert++) {
            ...
            int matlIndex = (materialBinding == PER_VERTEX
                                 ? curVert
                                 : materialBinding == PER_FACE ? face : 0);
            int normIndex = (normalBinding == PER_VERTEX
                                 ? curVert
...
////////////////////////////////////////////////////////////////////////
{
    ...
    for (int vert = 0; vert < vertsInFace; vert++) {
        pd.setCoordinateIndex(curVert);
        pd.setMaterialIndex(materialBinding == PER_VERTEX
                                ? curVert
                                : materialBinding == PER_FACE ? hitFace : 0);
        pd.setNormalIndex(normalBinding == PER_VERTEX
                              ? curVert
...
////////////////////////////////////////////////////////////////////////
{
    ...
    for (i = 0; i < numFaces; i++) {
        ...
        for (j = 0; j < vertsInFace; j++) {
            if (ce)
                nb->polygonVertex(ce->get3(curCoord));
            else
                nb->polygonVertex(vpCoords[curCoord]);
            curCoord++;
        }
        nb->endPolygon();
    }
