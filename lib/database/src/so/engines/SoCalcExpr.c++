...
static float *
lookupfloat(void *, const char *nm) {
...
static SbVec3f *
lookupvec3f(void *, const char *nm) {
...
Var::Var(const char *nm, Type type) : Expr(type), name(nm) {
    if (type == FLOAT)
...
float
Var::setFloat(float v) {
    return *pfloat = v;
}

SbVec3f
Var::setVec3f(const SbVec3f &v) {
    return *pvec3f = v;
}
...
Func_d::setArgs(ExprList *a) {
    args = a;
    if (args->getLength() != 1)
        err("Function %s: wrong number of arguments (%d should be 1)", name,
            args->getLength());
...
Func_dd::setArgs(ExprList *a) {
    args = a;
    if (args->getLength() != 2)
        err("Function %s: wrong number of arguments (%d should be 2)", name,
            args->getLength());
    if ((*args)[0]->type != FLOAT)
...
Func *
Funcv_ddd::dup() const {
    return new Funcv_ddd(name, func);
}
SbVec3f
Funcv_ddd::getVec3f() {
...
void
Funcv_ddd::setArgs(ExprList *a) {
    args = a;
    if (args->getLength() != 3)
        err("Function %s: wrong number of arguments (%d should be 3)", name,
            args->getLength());
    if ((*args)[0]->type != FLOAT)
        err("Function %s: first argument must be scalar", name);
    if ((*args)[1]->type != FLOAT)
...
BinaryOp::BinaryOp(Expr *ea, Expr *eb, int TypeBits) : a(ea), b(eb) {
    ...
    if (!OK)
        err("BinaryOp:  incompatible types (%s and %s)", a->typeName(),
            b->typeName());
}

AssignIndex::AssignIndex(Expr *_a, Expr *_b, Expr *_c) : Expr(Expr::FLOAT) {
    a = _a;
    b = _b;
    c = _c;
...
float
Assign::getFloat() {
    return a->setFloat(b->getFloat());
}
SbVec3f
Assign::getVec3f() {
    return a->setVec3f(b->getVec3f());
}
...
SbVec3f
Mult::getVec3f() {
    return (a->type == FLOAT) ? a->getFloat() * b->getVec3f()
...
float
Index::getFloat() {
    return a->getVec3f()[(int)b->getFloat()];
}
...
Ternary::Ternary(Expr *a, Expr *b, Expr *c) : etest(a), etrue(b), efalse(c) {
    if (etest->type != FLOAT)
...
float
Ternary::getFloat() {
    return etest->getFloat() ? etrue->getFloat() : efalse->getFloat();
}
SbVec3f
Ternary::getVec3f() {
    return etest->getFloat() ? etrue->getVec3f() : efalse->getVec3f();
}

void
Ternary::print(int level) {
#ifdef DEBUG
    printSpace(level);
...
Parser::Parser(float *(*lf)(void *data, const char *name),
               SbVec3f *(*lv)(void *data, const char *name), void *d) {
    lookupFloatField = lf;
    lookupVec3fField = lv;
    data = d;
