...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (whichInput == &min || whichInput == &max || whichInput == &step ||
        whichInput == &frequency || whichInput == &duty) {
        todo |= RECALC;
        ...
        if (off >= stages[curStage + 1].offset ||
            off < stages[curStage].offset) {
            enable = TRUE;
...
#ifdef DEVELOP
    printf("TimeCounter::inputChanged %s state=%s on=%d reset=%d enable=%d "
           "syncEnable=%d todo=",
           whichInput == &min
               ? "min   "
               : whichInput == &max
                           ...
                           : whichInput == &reset
                                 ...
                                 : whichInput == &syncIn
                                       ...
                                       : whichInput == &on
                                             ...
                                             : whichInput == &timeIn ? "------"
                                                                     : " ...  ",
           state == ON ? "ON    " : state == PAUSED ? "PAUSED" : "??????",
           on.getValue(), reset.getValue(), enable, syncEnable);
    if (todo) {
        int t = todo;
        if (todo & RECALC)
            printf("RECALC,"), t &= ~RECALC;
        if (todo & RESET)
            printf("RESET,"), t &= ~RESET;
        if (todo & SYNC)
            printf("SYNC,"), t &= ~SYNC;
        if (todo & PAUSE)
            printf("PAUSE,"), t &= ~PAUSE;
        if (todo & UNPAUSE)
            printf("UNPAUSE,"), t &= ~UNPAUSE;
        if (t)
            printf("????");
    } else
        printf("0");
    ...
    */
    ...
    if (todo & RECALC) {
        ...
        for (i = 1; i < nStages; i++) {
            stages[i].offset =
                stages[i - 1].offset + (stages[i - 1].duty * period);
        }
        ...
        for (i = 0; i < nStages; i++) {
            printf("\tstage %d:\tval=%d\tduty=%g\toffset=%g\n", i,
                   stages[i].val, stages[i].duty, stages[i].offset.getValue());
        }
#endif
    }
...
#ifdef DEVELOP
        printf(
            "TimeCounter::evaluate PAUSE now=%s baseTime=%s pauseOffset=%s\n",
            now.format("%s.%i").getString(),
            baseTime.format("%s.%i").getString(),
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (state == ON) {
        ...
        while (now - baseTime >= period)
            baseTime += period;
        while (baseTime - now > period)
            baseTime -= period;
    ...
    printf("TimeCounter::evaluate state=%s now=%s baseTime=%s offset=%s "
           ...
           now.format("%s.%i").getString(),
           baseTime.format("%s.%i").getString(),
           (now - baseTime).format("%s.%i").getString(), curStage,
           stages[curStage].val);
#endif
