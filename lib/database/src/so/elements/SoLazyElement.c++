...
SoLazyElement::getDiffuse(SoState *state, int index) {
    ...
    if (index > curElt->ivState.numDiffuseColors || index < 0) {
        SoDebugError::post("SoLazyElement::getDiffuse", "invalid index");
...
SoLazyElement::getTransparency(SoState *state, int index) {
    ...
    if (index > curElt->ivState.numTransparencies || index < 0) {
        SoDebugError::post("SoLazyElement::getTransparency", "invalid index");
...
#ifdef DEBUG
    if (index > curElt->ivState.numDiffuseColors || index < 0) {
        SoDebugError::post("SoLazyElement::getColorIndex", "invalid index");
...
SoLazyElement::getAmbient(SoState *state) {
    SoLazyElement *curElt = getInstance(state);
    if (state->isCacheOpen())
        curElt->registerGetDependence(state, AMBIENT_MASK);
...
SoLazyElement::getEmissive(SoState *state) {
    SoLazyElement *curElt = getInstance(state);
    if (state->isCacheOpen())
        curElt->registerGetDependence(state, EMISSIVE_MASK);
...
SoLazyElement::getSpecular(SoState *state) {
    SoLazyElement *curElt = getInstance(state);
    if (state->isCacheOpen())
        curElt->registerGetDependence(state, SPECULAR_MASK);
...
SoLazyElement::getBlending(SoState *state) {
    SoLazyElement *curElt = getInstance(state);
    if (state->isCacheOpen())
        curElt->registerGetDependence(state, BLENDING_MASK);
...
SoLazyElement::setTransparency(SoState *state, SoNode *node, int32_t numTransp,
    ...
    uint32_t testNodeId;
    if (numTransp == 1 && transp[0] == 0.0)
        testNodeId = 0;
    else
        testNodeId = node->getNodeId();
...
///////////////////////////////////////////////////////////////////////
void
SoLazyElement::setPacked(SoState *state, SoNode *node, int32_t numColors,
...
SoLazyElement::setMaterials(SoState *state, SoNode *node, uint32_t bitmask,
    ...
    if (realSet) {
        ...
        curElt->setMaterialElt(node, realSet, cPacker, diffuse, transp, ambient,
    ...
    if (state->isCacheOpen()) {
        ...
        if (notRealSet)
            curElt->registerRedundantSet(state, notRealSet);
    }
}
////////////////////////////////////////////////////////////////////////
//
...
SoLazyElement::setTranspElt(SoNode *node, int32_t numTrans,
    ...
    if (transpar[0] > 0.0) {
        if (ivState.transpType == SoGLRenderAction::SCREEN_DOOR) {
            ivState.stippleNum = (int)(transpar[0] * getNumPatterns());
        }
    }
    if (numTrans == 1 && transpar[0] == 0.0)
        ivState.transpNodeId = 0;
    else
...
SoLazyElement::setPackedElt(SoNode *node, int32_t numColors,
    ...
    if ((ivState.transpType == SoGLRenderAction::SCREEN_DOOR) &&
        ((colors[0] & 0xff) != 0xff)) {
        ivState.stippleNum =
            (int)(getNumPatterns() * (1. - (colors[0] & 0xff) * (1. / 255.)));
    }
...
#ifdef DEBUG
    SoDebugError::post("SoLazyElement::matches", "Should never be called\n");
#endif
    return TRUE;
}
/////////////////////////////////////////////////////////////////////////
