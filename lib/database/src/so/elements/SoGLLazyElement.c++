...
////////////////////////////////////////////////////////////////////////
void
SoGLLazyElement::setTranspElt(SoNode *node, int32_t numTrans,
...
void
SoGLLazyElement::setMaterialElt(
    SoNode *node, uint32_t mask, SoColorPacker *cPacker,
...
SoGLLazyElement::sendVPPacked(SoState *state, const unsigned char *pcolor) {
    ...
    if (ivState.transpType == SoGLRenderAction::SCREEN_DOOR) {
        ...
        if (glState.GLStippleNum != 0 || trans != 0xff) {
            ...
            int newStipNum =
                (int)(getNumPatterns() * (1. - trans * (1.0 / 255.)));
...
#endif /*DEBUG*/
    SoGLRenderCache *thisCache =
        (SoGLRenderCache *)SoCacheElement::getCurrentCache((SoState *)state);
...
SoGLLazyElement::reset(SoState *state, uint32_t bitmask) const {
    ...
    if (state->isCacheOpen()) {
        SoGLRenderCache *thisCache =
            (SoGLRenderCache *)SoCacheElement::getCurrentCache(
                (SoState *)state);
        SoGLLazyElement *cacheLazyElt = thisCache->getLazyElt();
        cacheLazyElt->GLSendBits |= bitmask;
    }

    for (int j = 0; (j < SO_LAZY_NUM_COMPONENTS) && bitmask;
         j++, bitmask >>= 1) {
        if (bitmask & 1) {
            switch (j) {
    ...
    if (ivState.transpType != stateLazyElt->ivState.transpType) {
        ...
        if (SoDebug::GetEnv("IV_DEBUG_CACHES")) {
            ...
            fprintf(stderr, "prev,  current %d %d\n", ivState.transpType,
                    stateLazyElt->ivState.transpType);
        }
#endif /*DEBUG*/
        return FALSE;
    }
            ...
            case (DIFFUSE_CASE):
                if (glState.GLDiffuseNodeId !=
                    ...
                    if (SoDebug::GetEnv("IV_DEBUG_CACHES")) {
                        ...
                        fprintf(stderr, "prev,  current %d %d,  %d %d\n",
                                glState.GLDiffuseNodeId, glState.GLTranspNodeId,
                                stateLazyElt->glState.GLDiffuseNodeId,
                                stateLazyElt->glState.GLTranspNodeId);
                    }
#endif /*DEBUG*/
...
// dependent on inventor matching GL.
{
    ...
    for (int j = 0; (j < SO_LAZY_NUM_COMPONENTS) && (sendBits != 0);
        ...
        if (sendBits & 1) {
            ...
            case (LIGHT_MODEL_CASE):
                if (glState.GLLightModel == ivState.lightModel)
                    break;
    ...
    if (state->isCacheOpen()) {
        ...
        uint32_t checkGL =
            bitmask & (~didRealSendBits) & (~realSendBits) & (levelSetBits);
...
SoGLLazyElement::mergeCacheInfo(SoGLRenderCache *childCache,
                                ...
                                uint32_t doSendFlag, uint32_t checkIVFlag,
    ...
    uint32_t         doSend = (~parentDidRealSendBits) & doSendFlag &
                      (~ivState.cacheLevelSendBits) &
                      (~ivState.cacheLevelSetBits);
    ...
    uint32_t moreGL =
        (~parentDidRealSendBits) & (parentSetBits) & (checkGLFlag | doSendFlag);
    if (moreGL)
        copyGLValues(moreGL, parentLazyElt);
...
SoGLLazyElement::sendStipple(const SoState *state, int patIndex) {
    ...
    // so just send the pattern as is
             ...
             (patternListBase >= 0 && currentContext != patternListContext))
        glPolygonStipple(patterns[patIndex]);
