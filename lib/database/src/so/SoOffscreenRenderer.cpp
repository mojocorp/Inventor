...
public:
    SoOffscreenRendererInternal()
    {
        ...
        if (CGLCreatePBuffer(1, 1, GL_TEXTURE_2D, GL_RGBA, 0, &pbuffer) != kCGLNoError) {
            ...
            SoDebugError::post("SoOffscreenRenderer", "error creating pbuffer\n");
            return;
        }
        if (CGLSetPBuffer(context, pbuffer, 0, 0, 0) != kCGLNoError) {
            CGLDestroyContext(context);
            CGLDestroyPBuffer(pbuffer);
            SoDebugError::post("SoOffscreenRenderer", "error attaching pbuffer to context\n");
            return;
        }
...
////////////////////////////////////////////////////////////////////////
{
    ...
    // Flip vertically - images read from OpenGL buffers are upside-down
    unsigned int rowBytes = vpSize[0] * bpp;
    unsigned int count = vpSize[1] / 2;
    unsigned char* scanline = new unsigned char[rowBytes];
    

    for (unsigned int i = 0; i < count; i++) {
        unsigned char* rowA = pixelBuffer + i * rowBytes;
        unsigned char* rowB = pixelBuffer + (vpSize[1] - i - 1) * rowBytes;
