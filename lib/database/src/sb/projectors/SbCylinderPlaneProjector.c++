...
SbCylinderPlaneProjector::SbCylinderPlaneProjector(float tol, SbBool orient)
    : SbCylinderSectionProjector(tol, orient) {}

SbCylinderPlaneProjector::SbCylinderPlaneProjector(const SbCylinder &c,
                                                   float edgeTol, SbBool orient)
...
SbRotation
SbCylinderPlaneProjector::getRotation(const SbVec3f &p1, SbBool tol1,
...

{
    if (tol1 && tol2) {
        ...
        if (v1.dot(v2) < 0.0) {
            ...
            return getRotation(p1, FALSE, ptOnCylP1Side, FALSE) *
            ...
            float angle =
                (cylinder.getRadius() == 0.0) ? 0 : (d / cylinder.getRadius());
