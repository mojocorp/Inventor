...
class SbImageRef : public SbRefCounted {
  public:
    ...
    SbImageRef(const SbVec3s &size, SbImage::Format format, size_t numBytes,
               const unsigned char *bytes = NULL);

    const SbVec3s &getSize() const;
    bool           setActiveMipmap(int level);
    void setValue(const SbVec3s &size, SbImage::Format format, size_t numBytes,
...
SbImageRef::SbImageRef() : format(SbImage::Format_Invalid), level(0) {
    setActiveMipmap(0);
}
...
SbImageRef::SbImageRef(const SbImageRef *other)
    ...
    setActiveMipmap(0);
    setValue(other->mipmaps[level].size, other->format,
             other->mipmaps[level].bytes.size(),
             &other->mipmaps[level].bytes[0]);
}

SbImageRef::SbImageRef(const SbVec3s &_size, SbImage::Format _format,
                       size_t _numBytes, const unsigned char *_bytes)
    : format(SbImage::Format_Invalid), level(0) {
    setActiveMipmap(0);
...
void
SbImageRef::setValue(const SbVec3s &_size, SbImage::Format _format,
                     size_t _numBytes, const unsigned char *_bytes) {
...
bool
SbImageRef::isNull() const {
    return (mipmaps[level].size[0] == 0 || mipmaps[level].size[1] == 0 ||
            format == SbImage::Format_Invalid);
}

int
SbImageRef::getNumComponents() const {
    switch (format) {
...
size_t
SbImageRef::getNumBytes() const {
    return mipmaps[level].bytes.size();
}

bool
SbImageRef::hasAlphaChannel() const {
    switch (format) {
    case SbImage::Format_Luminance:
        return false;
    case SbImage::Format_Luminance_Alpha:
...
bool
SbImageRef::operator==(const SbImageRef &other) const {
    ...
    for (size_t i = 0; i < mipmaps.size(); i++) {
        ...
        if (memcmp(&mipmaps[i].bytes[0], &other.mipmaps[i].bytes[0],
                   mipmaps[i].bytes.size()) != 0)
            return false;
    }
    return true;
}
...
SbImage::SbImage(const SbImage &other) : d(other.d) {}

SbImage::SbImage(const SbVec2s &size, Format fmt, size_t numBytes,
                 const unsigned char *bytes)
    : d(new SbImageRef(SbVec3s(size[0], size[1], 1), fmt, numBytes, bytes)) {}

SbImage::SbImage(const SbVec3s &size, Format fmt, size_t numBytes,
                 const unsigned char *bytes)
    : d(new SbImageRef(size, fmt, numBytes, bytes)) {}

SbImage::SbImage(const SbString &filename) : d(0) { load(filename); }
...
void
SbImage::setValue(const SbVec2s &size, Format fmt, size_t numBytes,
                  const unsigned char *bytes)
//
////////////////////////////////////////////////////////////////////////
{
...
void
SbImage::setValue(const SbVec3s &size, Format fmt, size_t numBytes,
                  const unsigned char *bytes)
//
////////////////////////////////////////////////////////////////////////
{
