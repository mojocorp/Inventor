...
SoEXTENDER
class INVENTOR_API SoNormalBundle : public SoBundle {
  ...
  public:
    ...
    void beginPolygon() { generator->beginPolygon(); }
    void polygonVertex(const SbVec3f &point) {
        generator->polygonVertex(point);
    }
    void endPolygon() { generator->endPolygon(); }
    ...
    int getNumGeneratedNormals() const { return generator->getNumNormals(); }
