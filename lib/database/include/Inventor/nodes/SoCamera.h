...
class SoCamera : public SoNode {
  ...
  public:
    enum ViewportMapping { // What will happen if the camera and
        ...
        LEAVE_ALONE = 4 ///< Do nothing. Camera image may become stretched out
                        ///< of proportion
    };
    ...
    // Fields
    SoSFEnum viewportMapping; ///< Treatment when aspectRatio not same as
                              ///< viewport's aspectRatio
    SoSFVec3f position;       ///< Location of viewpoint
    SoSFRotation
              orientation; ///< Orientation (rotation with respect to (0,0,-1) vector)
  ...
  private:
    ...
    void computeView(const SbViewportRegion &vpReg, SbViewVolume &viewVol,
                     SbBool &changeRegion);

    // Sets relevant info in elements in state
    void setElements(SoAction *action, SbViewVolume &viewVol, SbBool setRegion,
                     const SbViewportRegion &vpReg, SbBool doJitter,
                     const SbVec3f &jitterAmount);

    // Draws cropping frame when rendering
    void drawFrame(SoGLRenderAction *action, const SbViewportRegion &vpReg,
