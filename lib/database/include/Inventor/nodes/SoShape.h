...
/// All classes derived from <tt>SoShape</tt> draw geometry during render
/// traversal.
...
class SoShape : public SoNode {
  ...
  public:
    ...
    static void getScreenSize(SoState *state, const SbBox3f &boundingBox,
                              SbVec2s &rectSize);
  ...
  protected:
    ...
    // action are called.
    void invokeTriangleCallbacks(SoAction *action, const SoPrimitiveVertex *v1,
                                 const SoPrimitiveVertex *v2,
                                 const SoPrimitiveVertex *v3);
  ...
  private:
    ...
    static std::vector<SoPrimitiveVertex> primVerts; // Array of saved vertices
    static std::vector<SoPointDetail> vertDetails;   // Array of vertex details
    static std::vector<SoPrimitiveVertex>
                                      polyVerts; // Array of saved poly vertices
    static std::vector<SoPointDetail> polyDetails; // Array of poly details
    static struct GLUtesselator *     tobj; // Tesselator (for concave polygons)

    // These methods are used to render primitives generated by
    // subclasses. The userData pointer points to the SoShape instance.
    void GLRenderTriangle(SoGLRenderAction *action, const SoPrimitiveVertex *v1,
                          const SoPrimitiveVertex *v2,
                          const SoPrimitiveVertex *v3);
    void GLRenderLineSegment(SoGLRenderAction *       action,
                             const SoPrimitiveVertex *v1,
                             const SoPrimitiveVertex *v2);
    void GLRenderPoint(SoGLRenderAction *action, const SoPrimitiveVertex *v);

    // These methods are used to pick primitives generated by
    // subclasses. The userData pointer points to the SoShape instance.
    void rayPickTriangle(SoRayPickAction *action, const SoPrimitiveVertex *v1,
                         const SoPrimitiveVertex *v2,
                         const SoPrimitiveVertex *v3);
    void rayPickLineSegment(SoRayPickAction *        action,
                            const SoPrimitiveVertex *v1,
                            const SoPrimitiveVertex *v2);
    void rayPickPoint(SoRayPickAction *action, const SoPrimitiveVertex *v);
