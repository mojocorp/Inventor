...
class INVENTOR_API SoArray : public SoGroup {
  ...
  public:
    ...
    /// Array origin:
    enum Origin {
        FIRST,				///< First copy is rendered at the current local origin;all other copies are distributed relative to it"
        CENTER,				///< Copies are distributed relative to the center of the array
        LAST				///< Last copy is rendered at the current local origin;all other copies are distributed relative to it
    };

    SoSFEnum		origin;		///< Defines the base point from which copies are distributed.
