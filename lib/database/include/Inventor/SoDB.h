...
/// SoXt::init(), so if you are calling any of these methods, you do not need
/// to call SoDB::init() directly. All methods on this class are static.
///
///
...
class INVENTOR_API SoDB {
  ...
  public:
    ...
    /// Reads all graphs and paths from the file specified by the given \c
    /// SoInput.  If there is only one graph in the file and its root is an \c
    /// SoSeparator, a pointer to the root is returned. In all other cases, this
    /// creates an SoSeparator, adds the root nodes of all graphs read as
    /// children of it, and returns a pointer to it. This returns NULL on error.
    /// This processes directory paths in the same way as the other reading
    /// routines.
    ...
    /// The <b>ivVersion</b> number indicates which Inventor file version
    /// this header corresponds to.
    /// The user-defined callback functions <b>preCB</b> and <b>postCB</b> are
    /// called before and after a file with this header is read.
    /// The <b>userData</b> is passed to both callback functions.
    /// The method returns TRUE if the header is successfully registered.
    /// Note, nothing prevents you from registering the
    /// same string multiple times.
    static SbBool registerHeader(const SbString &headerString, SbBool isBinary,
                                 float ivVersion, SoDBHeaderCB *preCB,
                                 SoDBHeaderCB *postCB, void *userData = NULL);
    ...
    /// headers, and the \p substringOK flag is TRUE, then the method
    /// will search for a registered header that is a substring of
    /// the given string.
    /// The method returns TRUE if a matching registered header, or
    /// subheader, was found.
    static SbBool getHeaderData(const SbString &string, SbBool &isBinary,
                                float &ivVersion, SoDBHeaderCB *&preCB,
    ...
    /// is called.  The <b>realTime</b> global field, which is of type
    /// SoSFTime, can be connected to engines and nodes for real-time
    /// animation.  The database will automatically update the <b>realTime</b>
    /// global field 12 times per second, using a timer sensor.  Typically,
    /// there will be a node sensor on the root of the scene graph which
    /// schedules a redraw whenever the scene graph changes; by updating the
    /// <b>realTime</b> global field periodically, scene graphs that are
    /// connected to <b>realTime</b> (and are therefore animating) will be
    /// redrawn.  The rate at which the database updates <b>realTime</b> can be
    /// controlled with this routine.  Passing in a zero time will disable
    /// automatic update of <b>realTime</b>. If there are no enabled connections
    /// from the <b>realTime</b> field to any other field, the sensor is
    ...
    /// Returns how often the database is updating <b>realTime</b>.
