...
public:
    ...
    /// 3 digits of fraction precision.  \a fmt is a character string that consists of field descriptors and
    /// text characters, in a manner analogous to \c cftime (3C) and \c printf (3S).
    ...
    /// are copied from \a fmt into the result.  The following field descriptors are
    /// supported:
    ///
    /// %   the `%' character
    ...
    /// "elapsed\ time:\ %M\ minutes,\ %s\ seconds". The default value of \a fmt,
