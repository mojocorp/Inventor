...
class SoGLLazyElement : public SoLazyElement {
  ...
  public:
    ...
    virtual SbBool     matches(const SoElement *) const { return FALSE; }
    virtual SoElement *copyMatchInfo() const { return NULL; }
    ...
    // provide a public typedef for GLLazyState, so that GLRenderCache can use
    // it:
    typedef struct {
    ...
    void mergeCacheInfo(SoGLRenderCache *childCache,
                        SoGLRenderCache *parentCache, uint32_t doSendFlag,
                        uint32_t checkIVFlag, uint32_t checkGLFlag);
