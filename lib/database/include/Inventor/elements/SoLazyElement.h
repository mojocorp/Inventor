...
class SoLazyElement : public SoElement {
  ...
  public:
    ...
    static void setPacked(SoState *state, SoNode *node, int32_t numColors,
    ...
    int32_t getNumDiffuse() const { return ivState.numDiffuseColors; }
    int32_t getNumTransparencies() const { return ivState.numTransparencies; }
    ...
    SbBool isPacked() const { return ivState.packed; }
    SbBool isTransparent() const {
        return (ivState.packedTransparent || (ivState.numTransparencies > 1) ||
    ...
    static int32_t  getDefaultColorIndex() { return defaultColorIndices; }
  ...
  public:
    ...
    static void setMaterials(SoState *state, SoNode *node, uint32_t bitmask,
                             SoColorPacker *cPacker, const SoMFColor &diffuse,
                             const SoMFFloat &transp, const SoMFColor &ambient,
    ...
    // only be invoked by nodes that use the reallySend method on
    // SoGLLazyElement to establish correct cache dependencies by tracking what
    // was actually sent to GL.
    const uint32_t *getPackedPointer() const { return ivState.packedColors; }

    const SbColor *getDiffusePointer() const { return ivState.diffuseColors; }

    const int32_t *getColorIndexPointer() const { return ivState.colorIndices; }
  ...
  private:
    ...
    virtual void setTranspElt(SoNode *, int32_t numTrans, const float *trans,
...
class SoColorPacker {
  public:
    ...
    SbBool diffuseMatch(uint32_t nodeId) { return (nodeId == diffuseNodeId); }

    SbBool transpMatch(uint32_t nodeId) { return (nodeId == transpNodeId); }
    ...
    size_t getSize() const { return packedColors.size(); }

    void reallocate(size_t size) { packedColors.resize(size); }
    ...
    void packColors(const SbColor *diffuseColors, size_t numDiffuseColors,
        ...
        for (size_t i = 0; i < numDiffuseColors; i++) {
            packedColors[i] = diffuseColors[i].getPackedValue(
                transparencies[multTrans ? i : 0]);
        }
    }
