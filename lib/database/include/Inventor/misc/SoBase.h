...
class SoBase {
  public:
    ...
    /// \endcode
    /// returns TRUE if \a nodePtr is an instance of SoGroup or one of
    /// its subclasses.
    ...
    /// up using the getByName() methods of SoNode, SoEngine, or SoPath.
  ...
  protected:
    ...
    static uint32_t getCurrentWriteCounter() { return currentWriteCounter; }

    // Returns TRUE if the instance has multiple write references
    SbBool hasMultipleWriteRefs() const { return writeStuff.multWriteRef; }
  ...
  public:
    ...
    static void incrementCurrentWriteCounter() { currentWriteCounter++; }

    // Decrements the current write counter after a write operation,
    // in some rare cases
    static void decrementCurrentWriteCounter() { currentWriteCounter--; }
    ...
    // This defaults to "+" and is used when naming nodes that are DEF's and
    // USE'd. The ivdowngrade converter needs to set it to other than "+" since
    // that was an illegal character for Inventor V1.0 names.
