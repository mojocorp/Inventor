...
class SoChildList : public SoNodeList {
  ...
  public:
    ...
    void traverse(SoAction *action) { traverse(action, 0, getLength() - 1); }
  ...
  public:
    ...
    // topology:
    void addPathAuditor(SoPath *p) { auditors.push_back(p); }
    void removePathAuditor(SoPath *p) {
        std::vector<SoPath *>::iterator it =
            std::find(auditors.begin(), auditors.end(), p);
        if (it != auditors.end())
            auditors.erase(it);
    }
