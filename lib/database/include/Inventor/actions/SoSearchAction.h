...
/// action is applied.  The man page for <tt>SoBaseKit</tt> discusses the methods
...
class INVENTOR_API SoSearchAction : public SoAction {
  ...
  public:
    ...
    /// search criterion.
    SoType		getType(SbBool &derivedIsOk) const { derivedIsOk = derivedOk; return type; }
    ...
    /// \c SoSwitch node will traverse only the child or children it would
