...
#define SO__ENGINE_CHECK_INIT(className)                                       \
    if (classTypeId == SoType::badType()) {                                    \
        SoDebugError::post(                                                    \
            ...
            "Can't construct an engine of type " SO__QUOTE(className),         \
...
#define SO__ENGINE_CHECK_CONSTRUCT(where)                                      \
    do {                                                                       \
        if (inputData == NULL) {                                               \
            SoDebugError::post(where, "Instance not properly constructed.\n"   \
                                      "Did you forget to put "                 \
                                      "SO_ENGINE_CONSTRUCTOR()"                \
                                      " in the constructor?");                 \
            inputData =                                                        \
                new SoFieldData(parentInputData ? *parentInputData : NULL);    \
            outputData = new SoEngineOutputData(                               \
                parentOutputData ? *parentOutputData : NULL);                  \
        }                                                                      \
    } while (0)
#else
...
#define SO__ENGINE_ABSTRACT_METHODS(className)                                 \
    ...
    SoType className::getTypeId() const { return classTypeId; }                \
                                                                               \
    const SoFieldData *className::getFieldData() const { return inputData; }   \
                                                                               \
    const SoEngineOutputData *className::getOutputData() const {               \
        return outputData;                                                     \
    }
...
#define SO__ENGINE_INIT_ABSTRACT_CLASS(className, classPrintName, parent)      \
    do {                                                                       \
        classTypeId =                                                          \
            SoType::createType(parent::getClassTypeId(), classPrintName);      \
    ...
    do {                                                                       \
        ...
        if (firstInstance)                                                     \
            outputData->addOutput(this, SO__QUOTE(outputName),                 \
                                  &this->outputName, type::getClassTypeId());  \
...
#define SO_ENGINE_DEFINE_ENUM_VALUE(enumType, enumValue)                       \
    do {                                                                       \
        SO__ENGINE_CHECK_CONSTRUCT(__FILE__);                                  \
        if (firstInstance)                                                     \
            inputData->addEnumValue(SO__QUOTE(enumType), SO__QUOTE(enumValue), \
                                    enumValue);                                \
    } while (0)
    ...
    do {                                                                       \
        if (outputName.isEnabled()) {                                          \
            for (int _eng_out_i = 0;                                           \
                 _eng_out_i < outputName.getNumConnections(); _eng_out_i++) {  \
