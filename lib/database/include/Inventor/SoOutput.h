...
/// \ingroup General
/// This class is used for writing Inventor data files.  It supports ASCII
/// (default), UTF-8 and binary formats and provides some convenience functions
/// for handling files.  It can also write to a buffer in memory as well as to a
/// file pointer. A user-defined header can be specified for the output file.
///
...
class INVENTOR_API SoOutput {
  public:
    ...
    /// Deprecated - Sets whether output should be ASCII (default) or binary.
    /// \sa setFormat
  ...
  public:
    ...
    void incrementIndent(int amount = 1) { indentLevel += amount; }
    void decrementIndent(int amount = 1) { indentLevel -= amount; }
  ...
  public:
    ...
    void     setAnnotation(uint32_t bits) { annotation = bits; }
  ...
  private:
    ...
    SbBool isToBuffer() const { return toBuffer; }

    // Returns number of bytes in current buffer
    size_t bytesInBuf() const { return (curBuf - (char *)buffer); }
