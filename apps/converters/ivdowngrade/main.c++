...
getargs(int argc, char *argv[]) {
    ...
    while ((c = getopt(argc, argv, "hbv:")) != -1)
        switch (c) {
        case 'h':
            // help
            showUsage = 1;
    ...
    // ??? else read from stdin ? ... else err = 1; //??? can't read from stdin
    // right now.
...
// Fills in either the path list or the root node, depending what is in the
// file. If reading a node, we always create an extra group on top. This will
// not be written out later, it just lets us carry all the nodes around.
void
...
main(int argc, char *argv[]) {
    ...
    if (infile != NULL) {
        if (in.openFile(infile)) {
            ...
            if (SoDB::isValidHeader(buf)) {
                // If the filename includes a directory path, add the directory
                // name to the list of directories where to look for input files
                const char *slashPtr;
                ...
                // i.e. for copy/paste. If paths, we need to write paths back
                // out! Call our own routine to read...
                in.openFile(infile);
    ...
    // Based on the target, intitialize any helper classes needed to do the
    // downgrade
    if (targetVersion == 1.0) {
    ...
    if (root != NULL) {
        // Downgrade to version 1.0 happens as a separate process to the
        // SoWriteAction. This is because there are so many changes since 1.0 -
        // we really have to go in and make changes to the scene.
        if (targetVersion == 1.0)
