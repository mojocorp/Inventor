...
#define DICT_ENTER(dict, nodeClass, dictData)                                  \
    (dict->enter(                                                              \
        (unsigned long)nodeClass::getClassTypeId().getName().getString(),      \
        (void *)dictData))

#define DICT_FIND(dict, node, dictData)                                        \
    (dict->find((unsigned long)node->getTypeId().getName().getString(),        \
                dictData))
...
downgradeTexCoordPlane(SoNode *node2) {
    return SoTextureCoordinatePlaneV1::downgrade(
        (SoTextureCoordinatePlane *)node2);
}

SoNode *
downgradeIdxTriStripSet(SoNode *node2) {
    return SoIndexedTriangleMeshV1::downgrade(
        (SoIndexedTriangleStripSet *)node2);
}
