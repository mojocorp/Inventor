...
RevolutionSurface::createSurface(const SoMFVec3f *prof) {
    ...
    for (i = 0; i < nc; i++) {
        ...
        for (j = 0; j < nr; j++) {
            coordPtr[j * nc + i] =
                SbVec3f(profile[j][0] * ca, profile[j][1], profile[j][0] * sa);
        }
        // And normals
        if (i != 0 && i != nc - 1)
            for (j = 0; j < nr; j++) {
                normPtr[j * nc + i] = SbVec3f(norms->vector[j * nc][0] * ca,
                                              norms->vector[j * nc][1],
                                              norms->vector[j * nc][0] * sa);
            }
    }
...
//
RevolutionSurface::~RevolutionSurface() { root->unref(); }
