...
class FieldEditor : public SoXtComponent {
  ...
  private:
    ...
    void getFieldString(FieldInfo *info, SbString &string) const;

    // Fills in SbString with text containing current field value from editor
    void getEditorString(FieldInfo *info, SbString &string) const;
    ...
    void updateLineNumbers(FieldInfo *info, SbBool textChanged) const;
