...
TextWrapper::TextWrapper(const char *texti, int maxLineLength) {
    ...
    if (global_iconvCodeL2 == (iconv_t)-1 ||
        iconv(global_iconvCodeL2, &input, &inbytesleft, &output,
...
void
TextWrapper::WordWrap(Paragraph &p, const UCS2 *text, int nchars,
                      int maxLineLength) {
    int start = 0;
    int end = 0;
    ...
    while (end < nchars) {
        if (start + maxLineLength < nchars) {
            ...
            while (GETCHAR(text[end]) != ' ' && GETCHAR(text[end]) != '\t' &&
                   end > start)
                --end;
        } else
            end = nchars;
    ...
    while (end < nchars) {
        ...
        if (global_iconvCode28 == (iconv_t)-1 ||
            iconv(global_iconvCode28, &input, &inbytesleft, &output,
                  &outbytesleft) == (size_t)-1) {
            fprintf(stderr, "textomatic: iconv error.\n");
            (*(UCS2 *)outbuf) = 0;
        } else {
