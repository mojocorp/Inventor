...
Interface::writeToFile(SbBool asVanilla, char *newFileName) {
    ...
    if (!wa.getOutput()->openFile(fName)) {
        ...
        strcat(str, fName);
        SoXt::createSimpleErrorDialog(mgrWidget, "File Write Error Dialog",
                                      str);
    } else {
...
Interface::fileNewEvent() {
    if (worldInfo->isSceneEmpty() == FALSE) {
        createOkayCancelDialog(
            mgrWidget, Interface::newSceneCB, "New Scene Error Dialog",
...
Interface::fileOpenEvent() {
    if (worldInfo->isSceneEmpty() == FALSE) {
        createOkayCancelDialog(
            mgrWidget, Interface::openSceneCB, "Open Scene Error Dialog",
            "Your current scene is not empty.",
            "Hit OK to continue, CANCEL if you want to Save first.");
...
Interface::fileSaveEvent() {
    if (worldInfo->isSceneEmpty() == TRUE) {
        SoXt::createSimpleErrorDialog(
            mgrWidget, "File Save Empty Error Dialog",
...
Interface::fileSaveAsEvent() {
    if (worldInfo->isSceneEmpty() == TRUE) {
        SoXt::createSimpleErrorDialog(
            mgrWidget, "File Save As Empty Error Dialog",
            "The current scene is empty. Create a scene first,",
            "or fix my code to grey out this menu item.");
    } else
...
Interface::fileSaveVanillaEvent() {
    if (worldInfo->isSceneEmpty() == TRUE) {
        ...
        showFileSelectionDialog(
            (XtCallbackProc)Interface::writeToVanillaFileCB);
}
...
Interface::fileQuitEvent() {
    if (worldInfo->isSceneEmpty() == FALSE) {
        createOkayCancelDialog(
            mgrWidget, Interface::quitProgramCB, "Quit Error Dialog",
            "Your current scene is not empty.",
...
Interface::createOkayCancelDialog(Widget widget, XtCallbackProc okCB,
    ...
    n++;
    XtSetArg(args[n], XtNtitle, dialogTitle);
    n++;
    ...
    XtAddCallback(dialog, XmNokCallback, (XtCallbackProc)okCB, (XtPointer)this);
...
Interface::showAboutDialog() {
    if (access(IVPREFIX "/demos/Inventor/noodle.about", R_OK) != 0) {
        system("xmessage 'Sorry, could not find " IVPREFIX
               "/demos/Inventor/noodle.about' > /dev/null");
        return;
    }
...
Interface::showFileSelectionDialog(XtCallbackProc okCB) {
    ...
    if (fileDialog == NULL) {
        ...
        XtAddCallback(fileDialog, XmNokCallback, (XtCallbackProc)okCB,
                      (XtPointer)this);
    } else {
        ...
        XtAddCallback(fileDialog, XmNokCallback, (XtCallbackProc)okCB, this);
    }
...
Interface::openSceneCB(Widget, void *userData, void *) {
    ((Interface *)userData)
        ->showFileSelectionDialog((XtCallbackProc)Interface::readFromFileCB);
}
...
void
Interface::processTopbarEvent(Widget, NoodleMenuItem *data,
    ...
    case MM_SHAPE_TRI_STRIP:
        if (myNoodle && XmToggleButtonGetState(data->widget)) {
            ...
            myself->setRenderStyleRadioButtons(
                GeneralizedCylinder::TRIANGLE_STRIP_SET);
        }
        break;
    ...
    case MM_SHAPE_CUBIC_SPLINE:
        if (myNoodle && XmToggleButtonGetState(data->widget)) {
            myNoodle->renderShapeType =
                GeneralizedCylinder::CUBIC_SPLINE_SURFACE;
            myself->setRenderStyleRadioButtons(
                GeneralizedCylinder::CUBIC_SPLINE_SURFACE);
        }
        break;
    case MM_SHAPE_CUBIC_TO_EDGE:
        if (myNoodle && XmToggleButtonGetState(data->widget)) {
            myNoodle->renderShapeType =
                GeneralizedCylinder::CUBIC_TO_EDGE_SURFACE;
            myself->setRenderStyleRadioButtons(
                GeneralizedCylinder::CUBIC_TO_EDGE_SURFACE);
        }
        break;
    case MM_SHAPE_BEZIER:
        if (myNoodle && XmToggleButtonGetState(data->widget)) {
            ...
            myself->setRenderStyleRadioButtons(
                GeneralizedCylinder::BEZIER_SURFACE);
        }
        break;
    ...
    case MM_MANIPS_HBOX:
        if (XmToggleButtonGetState(data->widget)) {
            if (myWorld->getManipType() != SoHandleBoxManip::getClassTypeId()) {
                ...
                myself->setManipTypeRadioButtons(
...
void
Interface::setRenderStyleRadioButtons(
    ...
    XmToggleButtonSetState(menuItems[MM_SHAPE_CUBIC_SPLINE].widget, FALSE,
                           FALSE);
    XmToggleButtonSetState(menuItems[MM_SHAPE_CUBIC_TO_EDGE].widget, FALSE,
                           FALSE);
    ...
    for (i = 0; i < itemCount; i++) {
        ...
        for (int j = 0; j < subItemCount; j++) {
            if (pulldownData[i].subMenu[j].buttonType == MM_SEPARATOR) {
                ...
                case MM_SEPARATOR:
                    n = 0;
                    fprintf(stderr, "Programming Error in Interface::build\n");
                    break;
                default:
                    fprintf(stderr, "noodle INTERNAL ERROR: bad buttonType\n");
                    break;
                }
