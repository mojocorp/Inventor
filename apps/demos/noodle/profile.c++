...
createLineGraphCommon(SoPerspectiveCamera *&c, LineManip2 *&theManip,
    ...
    if (theManip != NULL)
        XtRemoveEventHandler(theWidget, LeaveWindowMask, FALSE,
                             leaveLineManipWindow, (XtPointer)theManip);
    ...
    XtAddEventHandler(theWidget, LeaveWindowMask, FALSE, leaveLineManipWindow,
                      (XtPointer)theManip);
...
createProfileGraph(Widget profileWidget, GeneralizedCylinder *surface) {
    ...
    SoSeparator *topNode =
        createLineGraphCommon(c, profileLineManip, profileWidget,
                              "ProfileGrid.iv", myProfileGridBuffer);
...
SoNode *
createCrossSectionGraph(Widget               crossSectionWidget,
                        GeneralizedCylinder *surface) {
    SoPerspectiveCamera *c;

    SoSeparator *topNode =
        createLineGraphCommon(c, crossSectionLineManip, crossSectionWidget,
                              "GridXY.iv", myGridXYBuffer);
...
createSpineGraph(Widget spineWidget, GeneralizedCylinder *surface) {
    ...
    SoSeparator *topNode = createLineGraphCommon(c, spineLineManip, spineWidget,
                                                 "Grid.iv", myGridBuffer);
...
createTwistGraph(Widget twistWidget, GeneralizedCylinder *surface) {
    ...
    SoSeparator *topNode = createLineGraphCommon(c, twistLineManip, twistWidget,
                                                 "Grid.iv", myGridBuffer);
    c->position.setValue(0.0, 0.0, 5.0);
...
makeReverseSection() {
    ...
    for (int first = 0, last = numCoords - 1; first < last; first++, last--) {
