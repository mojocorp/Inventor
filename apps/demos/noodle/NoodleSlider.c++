...
NoodleSlider::NoodleSlider(Widget parent, const char *name,
                           SbBool buildInsideParent, int startingMin,
                           int startingMax)
...
NoodleSlider::~NoodleSlider() { delete valueChangedCallbacks; }

Widget
NoodleSlider::buildWidget(Widget parent, int startingMin, int startingMax) {
    Arg wargs[10];
    int nargs;

    _form = XtCreateManagedWidget(getWidgetName(), xmFormWidgetClass, parent,
                                  NULL, 0);

    _value = XtCreateManagedWidget("value", xmTextWidgetClass, _form, NULL, 0);

    nargs = 0;
    XtSetArg(wargs[nargs], XmNorientation, XmHORIZONTAL);
    nargs++;
    XtSetArg(wargs[nargs], XmNdecimalPoints, 2);
    nargs++;
    XtSetArg(wargs[nargs], XmNminimum, startingMin);
    nargs++;
    XtSetArg(wargs[nargs], XmNmaximum, startingMax);
    nargs++;
    _slider = XtCreateManagedWidget("slider", xmScaleWidgetClass, _form, wargs,
                                    nargs);
...
NoodleSlider::initLayout() {
    ...
    int nargs;
    nargs = 0;
    XtSetArg(wargs[nargs], XmNborderWidth, 2);
    nargs++;
    XtSetValues(_form, wargs, nargs);

    nargs = 0;
    XtSetArg(wargs[nargs], XmNtopAttachment, XmATTACH_FORM);
    nargs++;
    XtSetArg(wargs[nargs], XmNbottomAttachment, XmATTACH_POSITION);
    nargs++;
    XtSetArg(wargs[nargs], XmNbottomPosition, 100);
    nargs++;
    XtSetArg(wargs[nargs], XmNleftAttachment, XmATTACH_POSITION);
    nargs++;
    XtSetArg(wargs[nargs], XmNleftPosition, 0);
    nargs++;
    XtSetArg(wargs[nargs], XmNrightAttachment, XmATTACH_POSITION);
    nargs++;
    XtSetArg(wargs[nargs], XmNrightPosition, 30);
    nargs++;
    XtSetValues(_value, wargs, nargs);

    nargs = 0;
    XtSetArg(wargs[nargs], XmNtopAttachment, XmATTACH_POSITION);
    nargs++;
    XtSetArg(wargs[nargs], XmNtopPosition, 0);
    nargs++;
    XtSetArg(wargs[nargs], XmNbottomAttachment, XmATTACH_FORM);
    nargs++;
    XtSetArg(wargs[nargs], XmNleftAttachment, XmATTACH_POSITION);
    nargs++;
    XtSetArg(wargs[nargs], XmNleftPosition, 30);
    nargs++;
    XtSetArg(wargs[nargs], XmNrightAttachment, XmATTACH_POSITION);
    nargs++;
    XtSetArg(wargs[nargs], XmNrightPosition, 100);
    nargs++;
    XtSetArg(wargs[nargs], XmNtitleString, (void *)NULL);
    nargs++;
...
NoodleSlider::setMin(float newVal) {
    ...
    if (newVal <= getValue() && newVal < getMax())
        valToUse = newVal;
    else
        valToUse = getMin();
...
NoodleSlider::setMax(float newVal) {
    ...
    if (newVal >= getValue() && newVal > getMin())
        valToUse = newVal;
    else
        valToUse = getMax();
