...
NoodleSurfaceGizmo::NoodleSurfaceGizmo(Widget parent, const char *name,
...
Widget
NoodleSurfaceGizmo::buildWidget(Widget parent) {
    Arg     resources[20];
    int     n = 0;
    SbVec2s sliderSize;
...
#define STRING(a) XmStringCreate(a, XmSTRING_DEFAULT_CHARSET)
    ...
    // Flip Normals button
    XtSetArg(resources[n], XmNlabelString, STRING("Flip Normals"));
    ++n;
    flipNormButton = XmCreatePushButtonGadget(_rowCol, "flip", resources, n);
    n = 0;
    ...
    // Crease Angle.
    // Label...
    XtSetArg(resources[n], XmNlabelString, STRING("Crease Angle:"));
    ++n;
    Widget creaseLabel =
        XmCreateLabelGadget(_rowCol, "Crease Angle", resources, n);
    n = 0;
    XtManageChild(creaseLabel);
    // Slider...
    creaseSlider = new NoodleSlider(_rowCol);
    creaseSlider->addValueChangedCallback(NoodleSurfaceGizmo::creaseCallback,
                                          this);
    ...
    // Min Num rows label
    XtSetArg(resources[n], XmNlabelString, STRING("Min num rows:"));
    ++n;
    Widget numRowsLabel =
        XmCreateLabelGadget(_rowCol, "Min num rows", resources, n);
    n = 0;
    ...
    ++n;
    minNumRowsEdit = XmCreateText(_rowCol, "minNumRowsEdit", resources, n);
    n = 0;
    ...
    // Min Num cols label
    XtSetArg(resources[n], XmNlabelString, STRING("Min num cols:"));
    ++n;
    Widget numColsLabel =
        XmCreateLabelGadget(_rowCol, "Min num cols", resources, n);
    n = 0;
    ...
    XtSetArg(resources[n], XmNeditMode, XmSINGLE_LINE_EDIT);
    ++n;
    minNumColsEdit = XmCreateText(_rowCol, "minNumColsEdit", resources, n);
    n = 0;
...
void
NoodleSurfaceGizmo::minNumRowsCallback(Widget textWidget, XtPointer data,
                                       XtPointer) {
    NoodleSurfaceGizmo * myself = (NoodleSurfaceGizmo *)data;
    GeneralizedCylinder *s = myself->myNoodle;
...
void
NoodleSurfaceGizmo::minNumColsCallback(Widget textWidget, XtPointer data,
                                       XtPointer) {
    NoodleSurfaceGizmo * myself = (NoodleSurfaceGizmo *)data;
    GeneralizedCylinder *s = myself->myNoodle;
