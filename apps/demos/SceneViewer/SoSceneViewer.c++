...
SoSceneViewer::SoSceneViewer(Widget parent, const char *name,
                             SbBool buildInsideParent, SoSelection *inputGraph,
                             const char *envFile)
...
//
void
SoSceneViewer::constructorCommon(SoSelection *inputGraph, const char *envFile,
                                 SbBool buildNow)
//
////////////////////////////////////////////////////////////////////////
{
    ...
    XtGetSubresources(getParentWidget(), &fileData[0], "intl", "Intl",
                      intl_file_resources, intl_num_file_resources, NULL, 0);
    XtGetSubresources(getParentWidget(), &editData[0], "intl", "Intl",
                      intl_edit_resources, intl_num_edit_resources, NULL, 0);
    XtGetSubresources(getParentWidget(), &manipData[0], "intl", "Intl",
                      intl_manip_resources, intl_num_manip_resources, NULL, 0);
    XtGetSubresources(getParentWidget(), &selData[0], "intl", "Intl",
                      intl_sel_resources, intl_num_sel_resources, NULL, 0);
    XtGetSubresources(getParentWidget(), &viewData[0], "intl", "Intl",
                      intl_view_resources, intl_num_view_resources, NULL, 0);
    XtGetSubresources(getParentWidget(), &editorData[0], "intl", "Intl",
                      intl_editor_resources, intl_num_editor_resources, NULL,
                      0);
    XtGetSubresources(getParentWidget(), &lightData[0], "intl", "Intl",
                      intl_light_resources, intl_num_light_resources, NULL, 0);
    ...
    // NOTE: since the camera may be switched by the viewer (ortho/perspective
    // toggle) make sure to get the camera from the viewer (and not cache the
    // camera).
    ...
    //     the current selection changes (e.g. attach/detach editors and
    //     manips).
    //
...
void
SoSceneViewer::replaceAllManips(SvEManipMode manipMode) // Current manipulator
//
////////////////////////////////////////////////////////////////////////
{
    detachManipFromAll();
...
void
SoSceneViewer::attachManipToAll(SvEManipMode manipMode) // Current manipulator
//
////////////////////////////////////////////////////////////////////////
{
    int i;
...
void
SoSceneViewer::attachManip(SvEManipMode manipMode, // Current manipulator
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (theXfManip) {
        ...
        if (!fp->getTail()->isOfType(SoTransform::getClassTypeId())) {
            fprintf(stderr,
                    "DBG> Fatal Error: in SoSceneViewer::attachManip\n");
...
void
SoSceneViewer::detachManip(SoPath *p) // Selection object that is being removed
//
////////////////////////////////////////////////////////////////////////
{
    //
...
//            We stop the search if we come to a material node to the left of
//            the
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if ((selectionPath = (SoPath *)target) == NULL) {
        ...
        //
        selectionPath =
            (*selection)[selection->getNumSelected() - 1]; // last guy
    }
    ...
    if ((editMtl == NULL) && (!isTailGroup)) {
        ...
        for (i = index; (i >= 0) && (editMtl == NULL); i--) {
            ...
            if (node->isOfType(
                    SoMaterial::getClassTypeId())) // found SoMaterial
    ...
    if (editMtl->ambientColor.isIgnored() ||
        ...
        editMtl->emissiveColor.isIgnored() || editMtl->shininess.isIgnored() ||
        editMtl->transparency.isIgnored()) {

        // Create a path to the material
        SoPath *mtlPath;
        if ((!ignoreNodekit) &&
            selectionPath->getTail()->isOfType(SoBaseKit::getClassTypeId())) {
            SoBaseKit *kit =
                (SoBaseKit *)((SoNodeKitPath *)selectionPath)->getTail();
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (isTailKit) {
        ...
        if ((editXform = SO_GET_PART(kit, "transform", SoTransform)) != NULL) {
            pathToXform =
                kit->createPathToPart("transform", TRUE, selectionPath);
            pathToXform->ref();
        } else {
    ...
    if (!isTailGroup && !isTailKit) {
        ...
        for (i = index; (i >= 0) && (editXform == NULL); i--) {
            node = group->getChild(i);
            if (node->isOfType(
                    SoTransform::getClassTypeId())) // found an SoMaterial
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if ((ok = SoDB::read(&in, n)) && n != NULL) {
        ...
        if (n->isOfType(SoLabel::getClassTypeId())) {
            l = (SoLabel *)n;
            isValid =
                (strcmp(l->label.getValue().getString(), SV_ENV_LABEL) == 0);
        }
        n->unref();
    } else if (!ok) {
        // display error dialog if there were reading errors
...
////////////////////////////////////////////////////////////////////////
{
    ...
    case SV_VIEW_USER:
        ...
        if (sv->userModeFlag)
            sv->currentViewer->setEventCallback(sv->userModeCB,
                                                sv->userModedata);
        else
    ...
    SbVec3f      center = vwrCamera->position.getValue() +
                     forward *
                         (vwrCamera->nearDistance.getValue() +
                          vwrCamera->farDistance.getValue()) /
                         2.0;
...
////////////////////////////////////////////////////////////////////////
{
    ...
    data->cascadeWidget =
        XtCreateWidget(data->name, xmCascadeButtonGadgetClass, menu, args, 1);
    ...
    XtAddCallback(data->onOffWidget, XmNvalueChangedCallback,
                  (XtCallbackProc)SoSceneViewer::lightToggleCB,
                  (XtPointer)data);
    ...
    data->editColorWidget = XtCreateWidget(
        "Edit Color", xmPushButtonGadgetClass, data->submenuWidget, NULL, 0);
    XtAddCallback(data->editColorWidget, XmNactivateCallback,
                  (XtCallbackProc)SoSceneViewer::editLightColorCB,
                  (XtPointer)data);
    ...
    XtAddCallback(data->removeWidget, XmNactivateCallback,
                  (XtCallbackProc)SoSceneViewer::removeLightCB,
                  (XtPointer)data);
        ...
        if (forHeadlight) {
            ...
            if (path != NULL) {
                ...
                if (dataType.isDerivedFrom(ptLightType)) {
                    ...
                    newLight->ref();
                    ((SoPointLightManip *)data->light)
                        ->replaceManip(path, (SoPointLight *)newLight);
                } else if (dataType.isDerivedFrom(dirLightType)) {
                    ...
                    ((SoDirectionalLightManip *)data->light)
                        ->replaceManip(path, (SoDirectionalLight *)newLight);
                } else if (dataType.isDerivedFrom(spotLightType)) {
                    newLight = new SoSpotLight;
                    newLight->ref();
                    ((SoSpotLightManip *)data->light)
                        ->replaceManip(path, (SoSpotLight *)newLight);
                }
                path->unref();
                data->light->unref();
                data->light = newLight;
    ...
    for (int i = 0; i < lightDataList.getLength(); i++) {
        ...
        if (data->scale != NULL) {
            if (data->scale->scaleFactor.getValue()[0] != lightManipSize)
                data->scale->scaleFactor.setValue(
                    lightManipSize, lightManipSize, lightManipSize);
        }
        if (data->scaleInverse != NULL) {
            float invSz = (lightManipSize == 0.0) ? 1.0 : 1.0 / lightManipSize;
...
//
SbBool
SoSceneViewer::objDropCB(void *userData, Atom target, void *data,
                         uint32_t numBytes)
//
////////////////////////////////////////////////////////////////////////
{
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (vwr->getWidget() == NULL) {
        fprintf(stderr, "ERROR - need to create the viewer widget before it "
                        "can be layed out\n");
        return;
    }
...
////////////////////////////////////////////////////////////////////////
{
    ...
    SoXt::getPopupArgs(XtDisplay(menuWidget), SCREEN(shell), popupargs,
                       &popupn);
#endif
    ...
    for (i = 0; i < itemCount; i++) {
        ...
        buttons[i] =
            XtCreateWidget(pulldownData[i].name, xmCascadeButtonGadgetClass,
                           menuWidget, args, 1);
        ...
        for (j = 0; j < subItemCount; j++) {
            if (pulldownData[i].subMenu[j].buttonType == SV_SEPARATOR)
                ...
                if (accel != NULL) {
                    ...
                    if (accelText != NULL) {
                        xmstr =
                            XmStringCreate(accelText, XmSTRING_DEFAULT_CHARSET);
    ...
    case SV_LIGHT:
        ...
        // update the lights label (show on/off with '*')
        {
            for (int i = 0; i < sv->lightDataList.getLength(); i++) {
                ...
                IS_LIGHT_ON(data->lightSwitch) ? strcpy(str, "* ")
                                               : strcpy(str, "  ");
...
//
SbBool
SoSceneViewer::isAffectedByTransform(SoNode *theNode) // node to be affected?
//
////////////////////////////////////////////////////////////////////////
{
...
//
SbBool
SoSceneViewer::isAffectedByMaterial(SoNode *theNode) // node to be affected?
//
////////////////////////////////////////////////////////////////////////
{
    if (theNode->isOfType(SoGroup::getClassTypeId()) ||
...
////////////////////////////////////////////////////////////////////////
{
    if (access(IVPREFIX "/demos/Inventor/SceneViewer.about", R_OK) != 0) {
        system("xmessage 'Sorry, could not find " IVPREFIX
               "/demos/Inventor/SceneViewer.about' > /dev/null");
        return;
    }
    ...
    sprintf(command,
            "acroread " IVPREFIX "/demos/Inventor/SceneViewer.about &");
    system(command);
}
