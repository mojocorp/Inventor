...
enum SvEViewer { SV_VWR_EXAMINER = 0, SV_VWR_FLY, SV_VWR_WALK, SV_VWR_PLANE };
...
class SoSceneViewer : public SoXtComponent {
  ...
  public:
    ...
    //
    SoSceneViewer(Widget parent = NULL, const char *name = NULL,
                  SbBool buildInsideParent = TRUE, SoSelection *root = NULL,
  ...
  protected:
    ...
    SoEXTENDER
    SoSceneViewer(Widget parent, const char *name, SbBool buildInsideParent,
                  SoSelection *root, const char *envFile, SbBool buildNow);
  ...
  private:
    ...
    static SbBool objDropCB(void *userData, Atom dataType, void *data,
                            uint32_t numBytes);
    ...
    findMtlPreTailCB(void *data, SoCallbackAction *accum, const SoNode *);
    ...
    void constructorCommon(SoSelection *root, const char *envFile,
                           SbBool buildNow);
};
