...
selectionPickCB(void *userData, const SoPickedPoint *pick) {
    ...
    if (selIndex == -1) {
        fprintf(stderr, "SceneViewer selectionPickCB - did not find sel node "
                        "in pick path!\n");
        return NULL;
    }
...
main(int argc, char **argv) {
    ...
    if (doRead) {
        ...
        // If reading from a file, call MyFileRead in libInventorWidgets sample
        // code. It will do an SoDB::readAll on Inventor files. On non-Inventor
        // files, it will use FTR rules to attempt file conversion to the
        // Inventor format before reading.
        SbString errmsg;
        if (readFromFile)
