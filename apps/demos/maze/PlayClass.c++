...
PlayClass::attenuatesamps(void *sampbuf, long sampsize, long nsamps,
                          double secs_per_buf, double max_level,
                          double *fade_level, double fade_time, int mode) {

    int i;

#ifdef DEBUG
    fprintf(stderr,
            "attenuating max_level = %f, fade_level = %f, fade_time = %f\n",
    ...
    for (i = 0; i < nsamps; i++) {
        ...
        if (mode != 2) {
            if (fade_time == 0.0) {
                ...
                *fade_level += (mode == 0 ? 1.0 : -1.0) * secs_per_buf /
                               (fade_time * nsamps);
...
PlayClass::init_audio(AFfilehandle audio_file, ALport *audio_port,
    ...
    case 44100:
        ...
        break;
    case 32000:
        audio_rate = AL_RATE_32000;
        frames_per_sec = 32000;
        break;
    case 22050:
        audio_rate = AL_RATE_22050;
        frames_per_sec = 22050;
        break;
    case 16000:
        audio_rate = AL_RATE_16000;
        frames_per_sec = 16000;
        break;
    case 11025:
        audio_rate = AL_RATE_11025;
        frames_per_sec = 11025;
        break;
    case 8000:
        audio_rate = AL_RATE_8000;
        frames_per_sec = 8000;
        break;
    default:
        if (!quiet) {
    ...
    if (pvbuf[1] != audio_rate) {
        ...
        } else /*polite*/
        {
            ...
            if (!quiet) {
                ...
                fprintf(stderr,
                        "%s: '%s': adjust system output rate (currently %6.1f "
                        "Hz)\n",
...
PlayClass::play_audio_samps(AFfilehandle audio_file, ALport audio_port,
    ...
    if (verbose) {
        printf("%s: '%s' %6.3f sec %7.1f Hz %6s %s %d-bit %s\n", myname,
               filename, fileplayingtime, file_rate,
               samps_per_frame == 1 ? "mono" : "stereo", compressionname,
               bits_per_samp, filefmt == AF_FILE_AIFFC ? "AIFF-C" : "AIFF");
...
top:
    ...
    for (i = 0; i < num_bufs; i++) {
        ...
        if ((frames_read = (int)AFreadframes(audio_file, AF_DEFAULT_TRACK,
                                             sampbuf, frames_per_buf)) <
            frames_per_buf) {
            if (!quiet) {
                fprintf(stderr,
                        "%s: warning short read for %s: expected %d frames, "
                        "got %d frames\n",
        ...
        if (!timed && !loop &&
            (sec_count > (double)fileplayingtime - out_time)) {
#ifdef DEBUG
    ...
    if ((frames_read = (int)AFreadframes(audio_file, AF_DEFAULT_TRACK, sampbuf,
                                         leftover_frames)) < leftover_frames) {
        if (!quiet) {
            fprintf(stderr,
                    "%s: warning short read for %s: expected %d frames, got %d "
                    "frames\n",
...
void
PlayClass::setMaxOutputLevel(double lvl) {
    max_level = lvl;
}
