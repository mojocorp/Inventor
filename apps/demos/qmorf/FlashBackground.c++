...
void
FlashBackground::calculateShape(Shape shape, SbVec3f *coords) {
...
void
FlashBackground::calculateColor(Shade shade, Scheme cscheme, SbColor *clrs) {
    for (int i = 0; i < meshSize + 1; i++) {
        double y = (double)i / (double)(meshSize);
...
FlashBackground::updateSceneColor(double t) {
    ...
    for (int i = 0; i < meshSize + 1; i++) {
        for (int j = 0; j < meshSize + 1; j++) {
            ...
            SbVec3f workAround = (1.0 - t) * schemeTable[currentScheme][index] +
