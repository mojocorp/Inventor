...
FaceList::FaceList(const SbVec3f *v, SoIndexedFaceSet *fs, SbBool vrb) {
    ...
    if (n_degenerate_faces != 0) {
        if (verbose)
            fprintf(stderr, "Detected %d degenerate faces\n",
                    n_degenerate_faces);
    }
    if (n_degenerate_edges != 0) {
        if (verbose)
            fprintf(stderr, "Detected %d degenerate edges\n",
                    n_degenerate_edges);
    }
}
    ...
    for (i = 0; i < f->nv; i++) {
        ...
        for (int j = 0; j < faces.getLength(); j++) {
            if (faces[j]->body == f->body) {
                ; // nothing to do if the other face is already part of this
                  // body
...
void
FaceList::extractBody(int b, int &bnf, int32_t *fNewFromOld,
                      int32_t *fOldFromNew, int &bnv, int32_t *vNewFromOld,
                      int32_t *vOldFromNew) {
    // build the face maps
    int f, bf;
    ...
    for (i = 0; i < getLength(); i++) {
        ...
        for (int j = 0; j < f->nv; j++) {
            if (f->degenerate) {
                ...
                SbVec3f t;
                vd[f->v[j]].averageNormals(norm->vector, f->normal, creaseAngle,
                                           f->v[j]);
            }
