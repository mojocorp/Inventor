...
static void
countLinesCB(void *userData, SoCallbackAction *, const SoPrimitiveVertex *,
...
static void
countPointsCB(void *userData, SoCallbackAction *, const SoPrimitiveVertex *)
//
////////////////////////////////////////////////////////////////////////
{
    int32_t *curCount = (int32_t *)userData;
...
countPrimitives(SoNode *root, int32_t &numTris, int32_t &numLines,
                int32_t &numPoints, int32_t &numNodes)
//
////////////////////////////////////////////////////////////////////////
{
    numTris = 0;
    ...
    ca.addPointCallback(SoShape::getClassTypeId(), countPointsCB,
                        (void *)&numPoints);
    ca.addLineSegmentCallback(SoShape::getClassTypeId(), countLinesCB,
                              (void *)&numLines);
    ca.addTriangleCallback(SoShape::getClassTypeId(), countTriangleCB,
                           (void *)&numTris);
    ca.addPreCallback(SoNode::getClassTypeId(), countNodesCB,
                      (void *)&numNodes);
    ca.apply(root);
}
...
//////////////////////////////////////////////////////////////
{
    ...
    if (ok) {
        printf("\n");
        system("hinv -c processor");
        printf("\n");
        system("hinv -t memory");
        printf("\n");
        system("hinv -c graphics");
        printf("\n");
...
//////////////////////////////////////////////////////////////
{
    ...
    window =
        XCreateWindow(display, RootWindow(display, vi->screen), 10, 10, windowX,
                      windowY, 0, vi->depth, InputOutput, vi->visual,
...
static SoSeparator *
setUpGraph(const SbViewportRegion &vpReg, SoInput *sceneInput, Options &options)
//
//////////////////////////////////////////////////////////////
{
...
//////////////////////////////////////////////////////////////
{
    fprintf(stderr, "Usage: %s [-b] [-f N] [-w X,Y] datafile\n", progName);
    fprintf(stderr, "\t-b      display results as bar chart\n"
...
static float
timeRendering(Options &options, const SbViewportRegion &vpr, SoSeparator *&root)
//
//////////////////////////////////////////////////////////////
{
    ...
    if (options.noTextures || options.oneTexture) { // override texture node
        ...
        if (options.oneTexture) {
            ...
            static unsigned char img[] = {255, 255, 0,   0, 255, 255, 0,  0, 0,
    ...
    for (frameIndex = 0;; frameIndex++) {
        ...
        sceneTransform->rotation.setValue(
            SbVec3f(1, 1, 1), frameIndex * 2 * M_PI / options.numFrames);
...
static void
drawBar(float asisTime, float noClearTime, float noMatTime, float noXformTime,
        float noTexTime, float oneTexTime, float noLitTime, float ptsTime,
        float invisTime, float freezeTime)
//
//////////////////////////////////////////////////////////////
{
    float              barValues[10];
    static const char *barXLabels[] = {"Total", "Clear", "Trav",  "Mat",
                                       "Xform", "Tex",   "TMgmt", "Lit",
                                       "Pipe",  "Fill"};
...
//////////////////////////////////////////////////////////////
{
    ...
    Display *    display;
    Window       window;
    float        asisTime, ptsTime, invisTime, freezeTime, noMatTime, noTexTime,
        noLitTime, noXformTime, noClearTime, oneTexTime;
    ...
    // draw timing bars
    if (options.showBars)
        drawBar(asisTime, noClearTime, noMatTime, noXformTime, noTexTime,
                oneTexTime, noLitTime, ptsTime, invisTime, freezeTime);
