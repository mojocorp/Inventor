...
IfStripper::IfStripper() { holder = NULL; }
...
IfStripper::strip(IfHolder *_holder) {
    ...
    haveNormals =
        (holder->doNormals && holder->stripSet->normalIndex.getNum() > 1);
...
IfStripper::createVertexList() {
    ...
    const int32_t *normalIndices =
        (!haveNormals ? NULL : holder->stripSet->normalIndex.getValues(0));
    const int32_t *texCoordIndices =
        (!haveTexCoords ? NULL
                        : holder->stripSet->textureCoordIndex.getValues(0));
    const int32_t *mtlIndices =
        (!haveMaterials ? NULL : holder->stripSet->materialIndex.getValues(0));
...
IfStripper::addEdge(StripEdge *newEdge) {
    ...
    if (edgeDict->find(key, listPtr)) {
        ...
        for (StripEdge *edge = (StripEdge *)listPtr; edge != NULL;
             edge = edge->next) {
