...
static char hourglass_bits[] = {
    0x00, 0x00, 0x00, 0xfe, 0xff, 0x00, 0x04, 0x40, 0x00, 0x04, 0x40,
    0x00, 0x04, 0x40, 0x00, 0xe8, 0x2e, 0x00, 0xd0, 0x17, 0x00, 0xa0,
    0x0b, 0x00, 0x40, 0x05, 0x00, 0x40, 0x05, 0x00, 0x40, 0x04, 0x00,
    0x40, 0x04, 0x00, 0x40, 0x04, 0x00, 0x20, 0x09, 0x00, 0x10, 0x11,
    0x00, 0x88, 0x23, 0x00, 0xc4, 0x47, 0x00, 0xe4, 0x4f, 0x00, 0xf4,
    0x5f, 0x00, 0xfe, 0xff, 0x00, 0x00, 0x00, 0x00};

static char hourglass_mask_bits[] = {
    0xff, 0xff, 0x01, 0xff, 0xff, 0x01, 0xfe, 0xff, 0x00, 0xfe, 0xff,
    0x00, 0xfe, 0xff, 0x00, 0xfc, 0x7f, 0x00, 0xf8, 0x3f, 0x00, 0xf0,
    0x1f, 0x00, 0xe0, 0x0f, 0x00, 0xe0, 0x0f, 0x00, 0xe0, 0x0f, 0x00,
    0xe0, 0x0f, 0x00, 0xe0, 0x0f, 0x00, 0xf0, 0x1f, 0x00, 0xf8, 0x3f,
    0x00, 0xfc, 0x7f, 0x00, 0xfe, 0xff, 0x00, 0xfe, 0xff, 0x00, 0xfe,
    0xff, 0x00, 0xff, 0xff, 0x01, 0xff, 0xff, 0x01};
...
print_usage(const char *progname) {
    ...
    fprintf(stderr, "\t-h : Print this message (help) and exit\n"
            ...
            do {
                ...
                if (files.count == 1 && strcmp(files.names[0], "-") == 0) {
                    ...
                    for (int i = 0; i < files.count; i++) {
                        ...
                        char *      ivfixCommand = (char *)alloca(
                            strlen("ivfix ") + strlen(filename) + 1);
...
initPerfMeter(SoXtViewer *vwr) {
    ...
    SoTimerSensor *perfSensor = new SoTimerSensor(perfSensorCB, perfMeterText);
...
createBusyCursor(Display *display) {
    ...
    Pixmap mask = XCreateBitmapFromData(display, d, hourglass_mask_bits,
                                        hourglass_width, hourglass_height);

    busyCursor =
        XCreatePixmapCursor(display, source, mask, &foreground, &background,
...
showAboutDialog() {
    if (access(IVPREFIX "/share/help/ivview/ivview.about", R_OK) != 0) {
        system("xmessage 'Sorry, could not find " IVPREFIX
               "/share/help/ivview/ivview.about' > /dev/null");
        return;
    }
...
getNewScene() {
    ...
    if (fileDialog == NULL) {
        ...
        n++;
        fileDialog = XmCreateFileSelectionDialog(shell, "File Dialog", args, n);

        XtAddCallback(fileDialog, XmNokCallback, (XtCallbackProc)newSceneCB,
                      NULL);
    }
...
editBackgroundColor() {
    ...
    if (backgroundColorEditor == NULL) {
        ...
        backgroundColorEditor->addColorChangedCallback(backgroundColorCB);
    }
...
buildAndLayoutMenus(Widget parent) {
    ...
    SoXt::getPopupArgs(XtDisplay(menuWidget), SCREEN(menuWidget), popupargs,
                       &popupn);
#endif
        ...
        for (int j = 0; j < subItemCount; j++) {
            if (pulldownData[i].subMenu[j].buttonType == IV_SEPARATOR) {
                ...
                case IV_RADIO_BUTTON:
                    ...
                    break;
                default:
                    fprintf(stderr, "ivview INTERNAL ERROR: bad buttonType\n");
                    break;
                }
...
static void
countTriangleCB(void *userData, SoCallbackAction *, const SoPrimitiveVertex *,
...
main(int argc, char **argv) {
    ...
    shell = XtAppInitialize(&appContext,        // context will be returned
    ...
    XtGetSubresources(shell, &fileData[0], "intl", "Intl", intl_file_resources,
                      intl_num_file_resources, NULL, 0);
    XtGetSubresources(shell, &editData[0], "intl", "Intl", intl_edit_resources,
                      intl_num_edit_resources, NULL, 0);
