...
NurbMaker::createNurbsGroup(SoQuadMesh *quadNode, SoCoordinate3 *coordNode) {
    ...
    if (coordNode != NULL && myWrap[0] == TRUE) {
        ...
        for (int topInd = 0, botInd = vPerRow * (vPerCol - 1); topInd < vPerRow;
             topInd++, botInd++) {
    ...
    for (int row = 0; row <= lastRowToDo; row += myShift[0]) {
        for (int col = 0; col <= lastColToDo; col += myShift[1]) {
            ...
            if (patchType != CUBIC_TO_EDGE) {
                ...
                applyCubicToEdgeKnotVectors(row, col, myNurb, lastRowToDo,
                                            lastColToDo);
            }
...
void
NurbMaker::setUserKnots(SbVec2s newNumKnots, float *newUKnots,
                        float *newVKnots) {
...
void
NurbMaker::getUserKnots(SbVec2s &numKnots, float *UKnots, float *VKnots) {
...
NurbMaker::applyCubicToEdgeKnotVectors(int row, int col,
                                       SoIndexedNurbsSurface *myNurb,
                                       int lastRowToDo, int lastColToDo) {
    float *uKs, *vKs;

    if (myWrap[1])
        uKs = myUKnots;
    else if (col == 0)
        uKs = cubic0FromEarlyEdgeKnots;
    else if (col == lastColToDo)
        uKs = cubic0FromLateEdgeKnots;
    else if (col == 1)
        uKs = cubic1FromEarlyEdgeKnots;
    else if (col == lastColToDo - 1)
        uKs = cubic1FromLateEdgeKnots;
    else if (col == 2)
        uKs = cubic2FromEarlyEdgeKnots;
    else if (col == lastColToDo - 2)
        uKs = cubic2FromLateEdgeKnots;
    else
        uKs = myUKnots;
    ...
    if (myWrap[0])
        ...
        vKs = cubic0FromEarlyEdgeKnots;
    else if (row == lastRowToDo)
        vKs = cubic0FromLateEdgeKnots;
    else if (row == 1)
        vKs = cubic1FromEarlyEdgeKnots;
    else if (row == lastRowToDo - 1)
        vKs = cubic1FromLateEdgeKnots;
    else if (row == 2)
        vKs = cubic2FromEarlyEdgeKnots;
    else if (row == lastRowToDo - 2)
        vKs = cubic2FromLateEdgeKnots;
    else
        vKs = myVKnots;
