...
SbBool
Triangulator::triangEdgeTest(const SoMFVec3f &coords, int32_t e0p0,
                             int32_t e0p1, int32_t e1p0, int32_t e1p1) {
...
SbBool
Triangulator::clockWiseTest(const SoMFVec3f &coords, const SoMFInt32 &indices,
                            int startVert, int numVerts) {
...
SbBool
Triangulator::triangInsideTest(const SoMFVec3f &coords, int32_t testPt,
                               int32_t pt0, int32_t pt1, int32_t pt2) {
...
SbBool
Triangulator::triangulate(const SoMFVec3f &coords,
                          const SoMFInt32 &inVals, // indices describing the
                                                   // input polygons
...
#define TEENY_VAL 0.0000001
#define IS_COORD_TEENY(a)                                                      \
    ((a)[0] < TEENY_VAL && (a)[1] < TEENY_VAL && (a)[2] < TEENY_VAL &&         \
     (a)[0] > -TEENY_VAL && (a)[1] > -TEENY_VAL && (a)[2] > -TEENY_VAL)
...
#undef IS_COORD_TEENY
        ...
        if (curNumVerts == 0) {
            ...
            if (!clockWiseTest(coords, outVals, curInd0, curNumVerts)) {
                ...
                for (early = 0, late = curNumVerts - 1; early < late;
                     early++, late--) {
                ...
                if (tri_ok == TRUE) {
                    for (vNum = curInd0; vNum < curInd0 + curNumVerts; vNum++) {
                        if (vNum != ind0 && vNum != ind1 && vNum != ind2) {
                            if (triangInsideTest(coords, outVals[vNum],
                                                 outVals[ind0], outVals[ind1],
                                                 outVals[ind2]))
                                tri_ok = FALSE;
                        }
                    }
                ...
                if (ind1 != curInd0 + 1) {
                    ...
                    for (test_ind0 = curInd0, test_ind1 = curInd0 + 1;
                        ...
                        if (test_ind1 == curInd0 + curNumVerts)
                            test_ind1 = curInd0;
                        tri_ok = triangEdgeTest(
                            coords, outVals[ind0], outVals[ind1],
                ...
                if (ind2 != ind1 + 1) {
                    for (test_ind0 = curInd0, test_ind1 = curInd0 + 1;
                        ...
                        if (test_ind1 == curInd0 + curNumVerts)
                            test_ind1 = curInd0;
                        tri_ok = triangEdgeTest(
                            coords, outVals[ind1], outVals[ind2],
                            outVals[test_ind0], outVals[test_ind1]);
                    }
                }
                ...
                if (ind2 != curInd0 + curNumVerts - 1) {
                    ...
                    for (test_ind0 = curInd0, test_ind1 = curInd0 + 1;
                        ...
                        if (test_ind1 == curInd0 + curNumVerts)
                            test_ind1 = curInd0;
                        tri_ok = triangEdgeTest(
                            coords, outVals[ind2], outVals[ind0],
                            outVals[test_ind0], outVals[test_ind1]);
                    }
                }
                if (tri_ok == TRUE)
