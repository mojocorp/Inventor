...
class NurbMaker {
  ...
  public:
    ...
    // and BEZIER to 3.
    void           setPatchShift(SbVec2s newShift) { myShift = newShift; }
    const SbVec2s &getPatchShift() { return myShift; }

    // Default is not to use the user knots, but standard ones for
    // BEZIER, CUBIC, or CUBIC_TO_EDGE
    void setUserKnots(SbVec2s newNumKnots, float *newUKnots = NULL,
                      float *newVKnots = NULL);
