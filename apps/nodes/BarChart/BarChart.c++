...
BarChart::generateChildren() {
    ...
    */
    ...
    children->append(sep);
    mtl = new SoMaterial();
    mtl->diffuseColor.setValue(poleColor.getValue()[0], poleColor.getValue()[1],
    ...
    case BAR:
        generateBars(barXSpace, barYSpace, barWidth, barDepth);
        break;
    }
    ...
    for (i = 0; i < xLabels.getNum() && i < xDimension.getValue(); ++i) {
        ...
        mtl->diffuseColor.setValue(xLabelColors[c][0], xLabelColors[c][1],
                                   xLabelColors[c][2]);
    ...
    for (i = 0; i < yLabels.getNum() && i < yDimension.getValue(); ++i) {
        ...
        mtl->diffuseColor.setValue(yLabelColors[c][0], yLabelColors[c][1],
                                   yLabelColors[c][2]);
    ...
    for (val = minValue.getValue() + zLabelIncrement.getValue();
         val <= maxValue.getValue(); val += zLabelIncrement.getValue()) {
        tr = new SoTranslation();
    ...
    for (val = minValue.getValue(); val <= maxValue.getValue();
        ...
        tr->translation.setValue(
            0, (1.8 / nzLabels) / (zLabelScale.getValue()[0] * 0.01), 0);
        sep->addChild(tr);
    }
}

void
BarChart::generateBars(float barXSpace, float barYSpace, float barWidth,
                       float barDepth) {
    ...
    if (maxbar > maxValue.getValue())
        maxbar = maxValue.getValue();
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            mtl->diffuseColor.setValue(valueColors[c][0], valueColors[c][1],
                                       valueColors[c][2]);
            ...
            if (++i >= xDimension.getValue())
                break;
            tr = new SoTranslation();
            tr->translation.setValue(barXSpace, 0, 0);
            ysep->addChild(tr);
        }
        if (++j >= yDimension.getValue())
            break;
        tr = new SoTranslation();
