...
void
myProjectPoint(SoXtRenderArea *myRenderArea, int mousex, int mousey,
...
myAppEventHandler(void *userData, XAnyEvent *anyevent) {
    ...
    case ButtonPress:
        ...
        if (myButtonEvent->button == Button1) {
            myProjectPoint(myRenderArea, myButtonEvent->x, myButtonEvent->y,
                           vec);
    ...
    case MotionNotify:
        ...
        if (myMotionEvent->state & Button1Mask) {
            myProjectPoint(myRenderArea, myMotionEvent->x, myMotionEvent->y,
                           vec);
            myAddPoint(myRenderArea, vec);
        }
        break;
