...
#define GEN_VERTEX(pv, x, y, z, s, t, normal)                                  \
    point.setValue(halfWidth *x, halfHeight *y, halfDepth *z);                 \
    if (useTexFunc)                                                            \
...
Pyramid::getSize(float &halfWidth, float &halfHeight, float &halfDepth) const {
    halfWidth = (baseWidth.isIgnored() ? 1.0 : baseWidth.getValue() / 2.0);
    halfHeight = (height.isIgnored() ? 1.0 : height.getValue() / 2.0);
    halfDepth = (baseDepth.isIgnored() ? 1.0 : baseDepth.getValue() / 2.0);
}
