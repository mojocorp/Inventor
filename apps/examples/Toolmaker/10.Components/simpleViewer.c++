...
//
simpleViewer::simpleViewer(Widget parent, const char *name,
                           SbBool                    buildInsideParent,
                           SoXtFullViewer::BuildFlag b, SoXtViewer::Type t)
    : SoXtFullViewer(
          parent, name, buildInsideParent, b, t,
...
simpleViewer::rightWheelMotion(float newVal) {
    ...
    float newFocalDist = focalDistance / pow(2.0, newVal - rightWheelVal);
    ...
    SbVec3f forward(-mx[2][0], -mx[2][1], -mx[2][2]);
    camera->position =
        camera->position.getValue() + (focalDistance - newFocalDist) * forward;
...
simpleViewer::computeTranslateValues() {
    ...
    if (camera->isOfType(SoPerspectiveCamera::getClassTypeId())) {
        float angle =
            ((SoPerspectiveCamera *)camera)->heightAngle.getValue() / 2;
