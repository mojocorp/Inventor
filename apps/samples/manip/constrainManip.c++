...
main(int argc, char **argv) {
    ...
    if (!myInput.openFile(fileName)) {
        ...
        fprintf(stderr, "usage-- constrainManip [-noConstraints] ] "
                        "[-translateMin x] [-translateMax x] [-scaleMin x] "
                        "[-scaleMax x] [-rotateMin] [-rotateMax] fileName\n");
        fprintf(stderr, "        where translateMin, translateMax, scaleMin, "
                        "scaleMax, rotateMin, and rotateMax are constraints to "
                        "be put on the draggers and manips.\n");
        exit(1);
    }
    ...
    if (doConstraints) {
        ...
        for (i = 0; i < pathList.getLength(); i++) {
            ...
            cbs->translateMin.setValue(translateMin, translateMin,
                                       translateMin);
            cbs->translateMax.setValue(translateMax, translateMax,
                                       translateMax);
