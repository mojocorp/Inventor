...
//
static void
editMenuPick(Widget w,        // menu item chosen
             void * userData, // userData, specified in buildPulldownMenu
...
#define PUSH_ITEM(ITEM, NAME, KONST, FUNC)                                     \
    ITEM = XtCreateManagedWidget(NAME, xmPushButtonGadgetClass, pulldown,      \
                                 args, n);                                     \
    XtAddCallback(ITEM, XmNactivateCallback, (XtCallbackProc)FUNC,             \
                  (XtPointer)KONST)

// A toggle item has on/off state
#define TOGGLE_ITEM(ITEM, NAME, KONST, FUNC)                                   \
    ITEM = XtCreateManagedWidget(NAME, xmToggleButtonGadgetClass, pulldown,    \
                                 args, n);                                     \
    XtAddCallback(ITEM, XmNvalueChangedCallback, (XtCallbackProc)FUNC,         \
                  (XtPointer)KONST)

// A separator draws a line between menu items
#define SEP_ITEM(NAME)                                                         \
    (void)XtCreateManagedWidget("separator", xmSeparatorGadgetClass, pulldown, \
                                NULL, 0)
...
////////////////////////////////////////////////////////////////////////
{
    ...
    n = 0;
    XtSetArg(args[n], XmNaccelerator, openAccel);
    n++;
    XtSetArg(args[n], XmNacceleratorText, openAccelText);
    n++;
    PUSH_ITEM(items[0], "Open...", k_OPEN_ITEM, fileMenuPick);

    n = 0;
    XtSetArg(args[n], XmNaccelerator, saveAccel);
    n++;
    XtSetArg(args[n], XmNacceleratorText, saveAccelText);
    n++;
    PUSH_ITEM(items[1], "Save...", k_SAVE_ITEM, fileMenuPick);

    n = 0;
    XtSetArg(args[n], XmNaccelerator, printAccel);
    n++;
    XtSetArg(args[n], XmNacceleratorText, printAccelText);
    n++;
    ...
    n = 0;
    XtSetArg(args[n], XmNaccelerator, quitAccel);
    n++;
    XtSetArg(args[n], XmNacceleratorText, quitAccelText);
    n++;
...
////////////////////////////////////////////////////////////////////////
{
    ...
    XtAddCallback(pulldown, XmNmapCallback, (XtCallbackProc)editMenuDisplay,
                  (XtPointer)menuItems);

    n = 0;
    XtSetArg(args[n], XmNuserData, menuItems);
    n++;
...
////////////////////////////////////////////////////////////////////////
{
    ...
    n = 0;
    XtSetArg(args[n], XmNsubMenuId, pulldown1);
    n++;
    menuButtons[0] = XtCreateManagedWidget("File", xmCascadeButtonGadgetClass,
                                           menubar, args, n);

    n = 0;
    XtSetArg(args[n], XmNsubMenuId, pulldown2);
    n++;
    menuButtons[1] = XtCreateManagedWidget("Edit", xmCascadeButtonGadgetClass,
                                           menubar, args, n);
