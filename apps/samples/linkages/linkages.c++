...
readResources() {
    ...
    if (in.openFile("linkagesResourceFile.iv", TRUE)) {
        ...
        SoDebugError::postWarning("main::readResources",
                                  "Can't read resources since the file "
                                  "linkageResourceFile.iv was not found");
    }
}

static void
newLinkCB(void *data, SoDragger *) {
    SoSeparator *scene = (SoSeparator *)data;
    scene->addChild(new Link);
}

static void
newCrankCB(void *data, SoDragger *) {
    SoSeparator *scene = (SoSeparator *)data;
    scene->addChild(new Crank);
}

static void
newHingeCB(void *data, SoDragger *) {
    SoSeparator *scene = (SoSeparator *)data;
    scene->addChild(new RivetHinge);
}

static void
newDoubleLinkCB(void *data, SoDragger *) {
    SoSeparator *scene = (SoSeparator *)data;
    scene->addChild(new DoubleLink);
}

static void
newPistonCB(void *data, SoDragger *) {
    SoSeparator *scene = (SoSeparator *)data;
...
makeWriteButton(Widget parentWidget, SoSeparator *scene) {
    ...
    Widget writeButton =
        XmCreatePushButtonGadget(parentWidget, "writeButton", resources, n);
