...
class MySimpleMaterialEditor : public SoXtComponent {
  public:
    MySimpleMaterialEditor(Widget parent = NULL, const char *name = NULL,
    ...
    void addCallback(MySimpleMaterialEditorCB *f, void *userData = NULL) {
        callbackList.addCallback((SoCallbackListCB *)f, userData);
    }

    void removeCallback(MySimpleMaterialEditorCB *f, void *userData = NULL) {
        callbackList.removeCallback((SoCallbackListCB *)f, userData);
    }

  protected:
    // This constructor takes a boolean whether to build the widget now.
    // Subclasses can pass FALSE, then call
    // MySimpleMaterialEditor::buildWidget() when they are ready for it to be
    // built.
    SoEXTENDER
    MySimpleMaterialEditor(Widget parent, const char *name,
                           SbBool buildInsideParent, SbBool showMaterialName,
                           SbBool buildNow);
  ...
  private:
    ...
    static void pasteCB(Widget, MySimpleMaterialEditor *,
                        XmAnyCallbackStruct *);
