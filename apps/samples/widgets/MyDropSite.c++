...
void
MyDropSite::registerCallback(Widget site, MyIconDropCB *iconCB,
    ...
    // Without _FILE, the actual data is being dropped for us to read from a
    // buffer.
    //
    int n = 0;
...
void
MyDropSite::handleDrop(Widget, XtPointer client_data, XtPointer call_data,
                       XtPointer) {
    Arg args[10];
    int n = 0;
...
Atom
MyDropSite::chooseTarget(Atom *exportList, int numExportTargets) {
    Atom chosenType = 0;
...
MyDropSite::transferObjectProc(Widget w, XtPointer, Atom *, Atom *type,
    ...
    if (*type == _ATOM_("INVENTOR_2_1_FILE") ||
        *type == _ATOM_("VRML_1_0_FILE") || *type == _ATOM_("INVENTOR_FILE") ||
