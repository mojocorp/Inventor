...
MyTextureEditor::MyTextureEditor(Widget parent, const char *name,
    ...
    for (i = 0; i < IMAGE_TOTAL; i++) {
        ...
        textureNames[i].iconImage =
            new char[IMAGE_SIZE * IMAGE_SIZE * 3]; // for RGB
    }
...
////////////////////////////////////////////////////////////////////////
{
    ...
    texXfNode->center.setValue(SbVec2f(.5, .5) -
                               texXfNode->translation.getValue());
...
////////////////////////////////////////////////////////////////////////
{
    ...
    viewer =
        new SoXtExaminerViewer(form, NULL, TRUE, SoXtFullViewer::BUILD_NONE);
...
#define ADD_ENTRY(NAME, ID, ACC, ACCTEXT)                                      \
    ...
    subButtons[subButNum] =                                                    \
        XmCreatePushButtonGadget(subMenu, NAME, args, n + 2);                  \
...
#undef ADD_ENTRY
    ...
    widgetList[PALETTE_BUTTON] = buttons[butNum] =
...
////////////////////////////////////////////////////////////////////////
{
    ...
    widgetList[PALETTE_MENU] =
        XmCreatePulldownMenu(widgetList[MENU_BAR], "widgetList", NULL, 0);
...
////////////////////////////////////////////////////////////////////////
{
    static int   attributeList[] = {GLX_RGBA, GLX_RED_SIZE,  1, GLX_GREEN_SIZE,
                                  1,        GLX_BLUE_SIZE, 1, GLX_DEPTH_SIZE,
                                  1,        None};
    XVisualInfo *vis =
        glXChooseVisual(XtDisplay(parent),
                        XScreenNumberOfScreen(XtScreen(parent)), attributeList);
    if (!vis) {
#ifdef DEBUG
    ...
    XtSetArg(args[n], SoGLwNvisualInfo, vis);
    n++;
    XtSetArg(args[n], XtNwidth, GLX_SIZE);
    n++;
    XtSetArg(args[n], XtNheight, GLX_SIZE);
    n++;
    XtSetArg(args[n], SoGLwNrgba, TRUE);
    n++;
    // This makes sure we get the maximum buffer configuration
    // (by showing interest the number of bits)
    XtSetArg(args[n], SoGLwNredSize, 1);
    n++;
    XtSetArg(args[n], SoGLwNgreenSize, 1);
    n++;
    XtSetArg(args[n], SoGLwNblueSize, 1);
    n++;
    ...
    XtAddCallback(glx, SoGLwNexposeCallback,
                  (XtCallbackProc)MyTextureEditor::glxExposeCB,
                  (XtPointer)this);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    widgetList[SCALE_X_LABEL] = labels[3] =
        XmCreateLabelGadget(form, "Repeat X:", args, n);
    widgetList[SCALE_Y_LABEL] = labels[4] =
        XmCreateLabelGadget(form, "Repeat Y:", args, n);
...
#define CREATE_FIELD(ID)                                                       \
    ...
    XtAddCallback(fields[num], XmNvalueChangedCallback,                        \
                  (XtCallbackProc)MyTextureEditor::fieldChangedCB,             \
                  (XtPointer)this);                                            \
...
////////////////////////////////////////////////////////////////////////
{
    ...
    widgetList[MAPP_PULLDOWN] =
        XmCreatePulldownMenu(form, "widgetList", NULL, 0);
    n = 0;
    XtSetArg(args[n], XmNuserData, this);
    n++;

#define ADD_ENTRY(NAME, ID)                                                    \
    widgetList[ID] = list[num] =                                               \
        XmCreatePushButtonGadget(widgetList[MAPP_PULLDOWN], NAME, args, n);    \
    XtAddCallback(list[num++], XmNactivateCallback,                            \
                  (XtCallbackProc)MyTextureEditor::mappingMenuCB,              \
                  (XtPointer)ID);
...
#define ADD_ENTRY(NAME, ID)                                                    \
    widgetList[ID] = list[num] =                                               \
        XmCreatePushButtonGadget(widgetList[OPT_PULLDOWN], NAME, args, n);     \
    XtAddCallback(list[num++], XmNactivateCallback,                            \
                  (XtCallbackProc)MyTextureEditor::optionMenuCB,               \
                  (XtPointer)ID);
...
////////////////////////////////////////////////////////////////////////
{
    char *str = (currentItem < 0)
                    ? ((selectedItem < 0) ? (char *)" "
                                          : textureNames[selectedItem].name)
...
////////////////////////////////////////////////////////////////////////
{
    ...
    case ROT_FIELD:
        val = texXfNode->rotation.getValue() * 180.0 /
              M_PI; // ??? prevent roundoff
        intVal = int(val);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (paletteList.getLength() == 0) {
        ...
        SoDebugError::post(
            ...
            "setting the environment variable SO_TEXTURE_DIR to a directory "
            "which has texture files in it.",
            paletteDir);
#endif
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (cursor == 0) {
        ...
        Pixmap source =
            XCreateBitmapFromData(display, d, (char *)hourglass_bits,
                                  hourglass_width, hourglass_height);
        cursor =
            XCreatePixmapCursor(display, source, source, &foreground,
                                &foreground, hourglass_x_hot, hourglass_y_hot);
        XFreePixmap(display, source);
    }
    XDefineCursor(display, window, cursor);
    XSync(display,
          FALSE); // better than XFlush() (we get the glClear() to show up)
    ...
    if ((dirp = opendir(palDir)) != NULL) {
        ...
        if (pal->user) {
            // open the custom file and read the content of it (list of
            // textures)
            FILE *file;
        ...
        // system installed. Look for any image files under the current
        // directory
        else {
            struct dirent *direntry;
...
// Use: private
SbBool
MyTextureEditor::readScaledImage(char *file, int xsize, int ysize, char *buf,
                                 int &zsize)
//
////////////////////////////////////////////////////////////////////////
{
...
////////////////////////////////////////////////////////////////////////
{
    ...
    XtAddCallback(dialog, XmNokCallback,
                  (XtCallbackProc)MyTextureEditor::newDialogCB,
                  (XtPointer)this);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    // register callback to destroy (and not just unmap) the dialog
    XtAddCallback(dialog, XmNokCallback,
                  (XtCallbackProc)MyTextureEditor::deleteDialogCB,
                  (XtPointer)this);
    XtAddCallback(dialog, XmNcancelCallback,
                  (XtCallbackProc)MyTextureEditor::deleteDialogCB,
                  (XtPointer)this);
    ...
    if (fileName != NULL) {
        ...
        if (dialogImage != NULL) {
            ...
            XtVaSetValues(widgetList[DIALOG_IMAGE], XmNwidth,
                          dialogImageSize[0], XmNheight, dialogImageSize[1],
                          NULL);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    widgetList[DIALOG_INFO] = labels[0] =
        XmCreateLabelGadget(form, "imageInfo", NULL, 0);
    widgetList[DIALOG_NAME] = labels[1] =
        XmCreateLabelGadget(form, "imageName", NULL, 0);

    static int   attributeList[] = {GLX_RGBA, GLX_RED_SIZE,  1, GLX_GREEN_SIZE,
                                  1,        GLX_BLUE_SIZE, 1, GLX_DEPTH_SIZE,
                                  1,        None};
    XVisualInfo *vis =
        glXChooseVisual(XtDisplay(shell),
                        XScreenNumberOfScreen(XtScreen(shell)), attributeList);
    if (!vis) {
#ifdef DEBUG
    ...
    XtSetArg(args[n], SoGLwNblueSize, 1);
    n++;
    glx = XtCreateWidget("imageDialogGLX", SoglwMDrawingAreaWidgetClass, form,
                         args, n);
...
void
MyTextureEditor::glxEventCB(Widget, MyTextureEditor *p, XAnyEvent *xe,
                            Boolean *) {
    p->handleEvent(xe);
}
...
MyTextureEditor::imageDialogOpenCB(Widget, MyTextureEditor *p, void *) {
    if (p->widgetList[DIALOG_FILE_BROWSER] == NULL) {
        ...
        // ??? need to find a way to save the motif file browser current
        // directory
        // ??? for next time around (we really shouldn't delete this guy, but
        // its
