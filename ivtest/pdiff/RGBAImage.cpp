...
bool RGBAImage::WriteToFile(const char* filename)
{
    QImage img((unsigned char*)Data, Width, Height, QImage::Format_RGB32);
...
RGBAImage* RGBAImage::ReadFromFile(const char* filename)
{
    ...
    RGBAImage* result = new RGBAImage(img.width(), img.height(), filename);
    for (int y=0; y<img.height(); y++) {
        for (int x=0; x<img.width(); x++) {
            result->Set(x, y, img.pixel(x, y));
        }
    }
    return result;
}
