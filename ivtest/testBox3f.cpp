...
float nextRand(float rangeMin, float rangeMax) {
...
TEST(SbBox3f, BoxPlaneIntersection) {
    ...
    for (int i=0; i<1000; i++) {
        ...
        do
        {
            outside = SbVec3f(nextRand(-10.0f, 10.0f),
                              nextRand(-10.0f, 10.0f),
                              nextRand(-10.0f, 10.0f));
        }
        while ((outside - box.getCenter()).length() <= mindist);

        SbPlane plane(box.getCenter() - outside, (box.getCenter() - outside).length());

        EXPECT_FALSE(box.intersect(plane)) << outside[0] << " "<< outside[1] << " "<< outside[2];
    }
    ...
    for (int i=0; i<1000; i++) {
        ...
        do
        {
            inside = SbVec3f(nextRand(-10.0f, 10.0f),
                             nextRand(-10.0f, 10.0f),
                             nextRand(-10.0f, 10.0f));
        }
        ...
        do
        {
            normal = SbVec3f(nextRand(-1.0f, 1.0f),
                             nextRand(-1.0f, 1.0f),
                             nextRand(-1.0f, 1.0f));
        }
